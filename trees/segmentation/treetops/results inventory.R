...
elliottStands2016 = st_drop_geometry(st_read("GIS/Planning/Elliott State Forest + Hakki stands 2016.gpkg", layer = "unified stands 2016", quiet = TRUE)) %>%
  filter(isBuffer == 0) %>% # exclude entirely off Elliott stands
...
radius2021 = left_join(read_xlsx("D:/Elliott/GIS/DOGAMI/2021 OLC Coos County/treetops/radius/standsByHeightClass.xlsx"), # trees by tile, stand, and height class
                       elliottStands2016 %>% select(standID2016, standArea, standAge2016, vegStrata, isPlantation, isBuffer),
...
organon2021 = left_join(read_feather(file.path(getwd(), "trees/Organon/Elliott tree lists 2016-2116.feather"), mmap = FALSE) %>%
                          ...
                          summarize(species = species[1], standAge = standAge[1] + 1, # model initialized in 2016 with start of growing season, convert to end of 2021 growing season (model year 2022) since LiDAR and imaging flights were August 30, September 13, and 15
                                    ...
                                    .groups = "drop"),
                        elliottStands2016 %>% select(standID2016, standArea, vegStrata, isPlantation, isBuffer),
...
randomForestStands2021 = randomForest2021 %>% 
  ...
  mutate(topHeightTreetops = pmin(treetops, 100 * standArea),
  ...
  summarize(treetops = sum(treetops), mergePoints = sum(mergePoints), noisePoints = sum(noisePoints), maybeNoisePoints = sum(maybeNoisePoints),
            standArea = standArea[1], standAge2016 = standAge2016[1], vegStrata = vegStrata[1], isPlantation = isPlantation[1], isBuffer = isBuffer[1],
            treetopsPerHectare = sum(treetops) / standArea[1],
...
cruise2016 = organonStands2021 %>% group_by(isPlantation) %>% summarize(stands = n(), areaHa = sum(standArea))
forest = elliottStands2016 %>% group_by(isPlantation) %>% summarize(stands = n(), areaHa = sum(standArea))
organon2021forestTotal = organon2021 %>% group_by(isPlantation) %>% 
  summarize(method = "ground inventory", stemsInCruisedArea = sum(standArea * (liveExpansionFactor + deadExpansionFactor))) %>% mutate(cruise2016 = cruise2016$areaHa, forestAreaHa = forest$areaHa, stemsInForest = forestAreaHa / cruise2016 * stemsInCruisedArea, stemsPerHectare = stemsInForest / forestAreaHa) # or tph = stemsInCruisedArea / cruise2016
...
organon2021densityByHeight = organon2021 %>% mutate(heightClassInM = round(height)) %>% 
  ...
  summarize(isPlantation = isPlantation[1], standArea = standArea[1], stemsPerHectare = sum(liveExpansionFactor + deadExpansionFactor), .groups = "drop") %>%
  group_by(isPlantation, heightClassInM) %>%
  summarize(cruisedStands = if_else(isPlantation[1], cruise2016$stands[2], cruise2016$stands[1]), # modelAreaWithHeightClassHa = sum(standArea), 
            cruiseAreaHa = if_else(isPlantation[1], cruise2016$areaHa[2], cruise2016$areaHa[1]),
            stemsInModel = sum(standArea * stemsPerHectare), 
            stemsInModelSigma = sqrt(cruisedStands / (cruisedStands - 1) * (sum((standArea * stemsPerHectare - stemsInModel / cruisedStands)^2)) + (cruisedStands - n()) * (0 - stemsInModel / cruisedStands)^2), # calculate standard deviation accounting for stands which aren't in group and therefore have zero stems per hectare
...
organon2021densityByHeight %>% group_by(isPlantation) %>% summarize(stemsInModel = sum(stemsInModel)) %>%
  mutate(modeledAreaHa = cruise2016$areaHa, stemsPerHectare = stemsInModel / modeledAreaHa, forestAreaHa = forest$areaHa, stemsInForest = stemsInModel * forestAreaHa / modeledAreaHa) # 3.2 + 3.9 = 7.1 M
...
randomForest2021densityByHeight = randomForest2021 %>% # mutate(heightClassInM = heightClassSize * round(heightClassInM / heightClassSize)) %>% 
  ...
  summarize(isPlantation = isPlantation[1], standArea = standArea[1], treetops = sum(treetops), .groups = "drop") %>%
  group_by(isPlantation, heightClassInM) %>%
  summarize(forestAreaHa = if_else(isPlantation[1], forest$areaHa[2], forest$areaHa[1]), forestAreaWithHeightClassHa = sum(standArea), treetops = sum(treetops), treetopsPerHectare = treetops / forestAreaHa, .groups = "drop")
...
combined2021densityByHeight = full_join(randomForest2021densityByHeight, organon2021densityByHeight, by = join_by(isPlantation, heightClassInM))


## Figure 11: whole forest inventory summary
stems2021 = bind_rows(randomForest2021, radius2021, organon2021forestTotal %>% select(method, isPlantation, stemsInForest) %>% rename(treetops = stemsInForest)) %>% 
  group_by(method, isPlantation) %>%
  summarize(stems = sum(treetops), .groups = "drop_last") %>%
  mutate(isPlantation = factor(isPlantation, levels = c(TRUE, FALSE)),
...
ggplot() +
  ...
  labs(x = "millions of stems", y = NULL, color = NULL, fill = NULL, title = paste("                       ", plotLetters[1], "individual tree inventory")) +
  scale_x_continuous(labels = scales::label_number(scale = 1E-6)) +
  theme(plot.margin = margin(l = 5, r = 25)) +
ggplot() +
  ...
  guides(fill = "none") +
  labs(x = bquote("mean stems ha"^-1), y = NULL, fill = NULL, title = paste(plotLetters[2], "natural regeneration")) +
...
ggplot() +
  ...
  guides(fill = "none") +
  labs(x = bquote("mean stems ha"^-1), y = NULL, fill = NULL, title = paste(plotLetters[3], "plantation density")) +
  scale_y_discrete(labels = NULL) +
...
ggplot() +
  ...
  geom_line(aes(x = stemsPerHectare, y = heightClassInM, alpha = heightClassInM >= 5, color = "Organon grown\nground inventory"), combined2021densityByHeight %>% filter(isPlantation == FALSE), orientation = "y") +
  geom_line(aes(x = treetopsPerHectare, y = heightClassInM, alpha = heightClassInM >= 5, color = "LiDAR"), combined2021densityByHeight %>% filter(isPlantation == FALSE), orientation = "y") +
...
ggplot() +
  ...
  geom_segment(aes(x = 0, y = 4.5, xend = 45, yend = 4.5), color = "grey20", linetype = "dashed", linewidth = 0.2) +
  geom_line(aes(x = stemsPerHectare, y = heightClassInM, alpha = heightClassInM >= 5, color = "Organon grown\nground inventory"), combined2021densityByHeight %>% filter(isPlantation), orientation = "y") +
  geom_line(aes(x = treetopsPerHectare, y = heightClassInM, alpha = heightClassInM >= 5, color = "LiDAR"), combined2021densityByHeight %>% filter(isPlantation), orientation = "y") +
...
plot_layout(nrow = 1, guides = "collect") &
  ...
  scale_color_manual(breaks = c("LiDAR", "Organon grown\nground inventory"), values = c("blue", "forestgreen")) &
