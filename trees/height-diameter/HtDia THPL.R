...
thplHeightFromDiameterChapmanRichards = nls(TotalHt ~ 1.37 + (a1 + a1p * isPlantation) *(1 - exp(b1*DBH))^(b2 + b2p * isPlantation), thpl2016, start = list(a1 = 65.3, a1p = -13.1, b1 = -0.022, b2 = 1.51, b2p = -0.31), weights = pmin(DBH^-2, 1)) # b1p not significant
thplHeightFromDiameterChapmanRichardsBal = nls(TotalHt ~ 1.37 + (a1 + a1p * isPlantation + (a2 + a2p * isPlantation) * basalAreaLarger + (a3 + a3p * isPlantation) * standBasalAreaPerHectare) * (1 - exp((b1 + b1p * isPlantation)*DBH))^(b2 + b2p * isPlantation), thpl2016, start = list(a1 = 64.9, a1p = 3.8, a2 = 0.023, a2p = 0.92, a3 = 0.022, a3p = -0.22, b1 = -0.021, b1p = 0.0066, b2 = 1.47, b2p = -0.29), weights = pmin(DBH^-2, 1))
thplHeightFromDiameterChapmanRichardsBalPhysio = nls(TotalHt ~ 1.37 + (a1 + (a2 + a2p * isPlantation) * basalAreaLarger + a3 * elevation + a4 * slope + a5 * sin(pi/180 * aspect) + a6 * cos(pi/180 * aspect)) * (1 - exp((b1 + b1p * isPlantation)*DBH))^(b2 + b2p * isPlantation), thpl2016, start = list(a1 = 69.4, a2 = 0.077, a2p = 0.72, a3 = -0.008, a4 = -0.031, a5 = 0.548, a6 = 0.980, b1 = -0.021, b1p = 0.0075, b2 = 1.49, b2p = -0.370), weights = pmin(DBH^-2, 1))
thplHeightFromDiameterChapmanRichardsBalRelHt = nls(TotalHt ~ 1.37 + (a1 + a1p * isPlantation + (a2 + a2p * isPlantation) * basalAreaLarger + (a3 + a3p * isPlantation) * standBasalAreaPerHectare + (a4 + a4p * isPlantation) * relativeHeight) * (1 - exp(b1*DBH))^(b2 + b2p * isPlantation), thpl2016, start = list(a1 = 8.7, a1p = 11.1, a2 = 0.18, a2p = 0.42, a3 = -0.0087, a3p = 0.070, a4 = 54.0, a4p = -28.4, b1 = -0.021, b2 = 0.65, b2p = 0.37), weights = pmin(DBH^-2, 1))
thplHeightFromDiameterChapmanRichardsPhysio = nls(TotalHt ~ 1.37 + (a1 + a1p * isPlantation + a2 * elevation + a3 * sin(pi/180 * slope) + a4 * cos(pi/180 * aspect) + a5 * sin(pi/180 * aspect) + a6 * topographicShelterIndex) * (1 - exp(b1*DBH))^(b2 + b2p * isPlantation), thpl2016, start = list(a1 = 68.5, a1p = -13.4, a2 = -0.0045, a3 = -8.09, a4 = 0.783, a5 = 0.766, a6 = 0.213, b1 = -0.022, b2 = 1.50, b2p = -0.31), weights = pmin(DBH^-2, 1)) # a4p not significant, a5p induces overfitting
thplHeightFromDiameterCurtis = nls(TotalHt ~ 1.37 + a1 * DBH / (1 + DBH)^b1, thpl2016, start = list(a1 = 1.6, b1 = 0.24), weights = pmin(DBH^-2, 1))
thplHeightFromDiameterHossfeld = nls(TotalHt ~ 1.37 + (a1 + a1p * isPlantation) / (1 + (b1 + b1p * isPlantation) *DBH^(b2 + b2p * isPlantation)), thpl2016, start = list(a1 = 75.4, a1p = -11.4, b1 = 462, b1p = -322, b2 = -1.54, b2p = 0.28), weights = pmin(DBH^-2, 1))
thplHeightFromDiameterKorf = nls(TotalHt ~ 1.37 + (a1 + a1p * isPlantation)*exp((b1 + b1p * isPlantation)*DBH^(b2 + b2p * isPlantation)), thpl2016, start = list(a1 = 102, a1p = 92, b1 = -17.7, b1p = 10.3, b2 = -0.725, b2p = 0.365), weights = pmin(DBH^-2, 1))
thplHeightFromDiameterLinear = lm(TotalHt ~ 0 + DBH + I(isPlantation*DBH), thpl2016, offset = breastHeight, weights = pmin(DBH^-2, 1))
thplHeightFromDiameterMichaelisMenten = nls(TotalHt ~ 1.37 + (a1 + a1p * isPlantation) * DBH^b1 / (a2 + a2p * isPlantation + DBH^b1), thpl2016, start = list(a1 = 72.1, a1p = -20.4, a2 = 208, a2p = -75.5, b1 = 1.18), weights = pmin(DBH^-2, 1)) # b1p not significant
...
thplHeightFromDiameterProdan = nls(TotalHt ~ 1.37 + DBH^2 / (a1*DBH^2 + (a2 + a2p * isPlantation)*DBH + a3 + a3p* isPlantation), thpl2016, start = list(a1 = 0.012, a2 = 0.41, a2p = 0.47, a3 = 17.9, a3p = -14.6), weights = pmin(DBH^-2, 1))
thplHeightFromDiameterPower = nls(TotalHt ~ 1.37 + (a1 + a1p * isPlantation)*DBH^(b1 + b1p * isPlantation), thpl2016, start = list(a1 = 2.9, a1p = -1.6, b1 = 0.63, b1p = 0.18), weights = pmin(DBH^-2, 1))
thplHeightFromDiameterRatkowsky = nls(TotalHt ~ 1.37 + (a1 + a1p * isPlantation)*exp((b1 + b1p * isPlantation)/(DBH + b2 + b2p * isPlantation)), thpl2016, start = list(a1 = 90.0, a1p = -25.8, b1 = -55.2, b1p = 14.5, b2 = 10.0, b2p = -1.52), weights = pmin(DBH^-2, 1))
thplHeightFromDiameterRichards = nls(TotalHt ~ 1.37 + (Ha + Hap*isPlantation) * (1 + ((1.37/Ha)^(1 - (d + dp*isPlantation)) - 1) * exp((-(kU + kUp * isPlantation) * DBH)/(d + dp*isPlantation)^((d + dp*isPlantation)/(1 - (d + dp*isPlantation)))))^(1/(1 - (d + dp*isPlantation))), thpl2016, start = list(Ha = 55.7, Hap = -29.2, d = 0.517, dp = 0.318, kU = 0.008, kUp = 0.016), weights = pmin(DBH^-2, 1))
thplHeightFromDiameterSharmaParton = nls(TotalHt ~ 1.37 + a1*topHeight^(a2 + a2p * isPlantation)*(1 - exp((b1 + b1p * isPlantation)*(tph/standBasalAreaPerHectare)^(b2 + b2p * isPlantation)*DBH))^(b3 + b3p * isPlantation), thpl2016, start = list(a1 = 22.6, a2 = 0.26, a2p = -0.050, b1 = -0.021, b1p = -0.014, b2 = 0.025, b2p = -0.187, b3 = 1.51, b3p = -0.44), weights = pmin(DBH^-2, 1))
thplHeightFromDiameterSharmaPartonBal = nls(TotalHt ~ 1.37 + (a1 + a1p * isPlantation)*topHeight^(a2 + a2p * isPlantation) * (1 - exp((b1 + b1p * isPlantation)*(tph/(standBasalAreaPerHectare + basalAreaLarger))^(b2 + b2p * isPlantation)*DBH))^(b3 + b3p * isPlantation), thpl2016, start = list(a1 = 18.5, a1p = 11.3, a2 = 0.30, a2p = -0.14, b1 = -0.019, b1p = -0.011, b2 = 0.089, b2p = -0.266, b3 = 1.49, b3p = -0.44), weights = pmin(DBH^-2, 1))
thplHeightFromDiameterSharmaPartonBalPhysio = nls(TotalHt ~ 1.37 + (a1 + a1p * isPlantation)*topHeight^(a2 + a2p * isPlantation) * (1 + a3 * elevation + a4 * sin(pi/180 * aspect) + a5 * cos(pi/180 * aspect) + a6 * topographicShelterIndex) * (1 - exp((b1 + b1p * isPlantation)*(tph/(standBasalAreaPerHectare + basalAreaLarger))^(b2 + b2p * isPlantation)*DBH))^(b3 + b3p * isPlantation), thpl2016, start = list(a1 = 16.5, a1p = 12.4, a2 = 0.33, a2p = -0.162, a3 = -0.00008, a4 = 0.0090, a5 = 0.0045, a6 = 0.00256, b1 = -0.020, b1p = -0.0091, b2 = 0.062, b2p = -0.235, b3 = 1.50, b3p = -0.45), weights = pmin(DBH^-2, 1))
thplHeightFromDiameterSharmaPartonPhysio = nls(TotalHt ~ 1.37 + (a1 + a1p * isPlantation)*topHeight^(a2 + a2p * isPlantation) * (1 + a3 * elevation + a4 * sin(pi/180 * aspect) + a5 * cos(pi/180 * aspect) + a6 * topographicShelterIndex) * (1 - exp((b1 + b1p * isPlantation)*(tph/(standBasalAreaPerHectare))^(b2 + b2p * isPlantation)*DBH))^(b3 + b3p * isPlantation), thpl2016, start = list(a1 = 17.6, a1p = 5.06, a2 = 0.31, a2p = -0.106, a3 = -0.00008, a4 = 0.0092, a5 = 0.0046, a6 = 0.00257, b1 = -0.023, b1p = -0.012, b2 = 0.0010, b2p = -0.159, b3 = 1.52, b3p = -0.46), weights = pmin(DBH^-2, 1))
thplHeightFromDiameterSharmaZhang = nls(TotalHt ~ 1.37 + (a1 + a1p * isPlantation)*standBasalAreaPerHectare^(a2 + a2p * isPlantation)*(1 - exp((b1 + b1p * isPlantation)*tph^(b2 + b2p * isPlantation)*DBH))^(b3 + b3p * isPlantation), thpl2016, start = list(a1 = 18.2, a1p = 7.93, a2 = 0.281, a2p = -0.187, b1 = -0.322, b1p = 0.293, b2 = -0.505, b2p = 0.463, b3 = 1.133, b3p = 0.020), weights = pmin(DBH^-2, 1))
thplHeightFromDiameterSharmaZhangBal = nls(TotalHt ~ 1.37 + (a1 + a1p * isPlantation)*standBasalAreaPerHectare^(a2 + a2p * isPlantation) * (1 + (a3 + a3p * isPlantation) * basalAreaLarger) * (1 - exp((b1 + b1p * isPlantation)*tph^(b2 + b2p * isPlantation)*DBH))^(b3 + b3p * isPlantation), thpl2016, start = list(a1 = 18.2, a1p = 7.93, a2 = 0.281, a2p = -0.187, a3 = 0, a3p = 0, b1 = -0.322, b1p = 0.293, b2 = -0.505, b2p = 0.463, b3 = 1.133, b3p = 0.020), weights = pmin(DBH^-2, 1))
thplHeightFromDiameterSibbesen = nls(TotalHt ~ 1.37 + (a1 + a1p * isPlantation)*DBH^((b1 + b1p * isPlantation)*DBH^(b2 + b2p * isPlantation)), thpl2016, start = list(a1 = 0.0019, a1p = 0.163, b1 = 4.98, b1p = -2.72, b2 = -0.175, b2p = 0.0427), weights = pmin(DBH^-2, 1))
thplHeightFromDiameterWeibull = nls(TotalHt ~ 1.37 + (a1 + a1p * isPlantation)*(1 - exp((b1 + b1p * isPlantation)*DBH^(b2 + b2p * isPlantation))), thpl2016, start = list(a1 = 63.6, a1p = -12.7, b1 = -0.00516, b1p = -0.00652, b2 = 1.29, b2p = -0.16), weights = pmin(DBH^-2, 1))
thplHeightFromDiameterWeibullBal = nls(TotalHt ~ 1.37 + (a1 + (a2 + a2p * isPlantation) * basalAreaLarger + (a3 + a3p * isPlantation) * standBasalAreaPerHectare) * (1 - exp((b1 + b1p * isPlantation)*DBH^(b2 + b2p * isPlantation))), thpl2016, start = list(a1 = 63.6, a2 = 0.035, a2p = 0.832, a3 = 0.0120, a3p = -0.184, b1 = -0.0052, b1p = -0.0024, b2 = 1.281, b2p = -0.133), weights = pmin(DBH^-2, 1))
thplHeightFromDiameterWeibullBalRelHt = nls(TotalHt ~ 1.37 + (a1  + a1p * isPlantation + (a2 + a2p * isPlantation) * basalAreaLarger + (a3 + a3p * isPlantation) * standBasalAreaPerHectare + (a4 + a4p * isPlantation) * pmin(relativeHeight, 1.25)) * (1 - exp((b1 + b1p * isPlantation) * (1 + (b2 + b2p * isPlantation) * pmin(relativeHeight, 1.25)) * DBH^b3)), thpl2016, start = list(a1 = -3.82, a1p = 87.0, a2 = 0.117, a2p = 1.86, a3 = -0.00099, a3p = -0.0684, a4 = 68.2, a4p = -53.9, b1 = -0.146, b1p = 0.140, b2 = -0.610, b2p = 2.25, b3 = 0.800), weights = pmin(DBH^-2, 1))
...
thplHeightFromDiameterResults = tribble(~method, ~pae, ~paeNR, ~paePl, ~bias, ~biasNR, ~biasPl, ~mae, ~maeNR, ~maePl, ~rmse, ~rmseNR, ~rmsePl, ~nse, ~nseNR, ~nsePl, ~pearson, ~pearsonNR, ~pearsonPl, ~aic, ~bic, ~power,
                                        ...
                                        "Michaelis-Menten", !!!as_row(thplHeightFromDiameterMichaelisMenten),
...
#thplDiameterFromHeightChapmanRichardsPhysio = nls_multstart(DBH ~ (a1 + a1p * isPlantation + a2 * elevation + a3 * sin(pi/180 * slope) + a4 * cos(pi/180 * aspect) + a5 * sin(pi/180 * aspect) + a6 * topographicShelterIndex)*log(1 - pmin((b1 + b1p * isPlantation)*(TotalHt - 1.37)^b2, 0.999999)), thpl2016, iter = 100,
#                                                            start_lower = list(a1 = -200, a1p = -100, a2 = -0.1, a3 = -1, a4 = -1, a5 = -1, a6 = -0.1, b1 = -0.1, b1p = -1, b2 = -1, pi = 3.1415296), 
#                                                            start_upper = list(a1 = 1, a1p = 100, a2 = 0.1, a3 = 1, a4 = 1, a5 = 1, a6 = 0.1, b1 = 0.1, b1p = 0.1, b2 = 1, pi = 3.1415296), modelweights = pmin(TotalHt^-2, 0.5))
...
thplDiameterFromHeightChapmanRichards = nls(DBH ~ a1*log(1 - pmin(b1*(TotalHt - 1.37)^b2, 0.9999)), thpl2016, start = list(a1 = -763, b1 = 0.0027, b2 = 1.05), weights = pmin(TotalHt^-2, 0.5)) # a1p and b2p not significant, poor convergence with b1p
thplDiameterFromHeightChapmanRichardsAal = nls(DBH ~ (a1 + a2 * tallerQuasiBasalArea)*log(1 - pmin(b1*(TotalHt - 1.37)^b2, 0.9999)), thpl2016, start = list(a1 = -100, a2 = 2, b1 = 0.0006, b2 = 1.07), weights = pmin(TotalHt^-2, 0.5)) # a1p, b1p not significant
thplDiameterFromHeightChapmanRichardsPhysio = nls(DBH ~ (a1 + a1p * isPlantation + a2 * elevation + a3 * sin(pi/180 * slope) + a4 * cos(pi/180 * aspect) + a5 * sin(pi/180 * aspect) + a6 * topographicShelterIndex)*log(1 - pmin((b1 + b1p * isPlantation)*(TotalHt - 1.37)^b2, 0.9999)), thpl2016, start = list(a1 = -500, a1p = 200, a2 = 0.07, a3 = 9.8, a4 = -7.2, a5 = -1.3, a6 = 0.7, b1 = 0.0047, b1p = 0.007, b2 = 1.005), weights = pmin(TotalHt^-2, 0.5), control = list(maxiter = 50)) # no physiographic effects significant
thplDiameterFromHeightChapmanRichardsRelHt = nls(DBH ~ (a1 + a2 * relativeHeight)*log(1 - pmin(b1*(TotalHt - 1.37)^b2, 0.999999)), thpl2016, start = list(a1 = -100, a2 = -100, b1 = 0.0014, b2 = 1.02), weights = pmin(TotalHt^-2, 0.5))
...
thplDiameterFromHeightPower = nls(DBH ~ a1*(TotalHt - 1.37)^b1, thpl2016, start = list(a1 = 1.93, b1 = 1.08), weights = pmin(TotalHt^-2, 0.5)) # no significant plantation effects
thplDiameterFromHeightPowerAal = nls(DBH ~ (a1 + a2 * tallerQuasiBasalArea)*(TotalHt - 1.37)^b1, thpl2016, start = list(a1 = 1.94, a2 = -0.00051, b1 = 1.09), weights = pmin(TotalHt^-2, 0.5)) # no significant plantation effects
thplDiameterFromHeightPowerPhysio = nls(DBH ~ (a1 + a2 * elevation + a3 * sin(pi/180 * slope) + a4 * cos(pi/180 * aspect) + a5 * sin(pi/180 * aspect) + a6 * topographicShelterIndex)*(TotalHt - 1.37)^b1, thpl2016, start = list(a1 = 2.26, a2 = -0.00047, a3 = -0.156, a4 = -0.0018, a5 = 0.0208, a6 = -0.0060, b1 = 1.08), weights = pmin(TotalHt^-2, 0.5)) # no significant physiographic effects
thplDiameterFromHeightPowerRelHt = nls(DBH ~ (a1 + (a2 + a2p * isPlantation) * relativeHeight)*(TotalHt - 1.37)^b1, thpl2016, start = list(a1 = 1.68, a2 = -0.11, a2p = 0.23, b1 = 1.13), weights = pmin(TotalHt^-2, 0.5)) # a1p and b1p not significant
#thplDiameterFromHeightSharmaParton = nls(DBH ~ a1*(TotalHt - 1.37)^a2*(exp(b1*(tph/topHeight)^b2*(TotalHt - 1.37)) - 1)^b3, thpl2016, start = list(a1 = 0.91, a2 = 1.39, b1 = 0.087, b2 = -0.029, b3 = -0.12), weights = pmin(TotalHt^-2, 0.5)) # NaN-inf from nls_multstart() point
#thplDiameterFromHeightSharmaParton = nls(DBH ~ a1*(TotalHt - 1.37)^(a2 + a2p * isPlantation)*(exp(b1*(tph/topHeight)^(b2 + b2p * isPlantation)*(TotalHt - 1.37)) - 1)^(b3 + b3p * isPlantation), thpl2016, start = list(a1 = 2.86, a2 = 0.790, a2p = -0.130, b1 = 0.216, b2 = -0.153, b2p = 0.184, b3 = 0.045, b3p = 0.0278), weights = pmin(TotalHt^-2, 0.5)) # step factor
thplDiameterFromHeightSibbesenForm = nls(DBH ~ a1*(TotalHt - 1.37)^(b1*(TotalHt - 1.37)^b2), thpl2016, start = list(a1 = 1.42, b1 = 1.29, b2 = -0.27), weights = pmin(TotalHt^-2, 0.5)) # no significant plantation effects
thplDiameterFromHeightSibbesenFormAal = nls(DBH ~ (a1 + a2 * tallerQuasiBasalArea)*(TotalHt - 1.37)^(b1*(TotalHt - 1.37)^b2), thpl2016, start = list(a1 = 1.39, a2 = -0.00036, b1 = 1.31, b2 = -0.029), weights = pmin(TotalHt^-2, 0.5)) # no significant plantation effects
thplDiameterFromHeightSibbesenFormPhysio = nls(DBH ~ (a1 + a1p * isPlantation + a2 * elevation + a3 * sin(pi/180 * slope) + a4 * cos(pi/180 * aspect) + a5 * sin(pi/180 * aspect) + a6 * topographicShelterIndex)*(TotalHt - 1.37)^(b1*(TotalHt - 1.37)^(b2 + b2p * isPlantation)), thpl2016, start = list(a1 = 2.32, a1p = -0.294, a2 = -0.00069, a3 = -0.116, a4 = 0.0374, a5 = 0.0047, a6 = -0.0056, b1 = 1.084, b2 = -0.0048, b2p = 0.0203), weights = pmin(TotalHt^-2, 0.5)) # no physiographic effects significant
thplDiameterFromHeightSibbesenFormRelHt = nls(DBH ~ (a1 + a2 * relativeHeight)*(TotalHt - 1.37)^(b1*(TotalHt - 1.37)^b2), thpl2016, start = list(a1 = 1.34, a2 = 0.30, b1 = 1.32, b2 = -0.040), weights = pmin(TotalHt^-2, 0.5))
...
thplParameters = bind_rows(bind_rows(bind_rows(c(method = "Chapman-Richards", thplHeightFromDiameterChapmanRichards$m$getPars())),
                                     ...
                                     bind_rows(c(method = "Michaelis-Menten", thplHeightFromDiameterMichaelisMenten$m$getPars())),
