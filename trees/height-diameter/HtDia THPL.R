...
thplHeightFromDiameter$gamPhysio = fit_gam("REML GAM phyiso", TotalHt ~ s(DBH, elevation, slope, topographicShelterIndex, bs = "ts", by = as.factor(isPlantation), k = 18, pc = thpl2016gamConstraint), data = thpl2016physio) # k reduces from 85 to 18 without aspect
...
thplHeightFromDiameter$resultsGnls %>% select(-responseVariable, -species, -biasNR, -biasPl, -rmse, -rmseNR, -rmsePl, -pearsonNR, -pearsonPl, -aic, -bic) %>% arrange(method)
...
thplDiameterFromHeight$chapmanRichardsAat = fit_gsl_nls(DBH ~ (a1 + a2 * tallerApproxBasalArea)*log(1 - pmin(b1*(TotalHt - 1.37)^b2, 0.9999)), thpl2016, start = list(a1 = -1200, a2 = 0, b1 = 0.0016, b2 = 1.06), control = nls.control(maxiter = 500), significant = FALSE) # a1p, b1p not significant, step factor with nlrob()
...
thplDiameterFromHeight$chapmanRichardsRelHt = fit_gsl_nls(DBH ~ (a1 + a9 * relativeHeight)*log(1 - pmin(b1*(TotalHt - 1.37)^b2, 0.9999)), thpl2016, start = list(a1 = -350, a9 = -66, b1 = 0.006, b2 = 0.98), control = list(maxiter = 500), significant = FALSE) # step factor with nlrob()
thplDiameterFromHeight$gam = fit_gam("REML GAM", DBH ~ s(TotalHt, by = as.factor(isPlantation), k = 5, pc = thpl2016gamConstraint), data = thpl2016, method = "REML", select = TRUE, weights = heightWeight) # newton() step failure with scat()
thplDiameterFromHeight$gamAat = fit_gam("REML GAM AA+T", DBH ~ s(TotalHt, tallerApproxBasalArea, standBasalAreaApprox, by = as.factor(isPlantation), k = 12, pc = thpl2016gamConstraint), data = thpl2016, method = "REML", select = TRUE, weights = heightWeight)
thplDiameterFromHeight$gamAatPhysio = fit_gam("REML GAM AA+T physio", DBH ~ s(TotalHt, tallerApproxBasalArea, standBasalAreaApprox, topographicShelterIndex, by = as.factor(isPlantation), k = 16, pc = thpl2016gamConstraint), data = thpl2016physio, method = "REML", select = TRUE, weights = heightWeight) # insufficient data
thplDiameterFromHeight$gamPhysio = fit_gam("REML GAM physio", DBH ~ s(TotalHt, elevation, slope, topographicShelterIndex, by = as.factor(isPlantation), k = 17, pc = thpl2016gamConstraint), data = thpl2016physio, method = "REML", select = TRUE, weights = heightWeight)
thplDiameterFromHeight$linear = fit_lm("linear", DBH ~ 0 + I(TotalHt - 1.37), thpl2016) # isPlantation*(TotalHt - 1.37) not significant
thplDiameterFromHeight$michaelisMentenForm = fit_nlrob("Michaelis-Menten", DBH ~ a1 * (TotalHt - 1.37)^b1 / (a2 - (TotalHt - 1.37)^b1), thpl2016, start = list(a1 = 519, a2 = 237, b1 = 1.00)) # a1p, a2p, b1p not significant
...
thplDiameterFromHeight$weibull = fit_nlrob("Sibbesen weibull", DBH ~ (a1*log(1 - pmin(b1*(TotalHt - 1.37), 0.9999)))^b2, thpl2016, start = list(a1 = -330, b1 = 0.0065, b2 = 1.01)) # a1p, b1p, b2p not significant
...
thplDiameterFromHeightResults = bind_rows(lapply(thplDiameterFromHeight, as_row)) %>%
  ...
  arrange(desc(deltaAic))
print(thplDiameterFromHeight$results %>% select(-responseVariable, -species, -biasNR, -biasPl, -rmse, -rmseNR, -rmsePl, -pearsonNR, -pearsonPl, -aic, -bic), n = 25)
...
thplParameters = bind_rows(bind_rows(lapply(thplHeightFromDiameter, get_coefficients),
                                     #get_coefficients(thplHeightFromDiameter$gamBalPhysio),
                                     lapply(thplHeightFromDiameterGnls, get_coefficients)) %>%
                             mutate(responseVariable = "height"),
                           bind_rows(lapply(thplDiameterFromHeight, get_coefficients)) %>%
                                     #get_coefficients(thplDiameterFromHeight$gamAatPhysio),
