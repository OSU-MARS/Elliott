...
if (otherOptions$fitDbh)
{
  ...
  otherDiameterFromHeight$chapmanReplaceRelHt = fit_gsl_nls("Chapman-Richards replace RelHt", DBH ~ (a1 + a9 * relativeHeight)*(exp(b1*(TotalHt - 1.37)) - 1)^b2, other2016, start = list(a1 = 10, a9 = 0, b1 = 0.3, b2 = 0.3), significant = FALSE) # a9 not significant, step factor with nls()
  ...
  #otherDiameterFromHeight$schnute = fit_gsl_nls("Schnute inverse", DBH ~ -1/a1 * log(1 - (1 - exp(-a2))*(TotalHt^b1 - 1.37^b1)/(Ha^b1 - 1.37^b1)), other2016, start = list(a1 = 0.00003, a2 = 0.01, b1 = 1.20, Ha = 50), control = gsl_nls_control(maxiter = 100)) # NaN-inf with nlrob(), NaNs with gsl_nls()
  ...
  #bind_rows(lapply(otherDiameterFromHeight$chapmanReplaceAbat$fit, get_model_coefficients))
  ...
  otherDiameterFromHeightGslNlsDefault$chapmanReplaceRelHt = fit_gsl_nls("Chapman-Richards replace RelHt", DBH ~ (a1 + a9 * relativeHeight)*(exp(b1*(TotalHt - 1.37)) - 1)^b2, other2016defaultWeight, start = list(a1 = 10, a9 = 0, b1 = 0.3, b2 = 0.3), significant = FALSE)
  ...
  #otherDiameterFromHeightGslNlsDefault$schnute = fit_gsl_nls("Schnute inverse", DBH ~ -1/a1 * log(1 - (1 - exp(-a2))*(TotalHt^b1 - 1.37^b1)/(Ha^b1 - 1.37^b1)), other2016defaultWeight, start = list(a1 = 0.00003, a2 = 0.01, b1 = 1.20, Ha = 50), control = gsl_nls_control(maxiter = 100))
...
if (otherOptions$fitHeightGnls)
{
  ...
  otherDiameterFromHeightGnls$chapmanReplaceRelHt = fit_gnls("Chapman-Richards replace RelHt GNLS", DBH ~ (a1 + a9 * relativeHeight)*(exp(b1*(TotalHt - 1.37)) - 1)^b2, other2016, start = otherDiameterFromHeight$chapmanReplaceRelHt$fit[[1]]$m$getPars(), weights = varPower(1.0, ~TotalHt | isPlantation), control = gnlsControl())
  ...
  otherDiameterFromHeightGnls$schnute = fit_gnls("Schnute inverse GNLS", DBH ~ -1/a1 * log(1 - (1 - exp(-a2))*(TotalHt^b1 - 1.37^b1)/(Ha^b1 - 1.37^b1)), other2016, start = otherDiameterFromHeight$schnute$fit[[1]]$m$getPars(), weights = varPower(1.0, ~TotalHt | isPlantation)) # step halving
...
if (otherOptions$fitDbhMixed)
{
  ...
  #otherDiameterFromHeightMixed$chapmanReplaceRelHt = fit_nlme("Chapman-Richards replace RelHt", DBH ~ (a1 + a1r + a9 * relativeHeight)*(exp(b1*(TotalHt - 1.37)) - 1)^b2, other2016, 
  #                                                            fixedFormula = a1 + a9 + b1 + b2 ~ 1, randomFormula = a1r ~ 1,
  #                                                            start = list(fixed = c(a1 = 10, a9 = 0, b1 = 0.3, b2 = 0.3)), control = nlmeControl(maxIter = 100, tolerance = 0.01, pnlsTol = 1, msTol = 0.001), significant = FALSE) # step halving, signularity in backsolve
  ...
  #otherDiameterFromHeightMixed$schnute = fit_nlme("Schnute inverse", DBH ~ -1/a1 * log(1 - (1 - exp(-a2))*(TotalHt^b1 - 1.37^b1)/((Ha + Har)^b1 - 1.37^b1)), other2016, 
