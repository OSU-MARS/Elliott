...
if (alruOptions$fitHeight)
{
  ...
  alruHeightFromDiameterNlrob$chapmanRichardsBalPhysio = fit_gsl_nls("Chapman-Richards BA+L physio", TotalHt ~ 1.37 + (a1 + a1p * isPlantation + a2 * basalAreaLarger + a5 * slope + a8 * topographicShelterIndex) * (1 - exp(b1*DBH))^b2, alru2016physio, start = list(a1 = 30, a1p = 3.6, a2 = 0.09, a5 = -0.28, a8 = 0.20, b1 = -0.049, b2 = 1.26)) # a4, a6, a7 not significant
  alruHeightFromDiameterNlrob$chapmanRichardsBalRelHt = fit_gsl_nls("Chapman-Richards BA+L RelHt", TotalHt ~ 1.37 + (a1 + a1p * isPlantation + (a2 + a2p * isPlantation) * basalAreaLarger + a3 * standBasalAreaPerHectare + (a9 + a9p * isPlantation) * relativeHeight) * (1 - exp(b1*DBH))^(b2 + b2p * isPlantation), alru2016, start = list(a1 = -3.1, a1p = 11, a2 = 0.01, a2p = 0.24, a3 = 0.07, a9 = 57, a9p = -28, b1 = -0.028, b2 = 0.16, b2p = 0.61)) # a2, a3, a3p, b2 not significant, often step factor with nlrob()
  ...
  alruHeightFromDiameterNlrob$sharmaZhangBal = fit_gsl_nls("Sharma-Zhang BA+L", TotalHt ~ 1.37 + (a1 + a2 * basalAreaLarger)*standBasalAreaPerHectare^(b1 + b1p * isPlantation) * (1 - exp(b2*tph^b3*DBH))^b4, alru2016, start = list(a1 = 19, a2 = 0.03, b1 = 0.05, b1p = 0.04, b2 = -0.08, b3 = -0.08, b4 = 1.18))
  ...
  if (alruOptions$fitPhysioGams)
  {
    ...
    save(file = "trees/height-diameter/data/ALRU2 TotalHt primary GAMs.Rdata", alruHeightFromDiameterGamBalPhysio, alruHeightFromDiameterGamPhysio)
    rm(alruHeightFromDiameterGamBalPhysio, alruHeightFromDiameterGamPhysio)
  } else {
...
if (alruOptions$fitHeightMixed)
{
  ...
  alruHeightFromDiameterMixed$gamm = fit_gam("REML GAM", TotalHt ~ s(DBH, bs = "ts", by = as.factor(isPlantation), k = 9, pc = alru2016gamConstraint), data = alru2016, mixed = TRUE)
  alruHeightFromDiameterMixed$gammBal = fit_gam("REML GAM BA+L", TotalHt ~ s(DBH, standBasalAreaPerHectare, basalAreaLarger, bs = "ts", by = as.factor(isPlantation), k = 18, pc = alru2016gamConstraint), data = alru2016, mixed = TRUE)
...
if (alruOptions$fitDbh)
{
  ...
  alruDiameterFromHeight$gamRelHt = fit_gam("REML GAM RelHt", DBH ~ s(TotalHt, relativeHeight, bs = "ts", by = as.factor(isPlantation), k = 12, pc = alru2016gamConstraint), data = alru2016, nthreads = 4)
  if (alruOptions$fitPhysioGams)
  {
    ...
    alruDiameterFromHeight$gamRelHtPhysio = fit_gam("REML GAM RelHt physio", DBH ~ s(TotalHt, elevation, slope, sin(3.14159/180 * aspect), relativeHeight, bs = "ts", by = as.factor(isPlantation), k = 57, pc = alru2016gamConstraint), data = alru2016physio, nthreads = 6)
  ...
  {
    alruDiameterFromHeight$gamAbatPhysioRelHt = fit_gam("REML GAM ABA+T RelHt physio", DBH ~ s(TotalHt, tallerApproxBasalArea, elevation, slope, sin(3.14159/180 * aspect), relativeHeight, bs = "ts", by = as.factor(isPlantation), k = 85, pc = alru2016gamConstraint), data = alru2016physio, nthreads = 6)
...
{
  alruDiameterFromHeightMixed = list(gamm = fit_gam("REML GAM", DBH ~ s(TotalHt, bs = "ts", by = as.factor(isPlantation), k = 8, pc = alru2016gamConstraint), data = alru2016, mixed = TRUE))
  alruDiameterFromHeightMixed$gammAbat = fit_gam("REML GAM ABA+T", DBH ~ s(TotalHt, tallerApproxBasalArea, standBasalAreaApprox, bs = "ts", by = as.factor(isPlantation), k = 14, pc = alru2016gamConstraint), data = alru2016, mixed = TRUE)
  alruDiameterFromHeightMixed$gammRelHt = fit_gam("REML GAM RelHt", DBH ~ s(TotalHt, relativeHeight, bs = "ts", by = as.factor(isPlantation), k = 12, pc = alru2016gamConstraint), data = alru2016, mixed = TRUE)
