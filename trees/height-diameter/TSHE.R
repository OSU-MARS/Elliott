...
tsheDiameterFromHeight = list(chapmanReplace = fit_nlrob("Chapman-Richards replace", DBH ~ a1*(exp(b1*(TotalHt - 1.37)) - 1)^b2, tshe2016, start = list(a1 = 32, b1 = 0.034, b2 = 0.73))) # no significant plantation effects
tsheDiameterFromHeight$chapmanReplaceAat = fit_nlrob("Chapman-Richards replace ABA+T", DBH ~ (a1 + a2 * tallerApproxBasalArea)*(exp(b1*(TotalHt - 1.37)) - 1)^b2, tshe2016, start = list(a1 = 35, a2 = -0.04, b1 = 0.034, b2 = 0.73))
tsheDiameterFromHeight$chapmanReplaceBal = fit_nlrob("Chapman-Richards replace BA+L", DBH ~ (a1 + a2 * basalAreaLarger) * (exp(b1*(TotalHt - 1.37)^b2) - 1), tshe2016, start = list(a1 = 35, a2 = -0.044, b1 = 0.033, b2 = 0.74), control = nls.control(maxiter = 250), significant = FALSE) # a1p, a2, a3, b1p not significant, a3 + b2p step size
tsheDiameterFromHeight$chapmanReplaceBalRelHt = fit_nlrob("Chapman-Richards replace BA+L RelHt", DBH ~ (a1 + a2 * basalAreaLarger + a9 * relativeHeight) * (exp(b1*(TotalHt - 1.37)^b2) - 1), tshe2016, start = list(a1 = 0.3, a2 = -0.003, a9 = -0.08, b1 = 2.6, b2 = 0.22), control = nls.control(maxiter = 250), significant = FALSE) # a1, a2, a3, a9 not significant
...
tsheDiameterFromHeight$chapmanRichards = fit_nlrob("Chapman-Richards inverse", DBH ~ (a1 + a1p * isPlantation)*log(1 - pmin((b1 + b1p * isPlantation)*(TotalHt - 1.37)^b2, 0.9999)), tshe2016, start = list(a1 = -80, a1p = 20, b1 = 0.035, b1p = 0.006, b2 = 0.77), control = nls.control(maxiter = 250))
tsheDiameterFromHeight$chapmanRichardsAat = fit_nlrob("Chapman-Richards inverse ABA+T", DBH ~ (a1 + a1p * isPlantation + a2 * tallerApproxBasalArea)*log(1 - pmin((b1 + b1p * isPlantation)*(TotalHt - 1.37)^b2, 0.9999)), tshe2016, start = list(a1 = -100, a1p = 30, a2 = 0.3, b1 = 0.03, b1p = 0.009, b2 = 0.75))
tsheDiameterFromHeight$chapmanRichardsPhysio = fit_nlrob("Chapman-Richards inverse physio", DBH ~ (a1 + a1p * isPlantation + a4 * elevation + a5 * sin(3.14159/180 * slope))*log(1 - pmin((b1 + b1p * isPlantation)*(TotalHt - 1.37)^b2, 0.9999)), tshe2016physio, start = list(a1 = -65, a1p = 23, a4 = -0.017, a5 = -15, b1 = 0.035, b1p = 0.008, b2 = 0.75)) # a6, a7, a8 not significant
tsheDiameterFromHeight$chapmanRichardsRelHt = fit_nlrob("Chapman-Richards inverse RelHt", DBH ~ (a1 + a9 * relativeHeight)*log(1 - pmin(b1*(TotalHt - 1.37)^b2, 0.9999)), tshe2016, start = list(a1 = -60, a9 = -14, b1 = 0.04, b2 = 0.72), significant = FALSE) # a1p, a9, b1p, b2p not significant, a1+a9-b1 not mutually significant
...
tsheDiameterFromHeight$michaelisMentenReplace = fit_nlrob("Michaelis-Menten replace", DBH ~ a1 * (TotalHt - 1.37)^b1 / (a2 - (TotalHt - 1.37)^b1), tshe2016, start = list(a1 = 153, a2 = 68, b1 = 0.83)) # a1p, a2p, b1p not significant
tsheDiameterFromHeight$naslund = fit_nlrob("Näslund inverse", DBH ~ (a1 + a1p * isPlantation) * sqrt(TotalHt - 1.37) / (1 + (a2 + a2p * isPlantation) * sqrt(TotalHt - 1.37)), tshe2016, start = list(a1 = 3.6, a1p = -0.47, a2 = -0.10, a2p = -0.013))
...
#tsheDiameterFromHeight$schnute = fit_gsl_nls("Schnute inverse", DBH ~ -1/a1 * log(1 - (1 - exp(-a2))*(TotalHt^b1 - 1.37^b1)/(Ha^b1 - 1.3^b1)), tshe2016, start = list(a1 = 0.00108, a2 = 0.058, b1 = 0.96, Ha = 32)) # converges from red alder values but fails to reconverge (singular gradient), NaN-inf or singular gradient with fit_nlrob()
...
tsheDiameterFromHeight$sibbesenReplace = fit_nlrob("Sibbesen replace", DBH ~ a1*(TotalHt - 1.37)^(b1*(TotalHt - 1.37)^b2), tshe2016, start = list(a1 = 2.32, b1 = 0.750, b2 = 0.057)) # no significant plantation effects
tsheDiameterFromHeight$sibbesenReplaceAat = fit_nlrob("Sibbesen replace ABA+T", DBH ~ (a1 + a2 * tallerApproxBasalArea)*(TotalHt - 1.37)^(b1*(TotalHt - 1.37)^b2), tshe2016, start = list(a1 = 2.33, a2 = -0.00001, b1 = 0.748, b2 = 0.0578)) # no significant plantation effects
tsheDiameterFromHeight$sibbesenReplacePhysio = fit_nlrob("Sibbesen replace physio", DBH ~ (a1 + a1p * isPlantation + a4 * elevation + a5 * sin(3.14159/180 * slope))*(TotalHt - 1.37)^(b1*(TotalHt - 1.37)^(b2 + b2p * isPlantation)), tshe2016physio, start = list(a1 = 2.115, a1p = -0.100, a4 = 0.00017, a5 = 0.486, b1 = 0.736, b2 = 0.0593, b2p = 0.0028)) # a6, a7, a8 not significant
tsheDiameterFromHeight$sibbesenReplaceRelHt = fit_nlrob("Sibbesen replace RelHt", DBH ~ (a1 + a9 * relativeHeight)*(TotalHt - 1.37)^(b1*(TotalHt - 1.37)^b2), tshe2016, start = list(a1 = 2.32, a9 = 0.084, b1 = 0.75, b2 = 0.056))
tsheDiameterFromHeight$weibull = fit_nlrob("Weibull inverse", DBH ~ (a1*log(1 - pmin(b1*(TotalHt - 1.37), 0.9999)))^b2, tshe2016, start = list(a1 = -225, b1 = 0.011, b2 = 0.82), control = nls.control(maxiter = 250)) # a1p, b1p, b2p not significant
...
if (htDiaOptions$includeInvestigatory)
{
  ...
  ggplot(tshe2016) +
    ...
    #geom_line(aes(x = predict(tsheDiameterFromHeight$chapmanReplaceAat), y = TotalHt, color = "Chapman-Richards replace ABA+T", group = isPlantation), alpha = 0.5) +
...
{
  save(file = "trees/height-diameter/data/TSHE models and stats.Rdata", tsheHeightFromDiameter, tsheDiameterFromHeight)
} else {
  save(file = "trees/height-diameter/data/TSHE stats.Rdata", tsheHeightFromDiameter, tsheDiameterFromHeight)
}
