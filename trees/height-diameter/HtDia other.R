...
otherHeightFromDiameterChapmanRichards = nls(TotalHt ~ 1.37 + (a1 + a1p * isPlantation) *(1 - exp(b1*DBH))^(b2 + b2p * isPlantation), other2016, start = list(a1 = 36.7, a1p = 5.57, b1 = -0.0171, b2 = 0.912, b2p = 0.0178), weights = pmin(DBH^-2, 1)) # b1p not significant
otherHeightFromDiameterChapmanRichardsBal = nls(TotalHt ~ 1.37 + (a1 + a2 * basalAreaLarger + a3 * standBasalAreaPerHectare) * (1 - exp(b1*DBH))^b2, other2016, start = list(a1 = 89.3, a2 = 1.166, a3 = -0.039, b1 = -0.00544, b2 = 0.873), weights = pmin(DBH^-2, 1), control = list(maxiter = 250))
otherHeightFromDiameterChapmanRichardsBalPhysio = nls(TotalHt ~ 1.37 + (a1 + a2 * basalAreaLarger + a3 * elevation + a4 * slope + a5 * sin(pi/180 * aspect) + a6 * cos(pi/180 * aspect)) * (1 - exp(b1*DBH))^b2, other2016, start = list(a1 = 89.3, a2 = 1.166, a3 = 0, a4 = 0, a5 = 0, a6 = 0, b1 = -0.00544, b2 = 0.873), weights = pmin(DBH^-2, 1)) # no significant physiographic effects
otherHeightFromDiameterChapmanRichardsBalRelHt = nls(TotalHt ~ 1.37 + (a1 + a1p * isPlantation + (a2 + a2p * isPlantation) * basalAreaLarger + (a3 + a3p * isPlantation) * standBasalAreaPerHectare + (a4 + a4p * isPlantation) * relativeHeight) * (1 - exp(b1*DBH))^(b2 + b2p * isPlantation), other2016, start = list(a1 = 8.7, a1p = 11.1, a2 = 0.18, a2p = 0.42, a3 = -0.0087, a3p = 0.070, a4 = 54.0, a4p = -28.4, b1 = -0.021, b2 = 0.65, b2p = 0.37), weights = pmin(DBH^-2, 1))
otherHeightFromDiameterChapmanRichardsPhysio = nls(TotalHt ~ 1.37 + (a1 + a1p * isPlantation + a2 * elevation + a3 * sin(pi/180 * slope) + a4 * cos(pi/180 * aspect) + a5 * sin(pi/180 * aspect) + a6 * topographicShelterIndex) * (1 - exp(b1*DBH))^(b2 + b2p * isPlantation), other2016, start = list(a1 = 68.5, a1p = -13.4, a2 = -0.0045, a3 = -8.09, a4 = 0.783, a5 = 0.766, a6 = 0.213, b1 = -0.022, b2 = 1.50, b2p = -0.31), weights = pmin(DBH^-2, 1)) # a4p not significant, a5p induces overfitting
otherHeightFromDiameterCurtis = nls(TotalHt ~ 1.37 + a1 * DBH / (1 + DBH)^b1, other2016, start = list(a1 = 1.6, b1 = 0.24), weights = pmin(DBH^-2, 1))
otherHeightFromDiameterHossfeld = nls(TotalHt ~ 1.37 + (a1 + a1p * isPlantation) / (1 + (b1 + b1p * isPlantation) *DBH^(b2 + b2p * isPlantation)), other2016, start = list(a1 = 75.4, a1p = -11.4, b1 = 462, b1p = -322, b2 = -1.54, b2p = 0.28), weights = pmin(DBH^-2, 1))
otherHeightFromDiameterKorf = nls(TotalHt ~ 1.37 + a1*exp(b1*DBH^b2), other2016, start = list(a1 = 704, b1 = -6.818, b2 = -0.175), weights = pmin(DBH^-2, 1), control = list(maxiter = 50)) # a1p, b2p not significant, singular gradient with b1p
otherHeightFromDiameterLinear = lm(TotalHt ~ 0 + DBH + I(isPlantation*DBH), other2016, offset = breastHeight, weights = pmin(DBH^-2, 1))
otherHeightFromDiameterMichaelisMenten = nls(TotalHt ~ 1.37 + a1*DBH^b1 / (a2 + DBH^b1), other2016, start = list(a1 = 53.4, a2 = 58.0, b1 = 0.964), weights = pmin(DBH^-2, 1)) # a1p, a2p, b1p not significant
...
otherHeightFromDiameterProdan = nls(TotalHt ~ 1.37 + DBH^2 / (a1*DBH^2 + (a2 + a2p * isPlantation)*DBH + a3 + a3p* isPlantation), other2016, start = list(a1 = 0.012, a2 = 0.41, a2p = 0.47, a3 = 17.9, a3p = -14.6), weights = pmin(DBH^-2, 1))
otherHeightFromDiameterPower = nls(TotalHt ~ 1.37 + (a1 + a1p * isPlantation)*DBH^(b1 + b1p * isPlantation), other2016, start = list(a1 = 2.9, a1p = -1.6, b1 = 0.63, b1p = 0.18), weights = pmin(DBH^-2, 1))
otherHeightFromDiameterRatkowsky = nls(TotalHt ~ 1.37 + (a1 + a1p * isPlantation)*exp((b1 + b1p * isPlantation)/(DBH + b2 + b2p * isPlantation)), other2016, start = list(a1 = 90.0, a1p = -25.8, b1 = -55.2, b1p = 14.5, b2 = 10.0, b2p = -1.52), weights = pmin(DBH^-2, 1))
otherHeightFromDiameterRichards = nls(TotalHt ~ 1.37 + Ha * (1 + ((1.37/Ha)^(1 - d) - 1) * exp((-(kU + kUp * isPlantation) * DBH)/d^(d/(1 - d))))^(1/(1 - d)), other2016, start = list(Ha = 17.0, d = 1.48, kU = 0.0407, kUp = 0.008), weights = pmin(DBH^-2, 1)) # Hap, dp not significant
...
otherHeightFromDiameterSharmaPartonBal = nls(TotalHt ~ 1.37 + (a1 + a1p * isPlantation)*topHeight^(a2 + a2p * isPlantation) * (1 - exp((b1 + b1p * isPlantation)*(tph/(standBasalAreaPerHectare + basalAreaLarger))^(b2 + b2p * isPlantation)*DBH))^(b3 + b3p * isPlantation), other2016, start = list(a1 = 18.5, a1p = 11.3, a2 = 0.30, a2p = -0.14, b1 = -0.019, b1p = -0.011, b2 = 0.089, b2p = -0.266, b3 = 1.49, b3p = -0.44), weights = pmin(DBH^-2, 1))
otherHeightFromDiameterSharmaPartonBalPhysio = nls(TotalHt ~ 1.37 + (a1 + a1p * isPlantation)*topHeight^(a2 + a2p * isPlantation) * (1 + a3 * elevation + a4 * sin(pi/180 * aspect) + a5 * cos(pi/180 * aspect) + a6 * topographicShelterIndex) * (1 - exp((b1 + b1p * isPlantation)*(tph/(standBasalAreaPerHectare + basalAreaLarger))^(b2 + b2p * isPlantation)*DBH))^(b3 + b3p * isPlantation), other2016, start = list(a1 = 12.5, a1p = 193, a2 = 0.45, a2p = -0.90, a3 = -0.00009, a4 = -0.016, a5 = 0.0038, a6 = 0.00256, b1 = -0.0052, b1p = -0.0036, b2 = 0.138, b2p = -0.60, b3 = 0.88, b3p = -0.073), weights = pmin(DBH^-2, 1))
otherHeightFromDiameterSharmaPartonPhysio = nls(TotalHt ~ 1.37 + (a1 + a1p * isPlantation)*topHeight^(a2 + a2p * isPlantation) * (1 + a3 * elevation + a4 * sin(pi/180 * aspect) + a5 * cos(pi/180 * aspect) + a6 * topographicShelterIndex) * (1 - exp((b1 + b1p * isPlantation)*(tph/(standBasalAreaPerHectare))^(b2 + b2p * isPlantation)*DBH))^(b3 + b3p * isPlantation), other2016, start = list(a1 = 14.0, a1p = 144, a2 = 0.44, a2p = -0.88, a3 = -0.00008, a4 = -0.018, a5 = 0.0027, a6 = -0.0037, b1 = -0.0041, b1p = -0.0067, b2 = 0.1568, b2p = -0.62, b3 = 0.88, b3p = -0.073), weights = pmin(DBH^-2, 1))
otherHeightFromDiameterSharmaZhang = nls(TotalHt ~ 1.37 + (a1 + a1p * isPlantation)*standBasalAreaPerHectare^(a2 + a2p * isPlantation)*(1 - exp((b1 + b1p * isPlantation)*tph^(b2 + b2p * isPlantation)*DBH))^(b3 + b3p * isPlantation), other2016, start = list(a1 = 56.1, a1p = -23.1, a2 = 0.042, a2p = 0.117, b1 = -0.0247, b1p = -0.0131, b2 = -0.0217, b2p = -0.112, b3 = 1.476, b3p = -0.456), weights = pmin(DBH^-2, 1))
otherHeightFromDiameterSharmaZhangBal = nls(TotalHt ~ 1.37 + (a1 + a1p * isPlantation)*standBasalAreaPerHectare^(a2 + a2p * isPlantation) * (1 + (a3 + a3p * isPlantation) * basalAreaLarger) * (1 - exp((b1 + b1p * isPlantation)*tph^(b2 + b2p * isPlantation)*DBH))^(b3 + b3p * isPlantation), other2016, start = list(a1 = 56.3, a1p = 14.7, a2 = 0.0412, a2p = -0.0535, a3 = 0.0146, a3p = 0.0146, b1 = -0.0249, b1p = -0.00024, b2 = -0.0240, b2p = -0.0969, b3 = 1.48, b3p = -0.370), weights = pmin(DBH^-2, 1))
otherHeightFromDiameterSibbesen = nls(TotalHt ~ 1.37 + a1*DBH^(b1*DBH^b2), other2016, start = list(a1 = 0.783, b1 = 1.163, b2 = -0.077), weights = pmin(DBH^-2, 1)) # a1p, b2p not significant
otherHeightFromDiameterWeibull = nls(TotalHt ~ 1.37 + (a1 + a1p * isPlantation)*(1 - exp((b1 + b1p * isPlantation)*DBH^(b2 + b2p * isPlantation))), other2016, start = list(a1 = 63.6, a1p = -12.7, b1 = -0.00516, b1p = -0.00652, b2 = 1.29, b2p = -0.16), weights = pmin(DBH^-2, 1))
otherHeightFromDiameterWeibullBal = nls(TotalHt ~ 1.37 + (a1 + a2 * basalAreaLarger + a3 * standBasalAreaPerHectare) * (1 - exp(b1*DBH^b2)), other2016, start = list(a1 = 94.2, a2 = 1.206, a3 = -0.761, b1 = -0.0100, b2 = 0.881), weights = pmin(DBH^-2, 1)) # a2p, a3p, b1p, b2p not significant
otherHeightFromDiameterWeibullBalRelHt = nls(TotalHt ~ 1.37 + (a1 + a2 * basalAreaLarger + a3 * standBasalAreaPerHectare + (a4 + a4p * isPlantation) * pmin(relativeHeight, 1.25)) * (1 - exp(b1*DBH^b2)), other2016, start = list(a1 = 0.296, a2 = 0.009, a3 = 0.081, a4 = 54.1, a4p = -24.2, b1 = -0.214, b2 = 0.650), weights = pmin(DBH^-2, 1), control = list(maxiter = 50))
...
otherHeightFromDiameterResults = tribble(~method, ~pae, ~paeNR, ~paePl, ~bias, ~biasNR, ~biasPl, ~mae, ~maeNR, ~maePl, ~rmse, ~rmseNR, ~rmsePl, ~nse, ~nseNR, ~nsePl, ~pearson, ~pearsonNR, ~pearsonPl, ~aic, ~bic, ~power,
                                        ...
                                        "Michaelis-Menten", !!!as_row(otherHeightFromDiameterMichaelisMenten),
...
#otherDiameterFromHeightChapmanRichardsPhysio = nls_multstart(DBH ~ (a1 + a1p * isPlantation + a2 * elevation + a3 * sin(pi/180 * slope) + a4 * cos(pi/180 * aspect) + a5 * sin(pi/180 * aspect) + a6 * topographicShelterIndex)*log(1 - pmin(b1*(TotalHt - 1.37)^b2, 0.999999)), other2016, iter = 100,
#                                                             start_lower = list(a1 = 1, a1p = -100, a2 = -0.1, a3 = -100, a4 = -1, a5 = -1, a6 = -1, b1 = -0.1, b2 = -2, pi = 3.1415926), 
#                                                             start_upper = list(a1 = 200, a1p = 100, a2 = 0.1, a3 = 100, a4 = 10, a5 = 1, a6 = 1, b1 = 0.1, b2 = 2, pi = 3.1415926), modelweights = pmin(TotalHt^-2, 0.5))
otherDiameterFromHeightChapmanRichards = nls(DBH ~ a1*log(1 - pmin(b1*(TotalHt - 1.37)^(b2 + b2p * isPlantation), 0.999999)), other2016, start = list(a1 = -7.8, b1 = 0.344, b2 = 0.344, b2p = -0.005), weights = pmin(TotalHt^-2, 0.5), control = list(maxiter = 500)) # a1p not significant
otherDiameterFromHeightChapmanRichardsAal = nls(DBH ~ (a1 + a2 * tallerQuasiBasalArea)*log(1 - pmin(b1*(TotalHt - 1.37)^(b2 + b2p * isPlantation), 0.999999)), other2016, start = list(a1 = -6.6, a2 = 0.0004, b1 = 0.398, b2 = 0.314, b2p = -0.023), weights = pmin(TotalHt^-2, 0.5)) # a1p, a2p not significant
otherDiameterFromHeightChapmanRichardsPhysio = nls(DBH ~ (a1 + a1p * isPlantation + a2 * elevation + a3 * sin(pi/180 * slope) + a4 * cos(pi/180 * aspect) + a5 * sin(pi/180 * aspect) + a6 * topographicShelterIndex)*log(1 - pmin(b1*(TotalHt - 1.37)^b2, 0.999999)), other2016, start = list(a1 = -200, a1p = 17.5, a2 = 0.046, a3 = 98, a4 = -1.1, a5 = -3.9, a6 = -0.35, b1 = 0.014, b2 = 0.96), weights = pmin(TotalHt^-2, 0.5)) # no physiographic effect significant, convergence fails with b1p
otherDiameterFromHeightChapmanRichardsRelHt = nls(DBH ~ (a1 + a2 * relativeHeight)*log(1 - pmin(b1*(TotalHt - 1.37)^(b2 + b2p * isPlantation), 0.999999)), other2016, start = list(a1 = -253, a2 = -196, b1 = 0.007, b2 = 0.94, b2p = -0.112), weights = pmin(TotalHt^-2, 0.5)) # a1p, a2p not significant
...
otherDiameterFromHeightPower = nls(DBH ~ (a1 + a1p * isPlantation)*(TotalHt - 1.37)^(b1 + b1p * isPlantation), other2016, start = list(a1 = 1.36, a1p = -0.48, b1 = 1.15, b1p = 0.11), weights = pmin(TotalHt^-2, 0.5))
otherDiameterFromHeightPowerAal = nls(DBH ~ (a1 + a1p * isPlantation + a2 * tallerQuasiBasalArea)*(TotalHt - 1.37)^b1, other2016, start = list(a1 = 1.41, a1p = -0.26, a2 = -0.00082, b1 = 1.16), weights = pmin(TotalHt^-2, 0.5)) # a2p, b2p not significant
otherDiameterFromHeightPowerPhysio = nls(DBH ~ (a1 + a2 * elevation + a3 * sin(pi/180 * slope) + a4 * cos(pi/180 * aspect) + a5 * sin(pi/180 * aspect) + a6 * topographicShelterIndex)*(TotalHt - 1.37)^b1, other2016, start = list(a1 = 1.81, a2 = -0.0007, a3 = -1.05, a4 = 0.034, a5 = -0.024, a6 = 0.00049, b1 = 1.21), weights = pmin(TotalHt^-2, 0.5)) # a1p, a4, a5, a6 not significant
otherDiameterFromHeightPowerRelHt = nls(DBH ~ (a1 + a2 * relativeHeight)*(TotalHt - 1.37)^(b1 + b1p * isPlantation), other2016, start = list(a1 = 1.32, a2 = 0.40, b1 = 1.11, b1p = -0.055), weights = pmin(TotalHt^-2, 0.5))
#otherDiameterFromHeightSharmaParton = nls(DBH ~ a1*(TotalHt - 1.37)^a2*(exp(b1*(tph/topHeight)^b2*(TotalHt - 1.37)) - 1)^b3, other2016, start = list(a1 = 0.1, a2 = 1.5, b1 = 0.1, b2 = 0.4, b3 = 0.1), weights = pmin(TotalHt^-2, 0.5), trace = TRUE) # step size, singular gradient
#otherDiameterFromHeightSharmaParton = nls(DBH ~ a1*(TotalHt - 1.37)^(a2 + a2p * isPlantation)*(exp(b1*(tph/topHeight)^(b2 + b2p * isPlantation)*(TotalHt - 1.37)) - 1)^(b3 + b3p * isPlantation), other2016, start = list(a1 = 1, a2 = 1, a2p = 0, b1 = 0.02, b2 = 0.26, b2p = 0, b3 = 0.9, b3p = 0), weights = pmin(TotalHt^-2, 0.5)) # signular gradient
otherDiameterFromHeightSibbesenForm = nls(DBH ~ (a1 + a1p * isPlantation)*(TotalHt - 1.37)^((b1 + b1p * isPlantation)*(TotalHt - 1.37)^(b2 + b2p * isPlantation)), other2016, start = list(a1 = 0.67, a1p = 1.84, b1 = 1.70, b1p = -1.26, b2 = -0.063, b2p = 0.30), weights = pmin(TotalHt^-2, 0.5))
otherDiameterFromHeightSibbesenFormAal = nls(DBH ~ (a1 + a1p * isPlantation + a2 * tallerQuasiBasalArea)*(TotalHt - 1.37)^((b1 + b1p * isPlantation)*(TotalHt - 1.37)^(b2 + b2p * isPlantation)), other2016, start = list(a1 = 0.58, a1p = 1.93, a2 = -0.0005, b1 = 1.93, b1p = -1.49, b2 = -0.085, b2p = 0.32), weights = pmin(TotalHt^-2, 0.5)) # a2 not significant
otherDiameterFromHeightSibbesenFormPhysio = nls(DBH ~ (a1 + a1p * isPlantation + a2 * elevation + a3 * sin(pi/180 * slope) + a4 * cos(pi/180 * aspect) + a5 * sin(pi/180 * aspect) + a6 * topographicShelterIndex)*(TotalHt - 1.37)^(b1*(TotalHt - 1.37)^(b2 + b2p * isPlantation)), other2016, start = list(a1 = 3.149, a1p = -0.509, a2 = -0.00090, a3 = -1.696, a4 = 0.035, a5 = -0.025, a6 = 0.0053, b1 = 0.865, b2 = 0.0515, b2p = 0.0248), weights = pmin(TotalHt^-2, 0.5)) # a4, a5, a6 not significant
otherDiameterFromHeightSibbesenFormRelHt = nls(DBH ~ (a1 + a1p * isPlantation + a2 * relativeHeight)*(TotalHt - 1.37)^(b1*(TotalHt - 1.37)^(b2 + b2p * isPlantation)), other2016, start = list(a1 = 1.94, a1p = -0.66, a2 = 0.20, b1 = 0.91, b2 = 0.034, b2p = 0.030), weights = pmin(TotalHt^-2, 0.5)) # a2p not significant
#otherDiameterFromHeightChapmanForm = nls(DBH ~ a1*(exp(b1*(TotalHt - 1.37)) - 1)^b2, other2016, start = list(a1 = 15, b1 = 0.12, b2 = 0.5), weights = pmin(TotalHt^-2, 0.5)) # NaN-inf, no convergence from nls_multstart()
#otherDiameterFromHeightChapmanFormAal = nls(DBH ~ (a1 + a2 * tallerQuasiBasalArea)*(exp(b1*(TotalHt - 1.37)) - 1)^b2, other2016, start = list(a1 = 15, a2 = -0, b1 = 0.01, b2 = 0.5), weights = pmin(TotalHt^-2, 0.5)) # NaN-inf
#otherDiameterFromHeightChapmanFormBal = nls(DBH ~ (a1 + a2 * basalAreaLarger) * (exp(b1*(TotalHt - 1.37)^b2) - 1), other2016, start = list(a1 = 15, a2 = -0, b1 = 0.01, b2 = 0.5), weights = pmin(TotalHt^-2, 0.5)) # NaN-inf
#otherDiameterFromHeightChapmanFormBalRelHt = nls(DBH ~ (a1 + a2 * basalAreaLarger + a3 * standBasalAreaPerHectare + a4 * relativeHeight) * (exp(b1*(TotalHt - 1.37)^b2) - 1), other2016, start = list(a1 = 13, a2 = -22.0, a3 = 10.4, a4 = -467, b1 = 0.0017, b2 = 0.971), weights = pmin(TotalHt^-2, 0.5)) # step factor
#otherDiameterFromHeightChapmanFormRelHt = nls(DBH ~ (a1 + a2 * relativeHeight)*(exp(b1*(TotalHt - 1.37)^b2) - 1), other2016, start = list(a1 = 500, a2 = -21, b1 = 0.004, b2 = 1.0), weights = pmin(TotalHt^-2, 0.5)) # step factor
...
otherParameters = bind_rows(bind_rows(bind_rows(c(method = "Chapman-Richards", otherHeightFromDiameterChapmanRichards$m$getPars())),
                                      ...
                                      bind_rows(c(method = "Michaelis-Menten", otherHeightFromDiameterMichaelisMenten$m$getPars())),
