...
otherHeightFromDiameterSharmaPartonBal = nlrob(TotalHt ~ 1.37 + a1*topHeight^a2 * (1 - exp(b1*(tph/(standBasalAreaPerHectare + basalAreaLarger))^b2*DBH))^b3, other2016, start = list(a1 = 90.7, a2 = -0.328, b1 = -0.027, b2 = -0.277, b3 = 0.834), maxit = 20, weights = pmin(DBH^-2, 1)) # a1p, a2p, b1p, b2p, b3p not significant
otherHeightFromDiameterSharmaPartonBalPhysio = nlrob(TotalHt ~ 1.37 + a1*topHeight^a2 * (1 + a3 * elevation + a4 * sin(3.14159/180 * aspect) + a5 * cos(3.14159/180 * aspect) + a6 * topographicShelterIndex) * (1 - exp(b1*(tph/(standBasalAreaPerHectare + basalAreaLarger))^b2*DBH))^b3, other2016physio, start = list(a1 = 40.5, a2 = -0.359, a3 = -0.0004, a4 = -0.015, a5 = -0.020, a6 = -0.0016, b1 = -0.017, b2 = -0.330, b3 = 0.819), weights = pmin(DBH^-2, 1), maxit = 30, control = nls.control(maxiter = 50)) # a2, a3, a4, a5, a6, b1p, b2p, b3p not significant, step factor with nlrob()
...
#otherDiameterFromHeightPowerAat = gsl_nls(DBH ~ (a1 + a1p * isPlantation + a2 * tallerQuasiBasalArea)*(TotalHt - 1.37)^b1, other2016, start = list(a1 = 2.0, a1p = -0.38, a2 = -0.013, b1 = 1.06), weights = pmin(TotalHt^-2, 0.5)) # a2p, b2p not significant, nlrob() failure to converge >500 steps
...
#otherDiameterFromHeightSharmaParton = nlrob(DBH ~ a1*(TotalHt - 1.37)^a2*(exp(b1*(tph/topHeight)^b2*(TotalHt - 1.37)) - 1)^b3, other2016, start = list(a1 = 5, a2 = 0.5, b1 = 0.01, b2 = 0.26, b3 = 0.5), weights = pmin(TotalHt^-2, 0.5), control = nls.control(maxiter = 50)) # step factor
...
otherDiameterFromHeightChapmanForm = nlrob(DBH ~ a1*(exp(b1*(TotalHt - 1.37)) - 1)^b2, other2016, start = list(a1 = 7.883, b1 = 0.118, b2 = 0.542), weights = pmin(TotalHt^-2, 0.5), maxit = 20, control = nls.control(maxiter = 50)) # a1p, b1p, b2p not significant, NaN-inf with nls(), no convergence from nls_multstart()
otherDiameterFromHeightChapmanFormAat = nlrob(DBH ~ (a1 + a2 * tallerQuasiBasalArea)*(exp(b1*(TotalHt - 1.37)) - 1)^b2, other2016, start = list(a1 = 6.85, a2 = 0.019, b1 = 0.13, b2 = 0.52), maxit = 20, weights = pmin(TotalHt^-2, 0.5)) # NaN-inf
otherDiameterFromHeightChapmanFormBal = nlrob(DBH ~ (a1 + a2 * basalAreaLarger) * (exp(b1*(TotalHt - 1.37)^b2) - 1), other2016, start = list(a1 = 1.03, a2 = -0.0025, b1 = 1.20, b2 = 0.36), maxit = 30, weights = pmin(TotalHt^-2, 0.5), control = nls.control(maxiter = 50)) # NaN-inf with nls()
otherDiameterFromHeightChapmanFormBalRelHt = nlrob(DBH ~ (a1 + a2 * basalAreaLarger + a3 * standBasalAreaPerHectare + a4 * relativeHeight) * (exp(b1*(TotalHt - 1.37)^b2) - 1), other2016, start = list(a1 = 0.51, a2 = -0.019, a3 = 0.016, a4 = -0.001, b1 = 1.73, b2 = 0.28), weights = pmin(TotalHt^-2, 0.5)) # step factor with nls()
otherDiameterFromHeightChapmanFormRelHt = nlrob(DBH ~ (a1 + a2 * relativeHeight)*(exp(b1*(TotalHt - 1.37)^b2) - 1), other2016, start = list(a1 = 1.09, a2 = -0.098, b1 = 1.14, b2 = 0.38), weights = pmin(TotalHt^-2, 0.5), maxit = 40, control = nls.control(maxiter = 50)) # step factor with nls()
otherDiameterFromHeightChapmanRichards = nlrob(DBH ~ a1*log(1 - pmin(b1*(TotalHt - 1.37)^(b2 + b2p * isPlantation), 0.9999)), other2016, start = list(a1 = -7.17, b1 = 0.319, b2 = 0.415, b2p = -0.0044), weights = pmin(TotalHt^-2, 0.5), maxit = 20, control = list(maxiter = 50)) # a1p not significant
otherDiameterFromHeightChapmanRichardsAat = nlrob(DBH ~ (a1 + a2 * tallerQuasiBasalArea)*log(1 - pmin(b1*(TotalHt - 1.37)^(b2 + b2p * isPlantation), 0.9999)), other2016, start = list(a1 = -8.91, a2 = -0.0010, b1 = 0.254, b2 = 0.466, b2p = -0.032), maxit = 50, weights = pmin(TotalHt^-2, 0.5)) # a1p, a2p not significant
otherDiameterFromHeightChapmanRichardsPhysio = nlrob(DBH ~ (a1 + a1p * isPlantation + a2 * elevation + a3 * sin(3.14159/180 * slope) + a4 * cos(3.14159/180 * aspect) + a5 * sin(3.14159/180 * aspect) + a6 * topographicShelterIndex)*log(1 - pmin(b1*(TotalHt - 1.37)^b2, 0.9999)), other2016physio, start = list(a1 = -6.4, a1p = 0.88, a2 = 0.001, a3 = 0.658, a4 = -0.216, a5 = -0.199, a6 = 0.012, b1 = 0.439, b2 = 0.300), weights = pmin(TotalHt^-2, 0.5), maxit = 500, control = list(maxiter = 500)) # no physiographic effect significant, convergence fails with b1p, unreliable nlrob() convergence
otherDiameterFromHeightChapmanRichards = nlrob(DBH ~ a1*log(1 - pmin(b1*(TotalHt - 1.37)^(b2 + b2p * isPlantation), 0.9999)), other2016, start = list(a1 = -7.17, b1 = 0.319, b2 = 0.415, b2p = -0.0044), weights = pmin(TotalHt^-2, 0.5), maxit = 20, control = list(maxiter = 50)) # a1p not significant
otherDiameterFromHeightChapmanRichardsRelHt = nlrob(DBH ~ (a1 + a2 * relativeHeight)*log(1 - pmin(b1*(TotalHt - 1.37)^(b2 + b2p * isPlantation), 0.9999)), other2016, start = list(a1 = -21.3, a2 = 1.5, b1 = 0.11, b2 = 0.60, b2p = 0.006), maxit = 20, weights = pmin(TotalHt^-2, 0.5)) # a1p, a2p not significant, nlrob() fails to converge from closer positions
otherDiameterFromHeightLinear = lm(DBH ~ 0 + I(TotalHt - 1.37) + I(isPlantation*(TotalHt - 1.37)), other2016, weights = TotalHt^-2)
otherDiameterFromHeightMichaelisMentenForm = nlrob(DBH ~ a1 * (TotalHt - 1.37)^b1 / (a2 - (TotalHt - 1.37)^b1), other2016, start = list(a1 = 28.7, a2 = 13.0, b1 = 0.60), weights = TotalHt^-2, maxit = 30, control = nls.control(maxiter = 50)) # a1p, a2p, b1p not significant
...
otherDiameterFromHeightPowerAat = nlrob(DBH ~ (a1 + a2 * tallerQuasiBasalArea)*(TotalHt - 1.37)^b1, other2016, start = list(a1 = 2.08, a2 = -0.0004, b1 = 0.81), weights = pmin(TotalHt^-2, 0.5), maxit = 500) # a2p, b2p not significant, nlrob() failure to converge >500 steps with a1p
...
otherDiameterFromHeightRuark = nlrob(DBH ~ a1*(TotalHt - 1.37)^(b1 + b1p * isPlantation) * exp((b2 + b2p * isPlantation) * (TotalHt - 1.37)), other2016, start = list(a1 = 2.54, b1 = 0.56, b1p = -0.28, b2 = 0.040, b2p = 0.043), weights = pmin(TotalHt^-2, 0.5)) # step factor with a1p
...
otherDiameterFromHeightSibbesenFormAat = nlrob(DBH ~ (a1 + a1p * isPlantation + a2 * tallerQuasiBasalArea)*(TotalHt - 1.37)^((b1 + b1p * isPlantation)*(TotalHt - 1.37)^(b2 + b2p * isPlantation)), other2016, start = list(a1 = 0.58, a1p = 1.93, a2 = -0.0005, b1 = 1.93, b1p = -1.49, b2 = -0.085, b2p = 0.32), weights = pmin(TotalHt^-2, 0.5)) # a2 not significant
...
otherDiameterFromHeightSibbesenFormRelHt = nlrob(DBH ~ (a1 + a2 * relativeHeight)*(TotalHt - 1.37)^((b1 + b1p * isPlantation)*(TotalHt - 1.37)^(b2 + b2p * isPlantation)), other2016, start = list(a1 = 2.84, a2 = -0.007, b1 = 0.407, b1p = -0.121, b2 = 0.260, b2p = 0.122), maxit = 30, weights = pmin(TotalHt^-2, 0.5)) # a1p, a2p not significant
