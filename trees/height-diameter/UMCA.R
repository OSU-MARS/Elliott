...
umcaHeightFromDiameter$sharmaPartonBal = fit_nlrob("Sharma-Parton BA+L", TotalHt ~ 1.37 + a1*topHeight^b1 * (1 - exp(b2*(tph/(standBasalAreaPerHectare + basalAreaLarger))^b3*DBH))^(b4 + b4p * isPlantation), umca2016, start = list(a1 = 14, b1 = 0.05, b2 = -0.06, b3 = -0.01, b4 = 1.2, b4p = -0.22)) # a1p, b1, b1p, b2p, b3p not significant
...
umcaHeightFromDiameter$sibbesen = fit_nlrob("Sibbesen", TotalHt ~ 1.37 + (a1 + a1p * isPlantation)*DBH^(b1*DBH^(b2 + b2p * isPlantation)), umca2016, start = list(a1 = 0.3, a1p = 0.1, b1 = 2.0, b2 = -0.16, b2p = -0.03)) # b1p not significant
umcaHeightFromDiameter$weibull = fit_nlrob("Weibull", TotalHt ~ 1.37 + a1*(1 - exp(b1*DBH^b2)), umca2016, start = list(a1 = 18, b1 = -0.04, b2 = 1.0)) # a1p, b1p, b2p not significant
umcaHeightFromDiameter$weibullBal = fit_nlrob("Weibull BA+L", TotalHt ~ 1.37 + (a1 + a2 * basalAreaLarger) * (1 - exp(b1*DBH^b2)), umca2016, start = list(a1 = 18, a2 = 0.02, b1 = -0.044, b2 = 1.1), significant = FALSE) # a1p, a2, a2p, a3, a3p, b1p, b2p not significant
umcaHeightFromDiameter$weibullBalRelHt = fit_nlrob("Weibull BA+L RelHt", TotalHt ~ 1.37 + (a1 + a2*basalAreaLarger + a9*pmin(relativeHeight, 1.25)) * (1 - exp((b1 + b1p * isPlantation) * DBH^b2)), umca2016, start = list(a1 = -2.5, a2 = 0.09, a9 = 55, b1 = -0.5, b1p = 0.3, b2 = 0.4), control = nls.control(maxiter = 500)) # a1p, a2p, a3, a3p, a4p, b2p not significant
...
umcaDiameterFromHeight = list(chapmanReplace = fit_gsl_nls("Chapman-Richards replace", DBH ~ a1*(exp(b1*(TotalHt - 1.37)) - 1)^b2, umca2016, start = list(a1 = 20, b1 = 0.1, b2 = 0.6))) # subject to a1-b1 evaporation, NaN-inf with nls() at multiple nls_multstart() points, NaN-inf with nlrob(), job NaN-inf with gsl_nls()
umcaDiameterFromHeight$chapmanReplaceAat = fit_gsl_nls("Chapman-Richards replace ABA+T", DBH ~ (a1 + a2 * tallerApproxBasalArea)*(exp(b1*(TotalHt - 1.37)) - 1)^b2, umca2016, start = list(a1 = 20, a2 = 0, b1 = 0.1, b2 = 0.6), significant = FALSE) # a2 not significant, a1-b1 parameter evaporation, NaN-inf with nls(), step factor with nlrob(), step factor with default gsl_nls tolerance
...
umcaDiameterFromHeight$chapmanRichards = fit_gsl_nls("Chapman-Richards inverse", DBH ~ a1*log(1 - pmin(b1*(TotalHt - 1.37)^(b2 + b2p * isPlantation), 0.9999)), umca2016, start = list(a1 = 34, b1 = -0.047, b2 = 1.43, b2p = -0.15), control = gsl_nls_control(maxiter = 250, xtol = 1E-5)) # a1-b1 parameter evaporation
umcaDiameterFromHeight$chapmanRichardsAat = fit_gsl_nls("Chapman-Richards inverse ABA+T", DBH ~ (a1 + a2 * tallerApproxBasalArea)*log(1 - pmin(b1*(TotalHt - 1.37)^(b2 + b2p * isPlantation), 0.9999)), umca2016, start = list(a1 = 35, a2 = -3, b1 = -0.01, b2 = 0.9, b2p = -0.2), control = gsl_nls_control(maxiter = 250, xtol = 1E-5), significant = FALSE) # a2 not significant, a1-b1 parameter evaporation
...
umcaDiameterFromHeight$naslund = fit_nlrob("Näslund inverse", DBH ~ (a1 + a1p * isPlantation) * sqrt(TotalHt - 1.37) / (1 + a2*sqrt(TotalHt - 1.37)), umca2016, start = list(a1 = 3.9, a1p = -1.0, a2 = -0.14)) # a2p not significant, a1p debatable
...
#umcaDiameterFromHeight$sharmaParton = fit_gsl_nls("modified Sharma-Parton", DBH ~ a1*(TotalHt - 1.37)^b1*(exp(b2*(tph/topHeight)^b3*(TotalHt - 1.37)) - 1)^b4, umca2016, start = list(a1 = 130, b1 = 0.52, b2 = 0.0003, b3 = -1.7, b4 = 0.27), control = gsl_nls_control(maxiter = 250, xtol = 1E-3)) # a2p, b2p, b3p not significant, NaN-inf with nls() at nls_multistart() point, NaN-inf or code error with nlrob(), NaN-inf with gls_nls()
umcaDiameterFromHeight$sibbesenReplace = fit_nlrob("Sibbesen replace", DBH ~ a1*(TotalHt - 1.37)^(b1*(TotalHt - 1.37)^b2), umca2016, start = list(a1 = 0.43, b1 = 2.45, b2 = -0.15)) # no significant plantation effects
umcaDiameterFromHeight$sibbesenReplaceAat = fit_nlrob("Sibbesen replace ABA+T", DBH ~ (a1 + a2 * tallerApproxBasalArea)*(TotalHt - 1.37)^(b1*(TotalHt - 1.37)^b2), umca2016, start = list(a1 = 0.36, a2 = 0.0002, b1 = 2.59, b2 = -0.156)) # a2 not significant
umcaDiameterFromHeight$sibbesenReplacePhysio = fit_nlrob("Sibbesen replace physio", DBH ~ (a1 + a1p * isPlantation + a4 * elevation + a5 * sin(3.14159/180 * slope) + a6 * cos(3.14159/180 * aspect) + a8 * topographicShelterIndex)*(TotalHt - 1.37)^(b1*(TotalHt - 1.37)^b2), umca2016physio, start = list(a1 = 1.53, a1p = -0.38, a4 = 0.001, a5 = -1.15, a6 = 0.14, a8 = 0.02, b1 = 1.95, b2 = -0.15)) # a7 not significant, step factor with b1p
umcaDiameterFromHeight$sibbesenReplaceRelHt = fit_nlrob("Sibbesen replace RelHt", DBH ~ (a1 + a9 * relativeHeight)*(TotalHt - 1.37)^(b1*(TotalHt - 1.37)^b2), umca2016, start = list(a1 = 0.496, a9 = -0.188, b1 = 2.31, b2 = -0.12)) # a4 not significant
umcaDiameterFromHeight$weibull = fit_gsl_nls("Weibull inverse", DBH ~ (a1*log(1 - pmin(b1*(TotalHt - 1.37), 0.9999)))^b2, umca2016, start = list(a1 = -3800, b1 = 0.0006, b2 = 1.03), control = gsl_nls_control(maxiter = 500)) # NaN-inf with nlrob()
...
if (htDiaOptions$includeInvestigatory)
{
  ...
  ggplot(umca2016) +
    ...
    #geom_line(aes(x = predict(umcaDiameterFromHeight$chapmanReplaceAat), y = TotalHt, color = "Chapman-Richards replace approximate BA+L", group = isPlantation), alpha = 0.5) +
...
{
  save(file = "trees/height-diameter/data/UMCA models and stats.Rdata", umcaHeightFromDiameter, umcaDiameterFromHeight)
} else {
  save(file = "trees/height-diameter/data/UMCA stats.Rdata", umcaHeightFromDiameter, umcaDiameterFromHeight)
}
