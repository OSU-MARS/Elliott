...
if (umcaOptions$fitHeight)
{
  ...
  umcaHeightFromDiameter$chapmanRichardsBalPhysio = fit_gsl_nls("Chapman-Richards BA+L physio", TotalHt ~ 1.37 + (a1 + a2 * basalAreaLarger + a4 * elevation) * (1 - exp(b1*DBH))^b2, umca2016, start = list(a1 = 20, a2 = 0.03, a4 = -0.012, b1 = -0.05, b2 = 1.06), significant = FALSE) # a1p, a2, a2p, a3, a4, a5, a6, a7, a8, b1p, b2p not significant
  umcaHeightFromDiameter$chapmanRichardsBalPhysioRelDbh = fit_gsl_nls("Chapman-Richards BA+L RelDbh physio", TotalHt ~ 1.37 + (a1 + a2 * basalAreaLarger + a4 * elevation + a10 * relativeDiameter) * (1 - exp(b1*DBH))^b2, umca2016, start = list(a1 = 20, a2 = 0, a4 = -0.01, a10 = 0, b1 = -0.05, b2 = 1.04), significant = FALSE) # a10, a10p not significant
  ...
  umcaHeightFromDiameter$chapmanRichardsPhysio = fit_gsl_nls("Chapman-Richards physio", TotalHt ~ 1.37 + (a1 + a4 * elevation) * (1 - exp(b1*DBH))^b2, umca2016, start = list(a1 = 20, a4 = -0.014, b1 = -0.05, b2 = 1.1), significant = FALSE) # a4, a5, a6, a7, a8, b1p, b2p not significant
  ...
  umcaHeightFromDiameter$chapmanRichardsRelDbhPhysio = fit_gsl_nls("Chapman-Richards RelDbh physio", TotalHt ~ 1.37 + (a1 + a4 * elevation + a10 * relativeDiameter) * (1 - exp(b1*DBH))^b2, umca2016, start = list(a1 = 22, a4 = -0.012, a10 = 0, b1 = -0.05, b2 = 1.1), significant = FALSE) # a10, a10p not significant
  ...
  umcaHeightFromDiameter$weibullBalRelHt = fit_gsl_nls("Weibull BA+L RelHt", TotalHt ~ 1.37 + (a1 + a2*basalAreaLarger + a9*pmin(relativeHeight, 1.5)) * (1 - exp((b1 + b1p * isPlantation) * DBH^b2)), umca2016, start = list(a1 = 0, a2 = 0.08, a9 = 33, b1 = -0.5, b1p = 0.3, b2 = 0.4), control = nls.control(maxiter = 500)) # a1, a1p, a2p, a3, a3p, a9p, b2p not significant
  ...
  if (umcaOptions$fitHeightNlrob)
  {
    ...
    umcaHeightFromDiameterNlrob$chapmanRichardsBal = fit_nlrob("Chapman-Richards BA+L", TotalHt ~ 1.37 + (a1 + a2 * basalAreaLarger) * (1 - exp(b1*DBH))^b2, umca2016, start = list(a1 = 18, a2 = 0.02, b1 = -0.04, b2 = 1.05), control = nls.control(maxiter = 250, tol = 0.001), significant = FALSE) # job step factor
    umcaHeightFromDiameterNlrob$chapmanRichardsBalPhysio = fit_nlrob("Chapman-Richards BA+L physio", TotalHt ~ 1.37 + (a1 + a2 * basalAreaLarger + a4 * elevation) * (1 - exp(b1*DBH))^b2, umca2016, start = list(a1 = 20, a2 = 0, a4 = -0.014, b1 = -0.05, b2 = 1.1), nls.control(tol = 0.001), significant = FALSE) # job step factor
    ...
    umcaHeightFromDiameterNlrob$chapmanRichardsBalPhysioRelDbh = fit_nlrob("Chapman-Richards BA+L RelDbh physio", TotalHt ~ 1.37 + (a1 + a2 * basalAreaLarger + a4 * elevation + a10 * relativeDiameter) * (1 - exp(b1*DBH))^b2, umca2016, start = list(a1 = 20, a2 = 0.03, a4 = -0.01, a10 = 0, b1 = -0.05, b2 = 1.06), control = nls.control(tol = 1E-4), significant = FALSE) # job step factor
    umcaHeightFromDiameterNlrob$chapmanRichardsBalRelDbh = fit_nlrob("Chapman-Richards BA+L RelDbh", TotalHt ~ 1.37 + (a1 + a2 * basalAreaLarger + a10 * relativeDiameter) * (1 - exp(b1*DBH))^b2, umca2016, start = list(a1 = 18, a2 = 0.015, a10 = 0, b1 = -0.05, b2 = 1.1), control = nls.control(tol = 0.01), significant = FALSE) # job step factor
    umcaHeightFromDiameterNlrob$chapmanRichardsPhysio = fit_nlrob("Chapman-Richards physio", TotalHt ~ 1.37 + (a1 + a4 * elevation) * (1 - exp(b1*DBH))^b2, umca2016, start = list(a1 = 20, a4 = -0.014, b1 = -0.05, b2 = 1.1), significant = FALSE)
    ...
    #umcaHeightFromDiameterNlrob$chapmanRichardsRelDbhPhysio = fit_nlrob("Chapman-Richards RelDbh physio", TotalHt ~ 1.37 + (a1 + a4 * elevation + a10 * relativeDiameter) * (1 - exp(b1*DBH))^b2, umca2016, start = list(a1 = 21, a4 = -0.012, a10 = 0, b1 = -0.05, b2 = 1.1), control = nls.control(maxiter = 100, tol = 1), significant = FALSE) # always job step factor
    ...
    umcaHeightFromDiameterNlrob$sharmaPartonPhysio = fit_nlrob("Sharma-Parton physio", TotalHt ~ 1.37 + (a1 + a4 * elevation)*topHeight^b1 * (1 - exp(b2*(tph/(standBasalAreaPerHectare))^b3*DBH))^b4, umca2016, start = list(a1 = 25, a4 = -0.025, b1 = -0.2, b2 = -0.04, b3 = 0, b4 = 1.1), control = nls.control(tol = 1E-4)) # b3 not significant, job step factor
    ...
    umcaHeightFromDiameterNlrob$sharmaZhangBal = fit_nlrob("Sharma-Zhang BA+L", TotalHt ~ 1.37 + (a1 + a2 * basalAreaLarger)*standBasalAreaPerHectare^b1 * (1 - exp(b2*tph^b3*DBH))^b4, umca2016, start = list(a1 = 18, a2 = 0.03, b1 = 0, b2 = -0.03, b3 = 0.10, b4 = 1.08), control = nls.control(tol = 0.001), significant = FALSE) # job step factor
    ...
    umcaHeightFromDiameterNlrob$weibullBal = fit_nlrob("Weibull BA+L", TotalHt ~ 1.37 + (a1 + a2 * basalAreaLarger) * (1 - exp(b1*DBH^b2)), umca2016, start = list(a1 = 17.5, a2 = 0.02, b1 = -0.044, b2 = 1.05), control = nls.control(tol = 1E-4), significant = FALSE) # job step halving
    umcaHeightFromDiameterNlrob$weibullBalRelHt = fit_nlrob("Weibull BA+L RelHt", TotalHt ~ 1.37 + (a1 + a2*basalAreaLarger + a9*pmin(relativeHeight, 1.5)) * (1 - exp((b1 + b1p * isPlantation) * DBH^b2)), umca2016, start = list(a1 = -1.2, a2 = 0.03, a9 = 35, b1 = -0.8, b1p = 0.5, b2 = 0.4), control = nls.control(maxiter = 500))
  ...
  umcaHeightFromDiameter$gamBalPhysio = fit_gam("REML GAM BA+L physio", TotalHt ~ s(DBH, basalAreaLarger, elevation, cos(3.14159/180 * aspect), bs = "ts", by = as.factor(isPlantation), k = 36, pc = gamConstraint), data = umca2016, constraint = umca2016gamConstraint)
  umcaHeightFromDiameter$gamBalPhysioRelDbh = fit_gam("REML GAM BA+L RelDbh physio", TotalHt ~ s(DBH, standBasalAreaPerHectare, basalAreaLarger, elevation, cos(3.14159/180 * aspect), relativeDiameter, bs = "ts", by = as.factor(isPlantation), k = 85, pc = gamConstraint), data = umca2016, constraint = umca2016gamConstraint)
  ...
  umcaHeightFromDiameter$gamPhysio = fit_gam("REML GAM physio", TotalHt ~ s(DBH, elevation, cos(3.14159/180 * aspect), topographicShelterIndex, bs = "ts", by = as.factor(isPlantation), k = 35, pc = gamConstraint), data = umca2016, constraint = umca2016gamConstraint)
  ...
  umcaHeightFromDiameter$gamRelDbhPhysio = fit_gam("REML GAM RelDbh physio", TotalHt ~ s(DBH, elevation, cos(3.14159/180 * aspect), topographicShelterIndex, relativeDiameter, bs = "ts", k = 57, by = as.factor(isPlantation), pc = gamConstraint), data = umca2016, constraint = umca2016gamConstraint)
...
if (umcaOptions$fitHeightMixed)
{
  ...
  umcaHeightFromDiameterMixed$chapmanRichardsBalPhysio = fit_nlme("Chapman-Richards BA+L physio", TotalHt ~ 1.37 + (a1 + a1r + a2 * basalAreaLarger + a4 * elevation) * (1 - exp(b1*DBH))^b2, umca2016,
  ...
  #umcaHeightFromDiameterMixed$chapmanRichardsPhysio = fit_nlme("Chapman-Richards physio", TotalHt ~ 1.37 + (a1 + a1r + a4 * elevation) * (1 - exp(b1*DBH))^b2, umca2016,
  ...
  #umcaHeightFromDiameterMixed$sharmaPartonBalPhysio = fit_nlme("Sharma-Parton BA+L physio", TotalHt ~ 1.37 + (a1 + a1r + a4 * elevation)*topHeight^b1 * (1 - exp(b2*(tph/(standBasalAreaPerHectare + basalAreaLarger))^b3*DBH))^b4, umca2016,
  ...
  #umcaHeightFromDiameterMixed$sharmaPartonPhysio = fit_nlme("Sharma-Parton physio", TotalHt ~ 1.37 + (a1 + a1r + a4 * elevation)*topHeight^b1 * (1 - exp(b2*(tph/(standBasalAreaPerHectare))^b3*DBH))^(b4 + b4p * isPlantation), umca2016, 
...
if (umcaOptions$fitDbh)
{
  ...
  #umcaDiameterFromHeight$powerPhysio = fit_gsl_nls("power physio", DBH ~ (a1 + a1p * isPlantation + a4 * elevation + a5 * sin(3.14159/180 * slope) + a6 * cos(3.14159/180 * aspect) + a8 * topographicShelterIndex)*(TotalHt - 1.37)^b1, umca2016, start = list(a1 = 4, a1p = -0.95, a4 = 0.002, a5 = -2.8, a6 = 0.4, a8 = 0.05, b1 = 0.94)) # a1p, a2, a7, b1p not significant, a4 debatable
  ...
  umcaDiameterFromHeight$ruarkAbatPhysio = fit_gsl_nls("Ruark ABA+T physio", DBH ~ (a1 + a2 * tallerApproxBasalArea + a5 * sin(3.14159/180 * slope))*(TotalHt - 1.37)^b1 * exp(b2 * (TotalHt - 1.37)), umca2016, start = list(a1 = 5.5, a2 = 0, a5 = -1, b1 = 0.5, b2 = 0.04), significant = FALSE) # a2, a3 not significant
  umcaDiameterFromHeight$ruarkAbatPhysioRelHt = fit_gsl_nls("Ruark ABA+T RelHt physio", DBH ~ (a1 + a2 * tallerApproxBasalArea + a5 * sin(3.14159/180 * slope) + a9 * relativeHeight)*(TotalHt - 1.37)^b1 * exp(b2 * (TotalHt - 1.37)), umca2016, start = list(a1 = 5.5, a2 = -0.02, a5 = 0, a9 = -4, b1 = 0.6, b2 = 0.07), significant = FALSE) # a2, a3, a5 not significant
  ...
  umcaDiameterFromHeight$ruarkPhysio = fit_gsl_nls("Ruark physio", DBH ~ (a1 + a5 * sin(3.14159/180 * slope))*(TotalHt - 1.37)^b1 * exp(b2 * (TotalHt - 1.37)), umca2016, start = list(a1 = 6, a5 = 0, b1 = 0.6, b2 = 0.04), significant = FALSE) # a1p, a4, a5, a6, a7, a8, b1p, b2p not significant
  ...
  umcaDiameterFromHeight$ruarkRelHtPhysio = fit_gsl_nls("Ruark RelHt physio", DBH ~ (a1 + a5 * sin(3.14159/180 * slope) + a9 * relativeHeight)*(TotalHt - 1.37)^b1 * exp(b2 * (TotalHt - 1.37)), umca2016, start = list(a1 = 6, a5 = -2, a9 = -2, b1 = 0.6, b2 = 0.05)) # a5 significant
  umcaDiameterFromHeight$schnute = fit_gsl_nls("Schnute inverse", DBH ~ -1/a1 * log(1 - (1 - exp(-a2))*(TotalHt^b1 - 1.37^b1)/(Ha^b1 - 1.37^b1)), umca2016, start = list(a1 = 0.000003, a2 = 0.002, b1 = 1.13, Ha = 177)) # NaN-inf with nlrob()
  ...
  umcaDiameterFromHeight$sibbesenReplaceAbatPhysio = fit_gsl_nls("Sibbesen replace ABA+T physio", DBH ~ (a1 + a2 * tallerApproxBasalArea + a4 * elevation)*(TotalHt - 1.37)^(b1*(TotalHt - 1.37)^b2), umca2016, start = list(a1 = 4.1, a2 = 0, a4 = 0, b1 = 0.49, b2 = 0.19), significant = FALSE) # a2, a3, a4 not significant
  umcaDiameterFromHeight$sibbesenReplaceAbatPhysioRelHt = fit_gsl_nls("Sibbesen replace ABA+T RelHt physio", DBH ~ (a1 + a3 * standBasalAreaApprox + a4 * elevation + a9 * relativeHeight)*(TotalHt - 1.37)^(b1*(TotalHt - 1.37)^b2), umca2016, start = list(a1 = 5.0, a3 = -0.014, a4 = -0.001, a9 = -5.3, b1 = 0.53, b2 = 0.22)) # a2, a3, a4 not significant
  ...
  umcaDiameterFromHeight$sibbesenReplacePhysio = fit_gsl_nls("Sibbesen replace physio", DBH ~ (a1 + a4 * elevation)*(TotalHt - 1.37)^(b1*(TotalHt - 1.37)^b2), umca2016, start = list(a1 = 3.3, a4 = 0.002, b1 = 0.48, b2 = 0.21), significant = FALSE) # a1p, a4, a5, a6, a7, a8 not significant, step factor with b1p
  ...
  if (umcaOptions$fitDbhNlrob)
  {
    ...
    #umcaDiameterFromHeightNlrob$powerPhysio = fit_nlrob("power physio", DBH ~ (a1 + a1p * isPlantation + a4 * elevation + a5 * sin(3.14159/180 * slope) + a6 * cos(3.14159/180 * aspect) + a8 * topographicShelterIndex)*(TotalHt - 1.37)^b1, umca2016, start = list(a1 = 4, a1p = -0.95, a4 = 0.002, a5 = -2.8, a6 = 0.4, a8 = 0.05, b1 = 0.94))
    ...
    umcaDiameterFromHeightNlrob$ruarkAbatPhysio = fit_nlrob("Ruark ABA+T physio", DBH ~ (a1 + a2 * tallerApproxBasalArea + a5 * sin(3.14159/180 * slope))*(TotalHt - 1.37)^b1 * exp(b2 * (TotalHt - 1.37)), umca2016, start = list(a1 = 5.5, a2 = -0.001, a5 = -2, b1 = 0.6, b2 = 0.04), control = nls.control(maxiter = 100, tol = 1E-4), significant = FALSE) # job step factor
    umcaDiameterFromHeightNlrob$ruarkAbatPhysioRelHt = fit_nlrob("Ruark ABA+T RelHt physio", DBH ~ (a1 + a2 * tallerApproxBasalArea + a5 * sin(3.14159/180 * slope) + a9 * relativeHeight)*(TotalHt - 1.37)^b1 * exp(b2 * (TotalHt - 1.37)), umca2016, start = list(a1 = 5.5, a2 = -0.03, a5 = -2, a9 = -5.5, b1 = 0.6, b2 = 0.07), control = nls.control(maxiter = 100, tol = 0.001)) # job step factor
    ...
    umcaDiameterFromHeightNlrob$ruarkPhysio = fit_nlrob("Ruark physio", DBH ~ (a1 + a5 * sin(3.14159/180 * slope))*(TotalHt - 1.37)^b1 * exp(b2 * (TotalHt - 1.37)), umca2016, start = list(a1 = 4.3, a5 = -0.6, b1 = 0.44, b2 = 0.060), control = nls.control(maxiter = 100, tol = 1E-4), significant = FALSE) # job step factor
    ...
    umcaDiameterFromHeightNlrob$ruarkRelHtPhysio = fit_nlrob("Ruark RelHt physio", DBH ~ (a1 + a5 * sin(3.14159/180 * slope) + a9 * relativeHeight)*(TotalHt - 1.37)^b1 * exp(b2 * (TotalHt - 1.37)), umca2016, start = list(a1 = 4.5, a5 = 0, a9 = -5, b1 = 0.45, b2 = 0.09), control = nls.control(maxiter = 100, tol = 1E-4), significant = FALSE) # job step factor
    #umcaDiameterFromHeightNlrob$schnute = fit_nlrob("Schnute inverse", DBH ~ -1/a1 * log(1 - (1 - exp(-a2))*(TotalHt^b1 - 1.37^b1)/(Ha^b1 - 1.37^b1)), umca2016, start = list(a1 = 0.000003, a2 = 0.002, b1 = 1.13, Ha = 177)) # NaN-inf
    ...
    umcaDiameterFromHeightNlrob$sibbesenReplaceAbatPhysio = fit_nlrob("Sibbesen replace ABA+T physio", DBH ~ (a1 + a2 * tallerApproxBasalArea + a4 * elevation)*(TotalHt - 1.37)^(b1*(TotalHt - 1.37)^b2), umca2016, start = list(a1 = 4.1, a2 = 0, a4 = 0, b1 = 0.49, b2 = 0.18), significant = FALSE)
    umcaDiameterFromHeightNlrob$sibbesenReplaceAbatPhysioRelHt = fit_nlrob("Sibbesen replace ABA+T RelHt physio", DBH ~ (a1 + a3 * standBasalAreaApprox + a4 * elevation + a9 * relativeHeight)*(TotalHt - 1.37)^(b1*(TotalHt - 1.37)^b2), umca2016, start = list(a1 = 5.1, a3 = -0.015, a4 = -0.001, a9 = -5.3, b1 = 0.53, b2 = 0.22), control = nls.control(maxiter = 100, tol = 1E-4)) # job step factor
    ...
    umcaDiameterFromHeightNlrob$sibbesenReplacePhysio = fit_nlrob("Sibbesen replace physio", DBH ~ (a1 + a4 * elevation)*(TotalHt - 1.37)^(b1*(TotalHt - 1.37)^b2), umca2016, start = list(a1 = 3.3, a4 = 0.002, b1 = 0.48, b2 = 0.21), significant = FALSE)
    ...
    #umcaDiameterFromHeightNlrob$sibbesenReplaceRelHtPhysio = fit_nlrob("Sibbesen replace RelHt physio", DBH ~ (a1 + a4 * elevation + a9 * relativeHeight)*(TotalHt - 1.37)^(b1*(TotalHt - 1.37)^b2), umca2016, maxit = 500, start = list(a1 = 4.2, a4 = 0, a9 = -2.8, b1 = 0.53, b2 = 0.18)) # >500 steps to converge
  ...
  umcaDiameterFromHeightGslNlsDefault$schnute = fit_gsl_nls("Schnute inverse", DBH ~ -1/a1 * log(1 - (1 - exp(-a2))*(TotalHt^b1 - 1.37^b1)/(Ha^b1 - 1.37^b1)), umca2016defaultWeight, start = list(a1 = 0.000003, a2 = 0.002, b1 = 1.13, Ha = 177))
  ...
  # individual term selection: TotalHt by = isPlantation, ABA + cos(aspect) retained by AIC but not significant (p > 0.06)
  ...
  umcaDiameterFromHeight$gamAbatPhysio = fit_gam("REML GAM ABA+T physio", DBH ~ s(TotalHt, standBasalAreaApprox, tallerApproxBasalArea, slope, bs = "ts", by = as.factor(isPlantation), k = 28, pc = gamConstraint), data = umca2016, constraint = umca2016gamConstraint) # >5 minute Zen 3 3.4 GHz fit time with ABA+T, elevation, slope, sin(aspect), and TSI: elevation dropped based on physio AICs, then TSI+aspect
  umcaDiameterFromHeight$gamAbatPhysioRelHt = fit_gam("REML GAM ABA+T RelHt physio", DBH ~ s(TotalHt, tallerApproxBasalArea, slope, sin(3.14159/180 * aspect), relativeHeight, bs = "ts", by = as.factor(isPlantation), k = 57, pc = gamConstraint), data = umca2016, constraint = umca2016gamConstraint) # insufficient data -> drop elevation and ABA
  umcaDiameterFromHeight$gamPhysio = fit_gam("REML GAM physio", DBH ~ s(TotalHt, elevation, slope, sin(3.14159/180 * aspect), topographicShelterIndex, bs = "ts", by = as.factor(isPlantation), k = 60, pc = gamConstraint), data = umca2016, constraint = umca2016gamConstraint) # 72.5% deviance explained (AIC 6089): 75.7% (5995) without elevation, 71.3% (6111) without slope, 72.4% (6043) without aspect, 72.7% (6034) without topographic shelter
  ...
  umcaDiameterFromHeight$gamRelHtPhysio = fit_gam("REML GAM RelHt physio", DBH ~ s(TotalHt, slope, sin(3.14159/180 * aspect), relativeHeight, bs = "ts", by = as.factor(isPlantation), k = 57, pc = gamConstraint), data = umca2016, constraint = umca2016gamConstraint) # drop elevation on AIC
...
if (umcaOptions$fitDbhMixed)
{
  ...
  #umcaDiameterFromHeightMixed$chapmanRichardsPhysio = fit_nlme("Chapman-Richards inverse physio", DBH ~ (a1 + a1r + a5 * sin(3.14159/180 * slope))*log(1 - pmin(b1*(TotalHt - 1.37)^b2, 0.9999)), umca2016, 
  ...
  #umcaDiameterFromHeightMixed$powerPhysio = fit_nlme("power physio", DBH ~ (a1 + a1r + a1p * isPlantation + a4 * elevation + a5 * sin(3.14159/180 * slope) + a6 * cos(3.14159/180 * aspect) + a8 * topographicShelterIndex)*(TotalHt - 1.37)^b1, umca2016, 
  ...
  umcaDiameterFromHeightMixed$ruarkAbatPhysio = fit_nlme("Ruark ABA+T physio", DBH ~ (a1 + a1r + a2 * tallerApproxBasalArea + a5 * sin(3.14159/180 * slope))*(TotalHt - 1.37)^b1 * exp(b2 * (TotalHt - 1.37)), umca2016, 
  ...
  umcaDiameterFromHeightMixed$ruarkAbatPhysioRelHt = fit_nlme("Ruark ABA+T RelHt physio", DBH ~ (a1 + a1r + a2 * tallerApproxBasalArea + a5 * sin(3.14159/180 * slope) + a9 * relativeHeight)*(TotalHt - 1.37)^b1 * exp(b2 * (TotalHt - 1.37)), umca2016, 
  ...
  umcaDiameterFromHeightMixed$ruarkPhysio = fit_nlme("Ruark physio", DBH ~ (a1 + a1r + a5 * sin(3.14159/180 * slope))*(TotalHt - 1.37)^b1 * exp(b2 * (TotalHt - 1.37)), umca2016, 
                                                     fixedFormula = a1 + a5 + b1 + b2 ~ 1, randomFormula = a1r ~ 1, 
  ...
  umcaDiameterFromHeightMixed$ruarkRelHtPhysio = fit_nlme("Ruark RelHt physio", DBH ~ (a1 + a1r + a5 * sin(3.14159/180 * slope) + a9 * relativeHeight)*(TotalHt - 1.37)^b1 * exp(b2 * (TotalHt - 1.37)), umca2016, 
                                                          fixedFormula = a1 + a5 + a9 + b1 + b2 ~ 1, randomFormula = a1r ~ 1, 
                                                          start = list(fixed = c(a1 = 6, a5 = -2, a9 = -2, b1 = 0.6, b2 = 0.05)))
  #umcaDiameterFromHeightMixed$schnute = fit_nlme("Schnute inverse", DBH ~ -1/a1 * log(1 - (1 - exp(-a2))*(TotalHt^b1 - 1.37^b1)/((Ha + Har)^b1 - 1.37^b1)), umca2016, 
  ...
  umcaDiameterFromHeightMixed$sibbesenReplaceAbatPhysio = fit_nlme("Sibbesen replace ABA+T physio", DBH ~ (a1 + a1r + a2 * tallerApproxBasalArea + a4 * elevation)*(TotalHt - 1.37)^(b1*(TotalHt - 1.37)^b2), umca2016, 
                                                                   fixedFormula = a1 + a2 + a4 + b1 + b2 ~ 1, randomFormula = a1r ~ 1, 
  ...
  umcaDiameterFromHeightMixed$sibbesenReplaceAbatPhysioRelHt = fit_nlme("Sibbesen replace ABA+T RelHt physio", DBH ~ (a1 + a1r + a3 * standBasalAreaApprox + a4 * elevation + a9 * relativeHeight)*(TotalHt - 1.37)^(b1*(TotalHt - 1.37)^b2), umca2016, 
  ...
  umcaDiameterFromHeightMixed$sibbesenReplacePhysio = fit_nlme("Sibbesen replace physio", DBH ~ (a1 + a1r + a4 * elevation)*(TotalHt - 1.37)^(b1*(TotalHt - 1.37)^b2), umca2016, 
                                                               fixedFormula = a1 + a4 + b1 + b2 ~ 1, randomFormula = a1r ~ 1, 
  ...
  umcaDiameterFromHeightMixed$sibbesenReplaceRelHtPhysio = fit_nlme("Sibbesen replace RelHt physio", DBH ~ (a1 + a1r + a4 * elevation + a9 * relativeHeight)*(TotalHt - 1.37)^(b1*(TotalHt - 1.37)^b2), umca2016, 
...
if (umcaOptions$fitHeight & umcaOptions$fitHeightMixed & umcaOptions$fitDbh & umcaOptions$fitDbhMixed)
{
  ...
  umcaResults = bind_rows(bind_rows(bind_rows(lapply(umcaHeightFromDiameter, get_list_stats)),
                                    ...
                                    #bind_rows(lapply(umcaHeightFromDiameterGnls, get_stats))) %>%
                            mutate(responseVariable = "height"),
...
{
  umcaHeightFromDiameterPreferred = list(linear = fit_lm("linear", TotalHt ~ 0 + DBH + I(isPlantation*DBH), umca2016, folds = 1, repetitions = 1))
  umcaHeightFromDiameterPreferred$hossfeld = fit_gsl_nls("Hossfeld IV", TotalHt ~ 1.37 + (a1 + a1p * isPlantation) / (1 + (b1 + b1p * isPlantation) *DBH^b2), umca2016, start = list(a1 = 21.4, a1p = -4.37, b1 = 43.8, b1p = -19.9, b2 = -1.27), folds = 1, repetitions = 1)
  ...
  umcaHeightFromDiameterPreferred$sharmaPartonPhysio = fit_gsl_nls("Sharma-Parton physio", TotalHt ~ 1.37 + (a1 + a4 * elevation)*topHeight^b1 * (1 - exp(b2*(tph/(standBasalAreaPerHectare))^b3*DBH))^(b4 + b4p * isPlantation), umca2016, start = list(a1 = 13, a4 = -0.01, b1 = 0.08, b2 = -0.05, b3 = 0.0, b4 = 1.2, b4p = -0.23), folds = 1, repetitions = 1)
  ...
  umcaDiameterFromHeightPreferred$gamRelHtPhysio = fit_gam("REML GAM RelHt physio", DBH ~ s(TotalHt, slope, sin(3.14159/180 * aspect), relativeHeight, bs = "ts", by = as.factor(isPlantation), k = 57, pc = gamConstraint), data = umca2016, constraint = umca2016gamConstraint, folds = 1, repetitions = 1)
...
if (htDiaOptions$includeInvestigatory)
{
  umcaHeightGam = fit_gam("REML GAM", TotalHt ~ s(DBH, bs = "ts", by = as.factor(isPlantation), k = 8, pc = gamConstraint) + 
                            s(standBasalAreaPerHectare, bs = "ts", by = as.factor(isPlantation), k = 4, pc = gamConstraint) + 
                            #s(basalAreaLarger, bs = "ts", by = as.factor(isPlantation), k = 3, pc = gamConstraint) + # BAL not significant
                            s(elevation, bs = "ts", k = 5, pc = gamConstraint) + 
                            s(slope, bs = "ts", k = 4, pc = gamConstraint) + 
                            #s(aspect, bs = "ts", k = 3, pc = gamConstraint) + # aspect not significant
                            #s(topographicShelterIndex, bs = "ts", k = 3, pc = gamConstraint) + # topographic shelter not significant
                            s(relativeDiameter, bs = "ts", by = as.factor(isPlantation), k = 4, pc = gamConstraint), 
                          data = umca2016, constraint = umca2016gamConstraint, folds = 1, repetitions = 1)
  ...
  umcaDbhGam = fit_gam("REML GAM", DBH ~ s(TotalHt, bs = "ts", by = as.factor(isPlantation), k = 8, pc = gamConstraint) +
                         s(standBasalAreaApprox, bs = "ts", by = as.factor(isPlantation), k = 4, pc = gamConstraint) +
                         s(tallerApproxBasalArea, bs = "ts", by = as.factor(isPlantation), k = 3, pc = gamConstraint) + # significant but negligible
                         s(elevation, bs = "ts", k = 3, pc = gamConstraint) + # significant but negligible
                         s(slope, bs = "ts", k = 4, pc = gamConstraint), # significant but negligible
                         #s(aspect, bs = "ts", k = 3, pc = gamConstraint) + # significant but zero
                         #s(topographicShelterIndex, bs = "ts", k = 3, pc = gamConstraint) + # significant but zero
                         #s(relativeHeight, bs = "ts", by = as.factor(isPlantation), k = 3, pc = gamConstraint), # significant but zero
                       data = umca2016, constraint = umca2016gamConstraint, folds = 1, repetitions = 1)
  k.check(umcaDbhGam)
  summary(umcaHeightGam)
  par(mfrow = c(2, 4), mar = c(2.2, 2.2, 0.5, 0) + 0.1, mgp = c(1.5, 0.4, 0))
...
if (htDiaOptions$includeInvestigatory)
{
  umcaHeightForest = train(TotalHt ~ DBH + standBasalAreaPerHectare + basalAreaLarger + elevation + slope + aspect + topographicShelterIndex + relativeDiameter, data = umca2016, method = "ranger", trControl = repeatedCrossValidation, 
                            importance = "impurity_corrected",
                            tuneGrid = expand.grid(mtry = c(4, 5, 6),
                                                   splitrule = "variance",
                                                   min.node.size = c(1, 2, 3)))
  umcaHeightForest
  varImp(umcaHeightForest)
  
  umcaDbhForest = train(DBH ~ TotalHt + standBasalAreaApprox + tallerApproxBasalArea + elevation + slope + aspect + topographicShelterIndex + relativeHeight, data = umca2016, method = "ranger", trControl = repeatedCrossValidation, 
                         importance = "impurity_corrected",
                         tuneGrid = expand.grid(mtry = c(6, 7, 8),
                                                splitrule = "variance",
                                                min.node.size = c(1, 2, 3)))
  umcaDbhForest
  varImp(umcaDbhForest)
}
