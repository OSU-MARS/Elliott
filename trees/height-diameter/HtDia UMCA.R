...
umcaHeightFromDiameterChapmanRichardsBalPhysio = nlrob(TotalHt ~ 1.37 + (a1 + a2 * basalAreaLarger + a4 * elevation + a5 * slope + a6 * sin(3.14159/180 * aspect) + a7 * cos(3.14159/180 * aspect) + a8 * topographicShelterIndex) * (1 - exp(b1*DBH))^(b2 + b2p * isPlantation), umca2016physio, start = list(a1 = 8.62, a2 = 0.050, a3 = 0, a4 = -0.014, a5 = 0.230, a6 = 0.413, a7 = -1.113, a8 = 0, b1 = -0.062, b2 = 1.265, b2p = -0.219), weights = pmin(DBH^-0.8, 1)) # a1p, a2, a2p, a3, a5, a6, a8, b1p not significant
...
umcaHeightFromDiameterSharmaParton = nlrob(TotalHt ~ 1.37 + a1*topHeight^b1 * (1 - exp((b2 + b2p * isPlantation)*(tph/standBasalAreaPerHectare)^b3*DBH))^b4, umca2016, start = list(a1 = 5.76, b1 = 0.261, b2 = -0.049, b2p = -0.024, b3 = 0.085, b4 = 1.234), weights = pmin(DBH^-0.8, 1)) # a1p, b1p, b3p, b4p not significant
umcaHeightFromDiameterSharmaPartonBal = nlrob(TotalHt ~ 1.37 + a1*topHeight^b1 * (1 - exp(b2*(tph/(standBasalAreaPerHectare + basalAreaLarger))^b3*DBH))^(b4 + b4p * isPlantation), umca2016, start = list(a1 = 9.074, b1 = 0.149, b2 = -0.065, b3 = 0.013, b4 = 1.299, b4p = -0.254), weights = pmin(DBH^-0.8, 1)) # a1p, b1, b1p, b2p, b3p not significant
umcaHeightFromDiameterSharmaPartonBalPhysio = nlrob(TotalHt ~ 1.37 + (a1 + a4 * elevation + a5 * slope + a6 * sin(3.14159/180 * aspect) + a7 * cos(3.14159/180 * aspect) + a8 * topographicShelterIndex)*topHeight^b1 * (1 - exp(b2*(tph/(standBasalAreaPerHectare + basalAreaLarger))^b3*DBH))^(b4 + b4p * isPlantation), umca2016physio, start = list(a1 = 9.30, a4 = -0.0007, a5 = 0, a6 = 0.0161, a7 = -0.040, a8 = 0.005, b1 = 0.151, b2 = -0.061, b3 = 0.061, b4 = 1.313, b4p = -0.253), weights = pmin(DBH^-0.8, 1)) # a4, a5, a6, a8, b1p, b2p, b3p not significant
umcaHeightFromDiameterSharmaPartonPhysio = nlrob(TotalHt ~ 1.37 + (a1 + a4 * elevation + a5 * slope + a6 * sin(3.14159/180 * aspect) + a7 * cos(3.14159/180 * aspect) + a8 * topographicShelterIndex)*topHeight^b1 * (1 - exp(b2*(tph/(standBasalAreaPerHectare))^b3*DBH))^(b4 + b4p * isPlantation), umca2016physio, start = list(a1 = 8.639, a4 = -0.0007, a5 = 0, a6 = 0.018, a7 = -0.044, a8 = 0.005, b1 = 0.169, b2 = -0.052, b3 = 0.109, b4 = 1.315, b4p = -0.247), weights = pmin(DBH^-0.8, 1)) # a5, a6, a8, b1p, b2p, b3, b3p not significant
umcaHeightFromDiameterSharmaZhang = nlrob(TotalHt ~ 1.37 + a1*standBasalAreaPerHectare^b1*(1 - exp(b2*tph^b3*DBH))^(b4 + b4p * isPlantation), umca2016, start = list(a1 = 13.6, b1 = 0.054, b2 = -0.030, b3 = 0.125, b4 = 1.295, b4p = -0.232), weights = pmin(DBH^-0.8, 1)) # a1p, b1, b1p, b2p, b3p not significant
umcaHeightFromDiameterSharmaZhangBal = nlrob(TotalHt ~ 1.37 + (a1 + a2 * basalAreaLarger)*standBasalAreaPerHectare^b1 * (1 - exp(b2*tph^(b3 + b3p * isPlantation)*DBH))^b4, umca2016, start = list(a1 = 8.742, a2 = -0.0001, b1 = 0.174, b2 = -0.039, b3 = 0.070, b3p = 0.090, b4 = 1.258), weights = pmin(DBH^-0.8, 1)) # a1p, a2, a2p, b1, b1p, b2p, b4p not significant
...
#confint_nlrob(umcaHeightFromDiameterSharmaZhangBal, level = 0.99, weights = pmin(umca2016$DBH^-0.8, 1))
...
umcaHeightFromDiameterResults = bind_rows(as_row(umcaHeightFromDiameterChapmanRichards),
                                          ...
                                          as_row(umcaHeightFromDiameterSharmaZhangBal, significant = FALSE),
