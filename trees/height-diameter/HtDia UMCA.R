...
umcaHeightFromDiameterMichaelisMenten = nlrob(TotalHt ~ 1.37 + (a1 + a1p*isPlantation)*DBH^b1 / (a2 + a2p * isPlantation + DBH^b1), umca2016, start = list(a1 = 21.4, a1p = -4.37, a2 = 43.8, a2p = -20.0, b1 = 1.27), weights = pmin(DBH^-0.8, 1)) # b1p not significant
umcaHeightFromDiameterParabolic = lm(TotalHt ~ 0 + DBH + I(DBH^2) + I(isPlantation*DBH) + I(isPlantation*DBH^2), umca2016, offset = breastHeight, weights = pmin(DBH^-0.8, 1))
umcaHeightFromDiameterProdan = nlrob(TotalHt ~ 1.37 + DBH^2 / ((a1 + a1p * isPlantation) * DBH^2 + (a2 + a2p * isPlantation)*DBH + a3), umca2016, start = list(a1 = 0.037, a1p = 0.020, a2 = 1.109, a2p = -0.472, a3 = 1.242), weights = pmin(DBH^-0.8, 1)) # a3p not significant
umcaHeightFromDiameterPower = nlrob(TotalHt ~ 1.37 + (a1 + a1p * isPlantation)*DBH^b1, umca2016, start = list(a1 = 0.821, a1p = 0.206, b1 = 0.810), weights = pmin(DBH^-0.8, 1)) # b1p not significant
umcaHeightFromDiameterRatkowsky = nlrob(TotalHt ~ 1.37 + a1*exp((b1 + b1p * isPlantation)/(DBH + b2)), umca2016, start = list(a1 = 21.6, b1 = -16.5, b1p = 1.974, b2 = 3.629), weights = pmin(DBH^-0.8, 1)) # a1p, b2p not significant
umcaHeightFromDiameterRichards = nlrob(TotalHt ~ 1.37 + (Ha + Hap * isPlantation) * (1 + ((1.37/Ha)^(1 - d) - 1) * exp((-(kU + kUp * isPlantation) * DBH)/d^(d/(1 - d))))^(1/(1 - d)), umca2016, start = list(Ha = 14.6, Hap = -4.146, d = 2.198, kU = 0.048, kUp = 0.045), weights = pmin(DBH^-0.8, 1)) # dp not significant
umcaHeightFromDiameterSharmaParton = nlrob(TotalHt ~ 1.37 + a1*topHeight^a2*(1 - exp((b1 + b1p * isPlantation)*(tph/standBasalAreaPerHectare)^b2*DBH))^b3, umca2016, start = list(a1 = 5.76, a2 = 0.261, b1 = -0.049, b1p = -0.024, b2 = 0.085, b3 = 1.234), weights = pmin(DBH^-0.8, 1)) # a1p, a2p, b2p, b3p not significant
umcaHeightFromDiameterSharmaPartonBal = nlrob(TotalHt ~ 1.37 + a1*topHeight^a2 * (1 - exp(b1*(tph/(standBasalAreaPerHectare + basalAreaLarger))^b2*DBH))^(b3 + b3p * isPlantation), umca2016, start = list(a1 = 9.074, a2 = 0.149, b1 = -0.065, b2 = 0.013, b3 = 1.299, b3p = -0.254), weights = pmin(DBH^-0.8, 1)) # a1p, a2, a2p, b1p, b2p not significant
umcaHeightFromDiameterSharmaPartonBalPhysio = nlrob(TotalHt ~ 1.37 + a1*topHeight^a2 * (1 + a3 * elevation + a4 * sin(3.14159/180 * aspect) + a5 * cos(3.14159/180 * aspect) + a6 * topographicShelterIndex) * (1 - exp(b1*(tph/(standBasalAreaPerHectare + basalAreaLarger))^b2*DBH))^(b3 + b3p * isPlantation), umca2016physio, start = list(a1 = 9.30, a2 = 0.151, a3 = -0.0007, a4 = 0.0161, a5 = -0.040, a6 = 0.005, b1 = -0.061, b2 = 0.061, b3 = 1.313, b3p = -0.253), weights = pmin(DBH^-0.8, 1)) # a2, a4, a5, a6, b1p, b2p not significant
umcaHeightFromDiameterSharmaPartonPhysio = nlrob(TotalHt ~ 1.37 + a1*topHeight^a2 * (1 + a3 * elevation + a4 * sin(3.14159/180 * aspect) + a5 * cos(3.14159/180 * aspect) + a6 * topographicShelterIndex) * (1 - exp(b1*(tph/(standBasalAreaPerHectare))^b2*DBH))^(b3 + b3p * isPlantation), umca2016physio, start = list(a1 = 8.639, a2 = 0.169, a3 = -0.0007, a4 = 0.018, a5 = -0.044, a6 = 0.005, b1 = -0.052, b2 = 0.109, b3 = 1.315, b3p = -0.247), weights = pmin(DBH^-0.8, 1)) # a2p, a4, a5, a6, b1p, b2, b2p not significant
umcaHeightFromDiameterSharmaZhang = nlrob(TotalHt ~ 1.37 + a1*standBasalAreaPerHectare^a2*(1 - exp(b1*tph^b2*DBH))^(b3 + b3p * isPlantation), umca2016, start = list(a1 = 13.6, a2 = 0.054, b1 = -0.030, b2 = 0.125, b3 = 1.295, b3p = -0.232), weights = pmin(DBH^-0.8, 1)) # a1p, a2, a2p, b1p, b2p not significant
umcaHeightFromDiameterSharmaZhangBal = nlrob(TotalHt ~ 1.37 + a1*standBasalAreaPerHectare^a2 * (1 + (a3 + a3p * isPlantation) * basalAreaLarger) * (1 - exp(b1*tph^(b2 + b2p * isPlantation)*DBH))^b3, umca2016, start = list(a1 = 8.742, a2 = 0.174, a3 = -0.0001, a3p = -0.007, b1 = -0.039, b2 = 0.070, b2p = 0.090, b3 = 1.258), weights = pmin(DBH^-0.8, 1)) # a1p, a2, a2p, a3, b1p, b3p not significant
umcaHeightFromDiameterSibbesen = nlrob(TotalHt ~ 1.37 + (a1 + a1p * isPlantation)*DBH^(b1*DBH^(b2 + b2p * isPlantation)), umca2016, start = list(a1 = 0.346, a1p = 0.153, b1 = 1.783, b2 = -0.150, b2p = -0.033), weights = pmin(DBH^-0.8, 1)) # b1p not significant
umcaHeightFromDiameterWeibull = nlrob(TotalHt ~ 1.37 + (a1 + a1p * isPlantation)*(1 - exp((b1 + b1p * isPlantation)*DBH^b2)), umca2016, start = list(a1 = 16.7, a1p = -3.627, b1 = -0.0315, b1p = -0.0256, b2 = 1.174), weights = pmin(DBH^-0.8, 1)) # b2p not significant
umcaHeightFromDiameterWeibullBal = nlrob(TotalHt ~ 1.37 + (a1 + a2 * basalAreaLarger + a3 * standBasalAreaPerHectare) * (1 - exp((b1 + b1p * isPlantation)*DBH^b2)), umca2016, start = list(a1 = 18.0, a2 = 0.079, a3 = -0.037, b1 = -0.037, b1p = -0.007, b2 = 1.043), weights = pmin(DBH^-0.8, 1)) # a1p, a2, a2p, a3p, b2p not significant
umcaHeightFromDiameterWeibullBalRelHt = nlrob(TotalHt ~ 1.37 + (a1 + a2*basalAreaLarger + a3*standBasalAreaPerHectare + a4*pmin(relativeHeight, 1.25)) * (1 - exp((b1 + b1p * isPlantation) * DBH^b2)), umca2016, start = list(a1 = -2.24, a2 = 0.017, a3 = -0.053, a4 = 21.4, b1 = -0.284, b1p = -0.094, b2 = 0.420), weights = pmin(DBH^-0.8, 1), control = nls.control(maxiter = 500)) # a1p, a2p, a3, a3p, a4p, b2p not significant
#confint_nlrob(umcaHeightFromDiameterWeibullBalRelHt, level = 0.99, weights = pmin(umca2016$DBH^-0.8, 1))
...
umcaHeightFromDiameterChapmanRichardsBal = get_height_error("Chapman-Richards BA+L", umcaHeightFromDiameterChapmanRichardsBal, umca2016, umca2016natural, umca2016plantation)
umcaHeightFromDiameterChapmanRichardsBalPhysio = get_height_error("Chapman-Richards BA+L physio", umcaHeightFromDiameterChapmanRichardsBalPhysio, umca2016physio, umca2016natural, umca2016plantationPhysio)
umcaHeightFromDiameterChapmanRichardsBalRelHt = get_height_error("Chapman-Richards BA+L RelHt", umcaHeightFromDiameterChapmanRichardsBalRelHt, umca2016, umca2016natural, umca2016plantation)
...
umcaHeightFromDiameterGam = get_height_error("GCV GAM", umcaHeightFromDiameterGam, umca2016, umca2016natural, umca2016plantation)
umcaHeightFromDiameterGamBal = get_height_error("GCV GAM BA+L", umcaHeightFromDiameterGamBal, umca2016, umca2016natural, umca2016plantation)
#umcaHeightFromDiameterGamBalPhysio = get_height_error("GCV GAM BA+L physio", umcaHeightFromDiameterGamBalPhysio, umca2016physio, umca2016natural, umca2016plantation) # slow
umcaHeightFromDiameterGamPhysio = get_height_error("GCV GAM physio", umcaHeightFromDiameterGamPhysio, umca2016physio, umca2016natural, umca2016plantation)
#save(umcaHeightFromDiameterGamBalPhysio, umcaHeightFromDiameterGamPhysio, file = "trees/height-diameter/HtDia UMCA spline height.rdata")
#load("trees/height-diameter/HtDia UMCA spline height.rdata")
...
umcaHeightFromDiameterSharmaPartonBal = get_height_error("Sharma-Parton BA+L", umcaHeightFromDiameterSharmaPartonBal, umca2016, umca2016natural, umca2016plantation)
umcaHeightFromDiameterSharmaPartonBalPhysio = get_height_error("Sharma-Parton BA+L physio", umcaHeightFromDiameterSharmaPartonBalPhysio, umca2016physio, umca2016natural, umca2016plantationPhysio)
...
umcaHeightFromDiameterSharmaZhangBal = get_height_error("Sharma-Zhang BA+L", umcaHeightFromDiameterSharmaZhangBal, umca2016, umca2016natural, umca2016plantation)
...
umcaHeightFromDiameterWeibullBal = get_height_error("Weibull BA+L", umcaHeightFromDiameterWeibullBal, umca2016, umca2016natural, umca2016plantation)
umcaHeightFromDiameterWeibullBalRelHt = get_height_error("Weibull BA+L RelHt", umcaHeightFromDiameterWeibullBalRelHt, umca2016, umca2016natural, umca2016plantation)
...
umcaHeightFromDiameterResults = bind_rows(as_row(umcaHeightFromDiameterChapmanRichards),
                                          ...
                                          as_row(umcaHeightFromDiameterGam),
                                          as_row(umcaHeightFromDiameterGamBal),
                                          #as_row(umcaHeightFromDiameterGamBalPhysio),
                                          as_row(umcaHeightFromDiameterGamPhysio),
...
#umcaHeightFromDiameterChapmanRichardsGnls = gnls(TotalHt ~ 1.37 + a1*(1 - exp(b1*DBH))^(b2 + b2p * isPlantation), umca2016, start = umcaHeightFromDiameterChapmanRichards$m$getPars(), weights = varPower(0.40, ~DBH | isPlantation), control = gnlsControl(nlsTol = 0.001, msVerbose = FALSE, returnObject = FALSE)) # step halving with plot correlation
...
#umcaHeightFromDiameterSharmaPartonGnls = gnls(TotalHt ~ 1.37 + a1*topHeight^a2 * (1 - exp((b1 + b1p * isPlantation)*(tph/standBasalAreaPerHectare)^b2*DBH))^b3, umca2016, start = umcaHeightFromDiameterSharmaParton$m$getPars(), weights = varPower(0.40, ~DBH | isPlantation), control = gnlsControl(nlsTol = 0.001, msVerbose = FALSE, returnObject = FALSE)) # step halving with plot correlation
#umcaHeightFromDiameterSharmaPartonBalGnls = gnls(TotalHt ~ 1.37 + a1*topHeight^a2 * (1 - exp(b1*(tph/(standBasalAreaPerHectare + basalAreaLarger))^b2*DBH))^(b3 + b3p * isPlantation), umca2016, start = umcaHeightFromDiameterSharmaPartonBal$m$getPars(), weights = varPower(0.40, ~DBH | isPlantation), control = gnlsControl(nlsTol = 0.001, msVerbose = FALSE, returnObject = FALSE)) # step halving with plot correlation
#umcaHeightFromDiameterSharmaZhangGnls = gnls(TotalHt ~ 1.37 + a1*standBasalAreaPerHectare^a2 * (1 - exp(b1*tph^b2*DBH))^(b3 + b3p * isPlantation), umca2016, start = umcaHeightFromDiameterSharmaZhang$m$getPars(), weights = varPower(0.40, ~DBH | isPlantation), control = gnlsControl(nlsTol = 0.001, msVerbose = FALSE, returnObject = FALSE)) # step halving with plot correlation
#umcaHeightFromDiameterSharmaZhangBalGnls = gnls(TotalHt ~ 1.37 + a1*standBasalAreaPerHectare^a2 * (1 + (a3 + a3p * isPlantation) * basalAreaLarger) * (1 - exp(b1*tph^(b2 + b2p * isPlantation)*DBH))^b3, umca2016, start = umcaHeightFromDiameterSharmaZhangBal$m$getPars(), weights = varPower(0.40, ~DBH | isPlantation), control = gnlsControl(nlsTol = 0.005, maxIter = 250, nlsMaxIter = 50, msVerbose = FALSE, returnObject = FALSE)) # step halving with plot correlation or nlsTol = 0.001
#umcaHeightFromDiameterWeibullGnls = gnls(TotalHt ~ 1.37 + (a1 + a1p * isPlantation)*(1 - exp((b1 + b1p * isPlantation)*DBH^b2)), umca2016, start = umcaHeightFromDiameterWeibull$m$getPars(), weights = varPower(0.40, ~DBH | isPlantation), control = gnlsControl(nlsTol = 0.001, msVerbose = FALSE, returnObject = FALSE)) # step halving with plot correlation
...
umcaHeightFromDiameterChapmanRichardsBalGnls = get_height_error("Chapman-Richards BA+L GNLS", umcaHeightFromDiameterChapmanRichardsBalGnls, umca2016, umca2016natural, umca2016plantation)
...
umcaHeightFromDiameterSharmaPartonBalGnls = get_height_error("Sharma-Parton BA+L GNLS", umcaHeightFromDiameterSharmaPartonBalGnls, umca2016, umca2016natural, umca2016plantation)
...
umcaHeightFromDiameterSharmaZhangBalGnls = get_height_error("Sharma-Zhang BA+L GNLS", umcaHeightFromDiameterSharmaZhangBalGnls, umca2016, umca2016natural, umca2016plantation)
...
umcaHeightFromDiameterWeibullBalGnls = get_height_error("Weibull BA+L GNLS", umcaHeightFromDiameterWeibullBalGnls, umca2016, umca2016natural, umca2016plantation)
...
umcaDiameterFromHeightChapmanFormBalRelHt = gsl_nls(DBH ~ (a1 + a2 * basalAreaLarger + a3 * standBasalAreaPerHectare + a4 * relativeHeight) * (exp(b1*(TotalHt - 1.37)^b2) - 1), umca2016, start = list(a1 = 1500, a2 = -400.0, a3 = 520, a4 = -2000, b1 = 0.0006, b2 = 1.02), weights = pmin(TotalHt^if_else(isPlantation, -1.6, -1.1), 0.5)) # step factor with nls() and nlrob()
umcaDiameterFromHeightChapmanFormRelHt = gsl_nls(DBH ~ (a1 + a2 * pmin(relativeHeight, 1.25))*(exp(b1*(TotalHt - 1.37)^b2) - 1), umca2016, start = list(a1 = 665, a2 = -406, b1 = 0.0034, b2 = 1.12), weights = pmin(TotalHt^if_else(isPlantation, -1.6, -1.1), 0.5), control = nls.control(maxiter = 50)) # step factor with nls(), step factor with nlrob()
...
umcaDiameterFromHeightPowerRelHt = nlrob(DBH ~ (a1 + a1p * isPlantation + (a2 + a2p * isPlantation) * relativeHeight)*(TotalHt - 1.37)^b1, umca2016, start = list(a1 = 2.37, a1p = -0.887, a2 = -1.508, a2p = 1513, b1 = 1.123), weights = pmin(TotalHt^if_else(isPlantation, -1.6, -1.1), 0.5)) 
...
umcaDiameterFromHeightSharmaParton = gsl_nls(DBH ~ a1*(TotalHt - 1.37)^a2*(exp(b1*(tph/topHeight)^b2*(TotalHt - 1.37)) - 1)^b3, umca2016, start = list(a1 = 104, a2 = 0.66, b1 = 0.0001, b2 = -1.17, b3 = 0.31), weights = pmin(TotalHt^if_else(isPlantation, -1.6, -1.1), 0.5)) # a2p, b2p, b3p not significant, NaN-inf with nls() at nls_multistart() point, NaN-inf or code error with nlrob()
...
umcaDiameterFromHeightSibbesenFormPhysio = nlrob(DBH ~ (a1 + a1p * isPlantation + a2 * elevation + a3 * sin(3.14159/180 * slope) + a4 * cos(3.14159/180 * aspect) + a5 * sin(3.14159/180 * aspect) + a6 * topographicShelterIndex)*(TotalHt - 1.37)^(b1*(TotalHt - 1.37)^b2), umca2016physio, start = list(a1 = 1.02, a1p = -0, a2 = 0, a3 = -0, a4 = -0, a5 = -0, a6 = 0, b1 = 1.96, b2 = -0.14), weights = pmin(TotalHt^if_else(isPlantation, -1.6, -1.1), 0.5)) # a2, a4, a5 not significant, step factor with b1p
umcaDiameterFromHeightSibbesenFormRelHt = nlrob(DBH ~ (a1 + a2 * relativeHeight)*(TotalHt - 1.37)^(b1*(TotalHt - 1.37)^b2), umca2016, start = list(a1 = 0.496, a2 = -0.188, b1 = 2.31, b2 = -0.12), weights = pmin(TotalHt^if_else(isPlantation, -1.6, -1.1), 0.5)) # a2 not significant
...
umcaDiameterFromHeightChapmanFormAat = get_dbh_error("Chapman-Richards form AA+T", umcaDiameterFromHeightChapmanFormAat, umca2016, umca2016natural, umca2016plantation)
umcaDiameterFromHeightChapmanFormBal = get_dbh_error("Chapman-Richards form BA+L", umcaDiameterFromHeightChapmanFormBal, umca2016, umca2016natural, umca2016plantation)
umcaDiameterFromHeightChapmanFormBalRelHt = get_dbh_error("Chapman-Richards form BA+L RelHt", umcaDiameterFromHeightChapmanFormBalRelHt, umca2016, umca2016natural, umca2016plantation)
...
umcaDiameterFromHeightChapmanRichardsAat = get_dbh_error("Chapman-Richards AA+T", umcaDiameterFromHeightChapmanRichardsAat, umca2016, umca2016natural, umca2016plantation)
...
umcaDiameterFromHeightGam = get_dbh_error("GCV GAM", umcaDiameterFromHeightGam, umca2016, umca2016natural, umca2016plantation)
umcaDiameterFromHeightGamAat = get_dbh_error("GCV GAM AA+T", umcaDiameterFromHeightGamAat, umca2016, umca2016natural, umca2016plantation)
#umcaDiameterFromHeightGamAatPhysio = get_dbh_error("GCV GAM AA+T physio", umcaDiameterFromHeightGamAatPhysio, umca2016physio, umca2016natural, umca2016plantation)
umcaDiameterFromHeightGamPhysio = get_dbh_error("GCV GAM physio", umcaDiameterFromHeightGamPhysio, umca2016physio, umca2016natural, umca2016plantation)
#save(umcaDiameterFromHeightGamAatPhysio, umcaDiameterFromHeightGamPhysio, file = "trees/height-diameter/HtDia UMCA spline DBH.rdata")
#load("trees/height-diameter/HtDia UMCA spline DBH.rdata")
...
umcaDiameterFromHeightPowerAat = get_dbh_error("power AA+T", umcaDiameterFromHeightPowerAat, umca2016, umca2016natural, umca2016plantation)
...
umcaDiameterFromHeightSibbesenFormAat = get_dbh_error("Sibbesen form AA+T", umcaDiameterFromHeightSibbesenFormAat, umca2016, umca2016natural, umca2016plantation)
...
umcaDiameterFromHeightResults = bind_rows(as_row(umcaDiameterFromHeightChapmanRichards),
                                          ...
                                          as_row(umcaDiameterFromHeightGam),
                                          as_row(umcaDiameterFromHeightGamAat),
                                          #as_row(umcaDiameterFromHeightGamAatPhysio),
                                          as_row(umcaDiameterFromHeightGamPhysio),
...
umcaParameters = bind_rows(bind_rows(get_coefficients(umcaHeightFromDiameterChapmanRichards),
                                     ...
                                     get_coefficients(umcaHeightFromDiameterGam),
                                     get_coefficients(umcaHeightFromDiameterGamBal),
                                     #get_coefficients(umcaHeightFromDiameterGamBalPhysio),
                                     get_coefficients(umcaHeightFromDiameterGamPhysio),
                           ...
                           bind_rows(get_coefficients(umcaDiameterFromHeightChapmanRichards),
                                     ...
                                     get_coefficients(umcaDiameterFromHeightGam),
                                     get_coefficients(umcaDiameterFromHeightGamAat),
                                     #get_coefficients(umcaDiameterFromHeightGamAatPhysio),
                                     get_coefficients(umcaDiameterFromHeightGamPhysio),
