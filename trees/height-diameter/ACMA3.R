...
acmaHeightFromDiameter$chapmanRichardsBal = fit_gsl_nls("Chapman-Richards BA+L", TotalHt ~ 1.37 + (a1 + a2 * basalAreaLarger) * (1 - exp(b1*DBH))^(b2 + b2p * isPlantation), acma2016, start = list(a1 = 64.9, a2 = 0.023, b1 = -0.021, b2 = 1.47, b2p = -0.29), significant = FALSE) # a1p, a2, a2p, a3, a3p not significant, b1p-b2p not mutually significant
acmaHeightFromDiameter$chapmanRichardsBalPhysio = fit_gsl_nls("Chapman-Richards BA+L physio", TotalHt ~ 1.37 + (a1 + a2 * basalAreaLarger + a5 * slope) * (1 - exp(b1*DBH))^(b2 + b2p * isPlantation), acma2016physio, start = list(a1 = 30, a2 = 0.06, a5 = -0.1, b1 = -0.03, b2 = 1.1, b2p = -0.19)) # a1p, a2, a2p, a3, a3p, a4, a7, a8, b1p not significant, a5-a6 not mutually significant, a3p debatable
acmaHeightFromDiameter$chapmanRichardsBalRelHt = fit_gsl_nls("Chapman-Richards BA+L RelHt", TotalHt ~ 1.37 + (a1 + a2 * basalAreaLarger + (a4 + a4p * isPlantation) * relativeHeight) * (1 - exp(b1*DBH))^b2, acma2016, start = list(a1 = 0.5, a2 = 0.25, a4 = 45, a4p = -14, b1 = -0.030, b2 = 0.5)) # a1, a1p, a2, a2p, a3, a3p, b1p, b2p not significant, NaN-inf with b1p, job step factor with nlrob()
...
acmaHeightFromDiameter$richards = fit_gsl_nls("unified Richards", TotalHt ~ 1.37 + Ha * (1 + ((1.37/Ha)^(1 - d) - 1) * exp((-(kU + kUp * isPlantation) * DBH)/d^(d/(1 - d))))^(1/(1 - d)), acma2016, start = list(Ha = 22.8, d = 0.723, kU = 0.025, kUp = 0.0064)) # Hap, dp not significant
...
acmaHeightFromDiameter$sharmaPartonBal = fit_gsl_nls("Sharma-Parton BA+L", TotalHt ~ 1.37 + a1*topHeight^b1 * (1 - exp(b2*(tph/(standBasalAreaPerHectare + basalAreaLarger))^b3*DBH))^(b4 + b4p * isPlantation), acma2016, start = list(a1 = 20, b1 = 0.1, b2 = -0.04, b3 = -0.1, b4 = 1.1, b4p = -0.24)) # a1p, b1p, b2p, b3p not significant
...
acmaHeightFromDiameter$weibullBal = fit_gsl_nls("Weibull BA+L", TotalHt ~ 1.37 + (a1 + a2 * basalAreaLarger) * (1 - exp((b1 + b1p * isPlantation)*DBH^b2)), acma2016, start = list(a1 = 26, a2 = 0.1, b1 = -0.027, b1p = -0.01, b2 = 0.99)) # a1p, a2p, a3p, b2p not significant, a2+a3 not mutually significant
...
acmaHeightFromDiameterNlrob$chapmanRichardsBal = fit_nlrob("Chapman-Richards BA+L", TotalHt ~ 1.37 + (a1 + a2 * basalAreaLarger) * (1 - exp(b1*DBH))^(b2 + b2p * isPlantation), acma2016, start = list(a1 = 64.9, a2 = 0.023, b1 = -0.021, b2 = 1.47, b2p = -0.29), significant = FALSE)
acmaHeightFromDiameterNlrob$chapmanRichardsBalPhysio = fit_nlrob("Chapman-Richards BA+L physio", TotalHt ~ 1.37 + (a1 + a2 * basalAreaLarger + a5 * slope) * (1 - exp(b1*DBH))^(b2 + b2p * isPlantation), acma2016physio, start = list(a1 = 30, a2 = 0.10, a5 = -0.1, b1 = -0.03, b2 = 1.1, b2p = -0.19), significant = FALSE) # a2, a3 not significant
acmaHeightFromDiameterNlrob$chapmanRichardsBalRelHt = fit_nlrob("Chapman-Richards BA+L RelHt", TotalHt ~ 1.37 + (a1 + a2 * basalAreaLarger + (a4 + a4p * isPlantation) * relativeHeight) * (1 - exp(b1*DBH))^b2, acma2016, start = list(a1 = 0.5, a2 = 0.25, a4 = 45, a4p = -14, b1 = -0.030, b2 = 0.5))
...
acmaHeightFromDiameterNlrob$richards = fit_nlrob("unified Richards", TotalHt ~ 1.37 + Ha * (1 + ((1.37/Ha)^(1 - d) - 1) * exp((-(kU + kUp * isPlantation) * DBH)/d^(d/(1 - d))))^(1/(1 - d)), acma2016, start = list(Ha = 22.8, d = 0.723, kU = 0.025, kUp = 0.0064)) # Hap, dp not significant
...
acmaHeightFromDiameterNlrob$sharmaPartonBal = fit_nlrob("Sharma-Parton BA+L", TotalHt ~ 1.37 + a1*topHeight^b1 * (1 - exp(b2*(tph/(standBasalAreaPerHectare + basalAreaLarger))^b3*DBH))^(b4 + b4p * isPlantation), acma2016, start = list(a1 = 20, b1 = 0.1, b2 = -0.04, b3 = -0.1, b4 = 1.1, b4p = -0.24)) # a1p, b1p, b2p, b3p not significant
...
acmaHeightFromDiameterNlrob$weibullBal = fit_nlrob("Weibull BA+L", TotalHt ~ 1.37 + (a1 + a2 * basalAreaLarger) * (1 - exp((b1 + b1p * isPlantation)*DBH^b2)), acma2016, start = list(a1 = 26, a2 = 0.1, b1 = -0.027, b1p = -0.01, b2 = 0.99)) # a1p, a2p, a3p, b2p not significant, a2+a3 not mutually significant
#acmaHeightFromDiameterNlrob$weibullBalRelHt = fit_gsl_nls("Weibull BA+L RelHt", TotalHt ~ 1.37 + (a1 + a2 * basalAreaLarger + a4 * pmin(relativeHeight, 1.25)) * (1 - exp((b1 + b1p * isPlantation)*DBH^b2)), acma2016, start = list(a1 = 100, a2 = -20, a4 = -100, b1 = 0.03, b1p = -0.001, b2 = 0.33), control = gsl_nls_control(maxiter = 250, xtol = 0.01)) # a3, a4p, b1p not significant, a1-a4 evaporation: step factor with a1p, a2p, a3p, b2p, b3p
...
acmaHeightFromDiameterGslNlsDefault$chapmanRichardsBal = fit_gsl_nls("Chapman-Richards BA+L", TotalHt ~ 1.37 + (a1 + a2 * basalAreaLarger) * (1 - exp(b1*DBH))^(b2 + b2p * isPlantation), acma2016defaultWeight, start = list(a1 = 64.9, a2 = 0.023, b1 = -0.021, b2 = 1.47, b2p = -0.29), significant = FALSE)
acmaHeightFromDiameterGslNlsDefault$chapmanRichardsBalPhysio = fit_gsl_nls("Chapman-Richards BA+L physio", TotalHt ~ 1.37 + (a1 + a2 * basalAreaLarger + a5 * slope) * (1 - exp(b1*DBH))^(b2 + b2p * isPlantation), acma2016defaultWeightPhysio, start = list(a1 = 30, a2 = 0.06, a5 = -0.1, b1 = -0.03, b2 = 1.08, b2p = -0.18))
acmaHeightFromDiameterGslNlsDefault$chapmanRichardsBalRelHt = fit_gsl_nls("Chapman-Richards BA+L RelHt", TotalHt ~ 1.37 + (a1 + a2 * basalAreaLarger + (a4 + a4p * isPlantation) * relativeHeight) * (1 - exp(b1*DBH))^b2, acma2016defaultWeight, start = list(a1 = 0.5, a2 = 0.25, a4 = 45, a4p = -14, b1 = -0.030, b2 = 0.5))
...
acmaHeightFromDiameterGslNlsDefault$richards = fit_gsl_nls("unified Richards", TotalHt ~ 1.37 + Ha * (1 + ((1.37/Ha)^(1 - d) - 1) * exp((-(kU + kUp * isPlantation) * DBH)/d^(d/(1 - d))))^(1/(1 - d)), acma2016defaultWeight, start = list(Ha = 22.8, d = 0.723, kU = 0.025, kUp = 0.0064))
...
acmaHeightFromDiameterGslNlsDefault$sharmaPartonBal = fit_gsl_nls("Sharma-Parton BA+L", TotalHt ~ 1.37 + a1*topHeight^b1 * (1 - exp(b2*(tph/(standBasalAreaPerHectare + basalAreaLarger))^b3*DBH))^(b4 + b4p * isPlantation), acma2016defaultWeight, start = list(a1 = 20, b1 = 0.1, b2 = -0.04, b3 = -0.1, b4 = 1.1, b4p = -0.24))
acmaHeightFromDiameterGslNlsDefault$sharmaPartonBalPhysio = fit_gsl_nls("Sharma-Parton BA+L physio", TotalHt ~ 1.37 + (a1 + a6 * sin(3.14159/180 * aspect))*topHeight^(b1 + b1p * isPlantation) * (1 - exp(b2*(tph/(standBasalAreaPerHectare + basalAreaLarger))^b3*DBH))^b4, acma2016defaultWeightPhysio, start = list(a1 = 12, a6 = 0.4, b1 = 0.2, b1p = 0.04, b2 = -0.037, b3 = -0.046, b4 = 1.06))
...
acmaHeightFromDiameterGslNlsDefault$sharmaZhangBal = fit_gsl_nls("Sharma-Zhang BA+L", TotalHt ~ 1.37 + (a1 + a2 * basalAreaLarger)*standBasalAreaPerHectare^b1 * (1 - exp(b2*tph^b3*DBH))^(b4 + b4p * isPlantation), acma2016defaultWeight, start = list(a1 = 22, a2 = 0.05, b1 = 0.05, b2 = -0.04, b3 = -0.04, b4 = 1.07, b4p = -0.2), significant = FALSE)
...
acmaHeightFromDiameterGslNlsDefault$weibullBal = fit_gsl_nls("Weibull BA+L", TotalHt ~ 1.37 + (a1 + a2 * basalAreaLarger) * (1 - exp((b1 + b1p * isPlantation)*DBH^b2)), acma2016defaultWeight, start = list(a1 = 26, a2 = 0.1, b1 = -0.027, b1p = -0.01, b2 = 0.99))
acmaHeightFromDiameterGslNlsDefault$weibullBalRelHt = fit_gsl_nls("Weibull BA+L RelHt", TotalHt ~ 1.37 + (a1 + a2 * basalAreaLarger + a4 * pmin(relativeHeight, 1.25)) * (1 - exp((b1 + b1p * isPlantation)*DBH^b2)), acma2016defaultWeight, start = list(a1 = 100, a2 = -20, a4 = -100, b1 = 0.03, b1p = -0.001, b2 = 0.33), control = gsl_nls_control(maxiter = 250, xtol = 0.01))
...
acmaDiameterFromHeight$chapmanRichardsAbat = fit_gsl_nls("Chapman-Richards inverse ABA+T", DBH ~ (a1 + a2 * tallerApproxBasalArea)*log(1 - pmin(b1*(TotalHt - 1.37)^b2, 0.9999)), acma2016, start = list(a1 = 40, a2 = 0, b1 = -0.021, b2 = 1.6), control = list(maxiter = 500), significant = FALSE) # a2, b2p not significant
...
acmaDiameterFromHeight$ruarkAbat = fit_gsl_nls("Ruark ABA+T", DBH ~ (a1 + a2 * tallerApproxBasalArea)*(TotalHt - 1.37)^(b1 + b1p * isPlantation) * exp(b2 * (TotalHt - 1.37)), acma2016, start = list(a1 = 1.4, a2 = 0, b1 = 1.45, b1p = -0.3, b2 = -0.03), significant = FALSE) # a2, a3, b1p-b2p not mutually significant
...
acmaDiameterFromHeight$sharmaParton = fit_gsl_nls("modified Sharma-Parton", DBH ~ a1*(TotalHt - 1.37)^b1*(exp(b2*(tph/topHeight)^b3*(TotalHt - 1.37)) - 1)^b4, acma2016, start = list(a1 = 3.3, b1 = 0.9, b2 = 0.001, b3 = 1, b4 = 0.5), control = gsl_nls_control(maxiter = 500, xtol = 0.002)) # NaN-inf from nls() at nls_multstart() positions, NaN-inf or code error with nlrob(), a1 -> 0 with gsl_nls() -> eliminate b4
...
acmaDiameterFromHeight$sibbesenReplaceAbat = fit_gsl_nls("Sibbesen replace ABA+T", DBH ~ (a1 + a2 * tallerApproxBasalArea)*(TotalHt - 1.37)^(b1*(TotalHt - 1.37)^b2), acma2016, start = list(a1 = 0.5, a2 = 0.0002, b1 = 2.2, b2 = -0.1), significant = FALSE) # a2 not significant
...
acmaDiameterFromHeightNlrob$chapmanRichardsAbat = fit_nlrob("Chapman-Richards inverse ABA+T", DBH ~ (a1 + a2 * tallerApproxBasalArea)*log(1 - pmin(b1*(TotalHt - 1.37)^b2, 0.9999)), acma2016, start = list(a1 = 40, a2 = 0, b1 = -0.021, b2 = 1.6), control = list(maxiter = 500), significant = FALSE)
acmaDiameterFromHeightNlrob$chapmanRichardsPhysio = fit_nlrob("Chapman-Richards inverse physio", DBH ~ (a1 + a8 * topographicShelterIndex)*log(1 - pmin(b1*(TotalHt - 1.37)^b2, 0.9999)), acma2016physio, start = list(a1 = 40, a8 = 0, b1 = -0.023, b2 = 1.6), significant = FALSE)
...
acmaDiameterFromHeightNlrob$ruarkAbat = fit_nlrob("Ruark ABA+T", DBH ~ (a1 + a2 * tallerApproxBasalArea)*(TotalHt - 1.37)^(b1 + b1p * isPlantation) * exp(b2 * (TotalHt - 1.37)), acma2016, start = list(a1 = 1.4, a2 = 0, b1 = 1.45, b1p = -0.3, b2 = -0.03), significant = FALSE)
...
acmaDiameterFromHeightNlrob$ruarkRelHt = fit_nlrob("Ruark RelHt", DBH ~ (a1 + a9 * relativeHeight)*(TotalHt - 1.37)^(b1 + b1p * isPlantation) * exp(b2 * (TotalHt - 1.37)), acma2016, start = list(a1 = 1.4, a9 = 0, b1 = 1.45, b1p = -0.3, b2 = -0.03), significant = FALSE)
acmaDiameterFromHeightNlrob$ruarkRelHtPhysio = fit_nlrob("Ruark RelHt physio", DBH ~ (a1 + a7 * sin(3.14159/180 * aspect) + a9 * relativeHeight)*(TotalHt - 1.37)^(b1 + b1p * isPlantation) * exp(b2 * (TotalHt - 1.37)), acma2016physio, start = list(a1 = 1.2, a7 = -0.06, a9 = 0, b1 = 1.45, b1p = -0.064, b2 = 0.03), significant = FALSE)
...
acmaDiameterFromHeightNlrob$sibbesenReplaceAbat = fit_nlrob("Sibbesen replace ABA+T", DBH ~ (a1 + a2 * tallerApproxBasalArea)*(TotalHt - 1.37)^(b1*(TotalHt - 1.37)^b2), acma2016, start = list(a1 = 0.5, a2 = 0.0002, b1 = 2.2, b2 = -0.1), significant = FALSE)
...
acmaDiameterFromHeightGslNlsDefault$chapmanReplaceAbat = fit_gsl_nls("Chapman-Richards replace ABA+T", DBH ~ (a1 + a2 * tallerApproxBasalArea)*(exp(b1*(TotalHt - 1.37)) - 1)^b2, acma2016, start = list(a1 = 50, a2 = 15, b1 = 0.001, b2 = 1.0), control = gsl_nls_control(maxiter = 250, xtol = 1E-3), significant = FALSE)
#acmaDiameterFromHeightGslNlsDefault$chapmanReplaceBal = fit_gsl_nls("Chapman-Richards replace BA+L", DBH ~ (a1 + a2 * basalAreaLarger) * (exp(b1*(TotalHt - 1.37)^b2) - 1), acma2016, start = list(a1 = 50, a2 = -15, b1 = 0.001, b2 = 1.0), control = gsl_nls_control(maxiter = 250, xtol = 1E-3), significant = FALSE)
#acmaDiameterFromHeightGslNlsDefault$chapmanReplaceBalRelHt = fit_gsl_nls("Chapman-Richards replace BA+L RelHt", DBH ~ (a1 + a2 * basalAreaLarger + a3 * standBasalAreaPerHectare + a9 * relativeHeight) * (exp(b1*(TotalHt - 1.37)^b2) - 1), acma2016, start = list(a1 = 50, a2 = -15, a3 = 4, a9 = -200, b1 = 0.001, b2 = 1.00), control = gsl_nls_control(maxiter = 250, xtol = 1E-3), significant = FALSE)
...
acmaDiameterFromHeightGslNlsDefault$chapmanRichardsAbat = fit_gsl_nls("Chapman-Richards inverse ABA+T", DBH ~ (a1 + a2 * tallerApproxBasalArea)*log(1 - pmin(b1*(TotalHt - 1.37)^b2, 0.9999)), acma2016, start = list(a1 = 40, a2 = 0, b1 = -0.021, b2 = 1.6), control = list(maxiter = 500), significant = FALSE)
...
acmaDiameterFromHeightGslNlsDefault$ruarkAbat = fit_gsl_nls("Ruark ABA+T", DBH ~ (a1 + a2 * tallerApproxBasalArea)*(TotalHt - 1.37)^(b1 + b1p * isPlantation) * exp(b2 * (TotalHt - 1.37)), acma2016, start = list(a1 = 1.4, a2 = 0, b1 = 1.45, b1p = -0.3, b2 = -0.03), significant = FALSE)
...
acmaDiameterFromHeightGslNlsDefault$sibbesenReplaceAbat = fit_gsl_nls("Sibbesen replace ABA+T", DBH ~ (a1 + a2 * tallerApproxBasalArea)*(TotalHt - 1.37)^(b1*(TotalHt - 1.37)^b2), acma2016, start = list(a1 = 0.5, a2 = 0.0002, b1 = 2.2, b2 = -0.1), significant = FALSE)
...
{
  save(file = "trees/height-diameter/data/ACMA3 models and stats.Rdata", 
       acmaHeightFromDiameter, acmaHeightFromDiameterGslNlsDefault, acmaHeightFromDiameterNlrob,
       acmaDiameterFromHeight, acmaDiameterFromHeightGslNlsDefault, acmaDiameterFromHeightNlrob)
} else {
  save(file = "trees/height-diameter/data/ACMA3 stats.Rdata", 
       acmaHeightFromDiameter, acmaHeightFromDiameterGslNlsDefault, acmaHeightFromDiameterNlrob,
       acmaDiameterFromHeight, acmaDiameterFromHeightGslNlsDefault, acmaDiameterFromHeightNlrob)
}
