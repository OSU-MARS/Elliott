...
psme2016 = trees2016 %>% filter(Species == "DF", isLiveUnbroken, TotalHt > 0) %>% # live Douglas-firs measured for height
  mutate(dbhWeight = pmin(1/(1.84*DBH^(0.71 - 0.048*isPlantation)), 5), # residuals.R::varianceForHeight$psme
         heightWeight = pmin(1/((5.74 - 5.73*isPlantation)*(TotalHt - 1.37)^(1.08 + 1.79*isPlantation)), 5)) # residuals.R:varianceForDbh$psme
...
psme2016defaultWeight = psme2016 %>% mutate(dbhWeight = pmin(1/DBH, 5))
psme2016defaultWeightPhysio = psme2016defaultWeight %>% filter(is.na(elevation) == FALSE)

psmeOptions = list(fitHeightPrimary = FALSE, # 
                   fitHeightNlrobAndFixedWeight = FALSE, # 
                   fitDbhPrimary = FALSE, # 
                   fitDbhNlrobAndFixedWeight = TRUE, # 
                   fitSlowGams = FALSE)
...
if (psmeOptions$fitHeightPrimary)
{
  ...
  psmeHeightFromDiameter$chapmanRichardsBal = fit_gsl_nls("Chapman-Richards BA+L", TotalHt ~ 1.37 + (a1 + a1p * isPlantation + (a2 + a2p * isPlantation) * basalAreaLarger + (a3 + a3p * isPlantation) * standBasalAreaPerHectare) * (1 - exp(b1*DBH))^(b2 + b2p * isPlantation), psme2016, start = list(a1 = 72.9, a1p = -11.8, a2 = 0.087, a2p = 0.84, a3 = -0.0021, a3p = -0.073, b1 = -0.016, b2 = 1.26, b2p = -0.054))
  ...
  psmeHeightFromDiameter$sharmaZhangBal = fit_gsl_nls("Sharma-Zhang BA+L", TotalHt ~ 1.37 + (a1 + (a2 + a2p * isPlantation) * basalAreaLarger)*standBasalAreaPerHectare^b1 * (1 - exp(b2*tph^(b3 + b3p * isPlantation)*DBH))^(b4 + b4p * isPlantation), psme2016, start = list(a1 = 54, a2 = 0.03, a2p = 0.7, b1 = 0.05, b2 = -0.03, b3 = -0.07, b3p = -0.09, b4 = 1.55, b4p = -0.5))
  ...
  psmeHeightFromDiameter$weibullBal = fit_gsl_nls("Weibull BA+L", TotalHt ~ 1.37 + (a1 + (a2 + a2p * isPlantation) * basalAreaLarger + (a3 + a3p * isPlantation) * standBasalAreaPerHectare) * (1 - exp((b1 + b1p * isPlantation)*DBH^(b2 + b2p * isPlantation))), psme2016, start = list(a1 = 62.5, a2 = 0.04, a2p = 0.85, a3 = 0.02, a3p = -0.2, b1 = -0.0045, b1p = -0.0026, b2 = 1.3, b2p = -0.15))
  # GAMs
...
if (psmeOptions$fitHeightNlrobAndFixedWeight)
{
  ...
  psmeHeightFromDiameterNlrob$richards = fit_nlrob("unified Richards", TotalHt ~ 1.37 + (Ha + Hap*isPlantation) * (1 + ((1.37/Ha)^(1 - (d + dp*isPlantation)) - 1) * exp((-(kU + kUp * isPlantation) * DBH)/(d + dp*isPlantation)^((d + dp*isPlantation)/(1 - (d + dp*isPlantation)))))^(1/(1 - (d + dp*isPlantation))), psme2016, start = list(Ha = 65.3, Hap = -29.3, d = 0.574, dp = 0.151, kU = 0.0118, kUp = 0.0126))
  psmeHeightFromDiameterNlrob$sharmaParton = fit_nlrob("Sharma-Parton", TotalHt ~ 1.37 + a1*topHeight^(b1 + b1p * isPlantation)*(1 - exp((b2 + b2p * isPlantation)*(tph/standBasalAreaPerHectare)^(b3 + b3p * isPlantation)*DBH))^(b4 + b4p * isPlantation), psme2016, start = list(a1 = 14, b1 = 0.37, b1p = -0.06, b2 = -0.018, b2p = -0.025, b3 = 0.10, b3p = -0.25, b4 = 1.5, b4p = -0.42))
  psmeHeightFromDiameterNlrob$sharmaPartonBal = fit_gsl_nls("Sharma-Parton BA+L", TotalHt ~ 1.37 + (a1 + a1p * isPlantation)*topHeight^b1 * (1 - exp((b2 + b2p * isPlantation)*(tph/(standBasalAreaPerHectare + basalAreaLarger))^b3*DBH))^(b4 + b4p * isPlantation), psme2016, start = list(a1 = 22, a1p = -1.5, b1 = 0.27, b2 = -0.023, b2p = 0.004, b3 = -0.1, b4 = 1.55, b4p = -0.6)) # b1p, b3p not significant, job step size with nlrob()
  psmeHeightFromDiameterNlrob$sharmaPartonBalPhysio = fit_gsl_nls("Sharma-Parton BA+L physio", TotalHt ~ 1.37 + (a1 + a1p * isPlantation + a4 * elevation + a5 * slope + a6 * sin(3.14159/180 * aspect) + a7 * cos(3.14159/180 * aspect) + a8 * topographicShelterIndex)*topHeight^b1 * (1 - exp((b2 + b2p * isPlantation)*(tph/(standBasalAreaPerHectare + basalAreaLarger))^(b3 + b3p * isPlantation)*DBH))^(b4 + b4p * isPlantation), psme2016physio, start = list(a1 = 21, a1p = -0.4, a4 = -0.002, a5 = -0.04, a6 = 0.13, a7 = 0.2, a8 = 0.08, b1 = 0.28, b2 = -0.018, b2p = -0.006, b3 = 0.1, b3p = -0.2, b4 = 1.5, b4p = -0.5)) # b1p not significant
  psmeHeightFromDiameterNlrob$sharmaPartonPhysio = fit_nlrob("Sharma-Parton physio", TotalHt ~ 1.37 + (a1 + a4 * elevation + a5 * slope + a6 * sin(3.14159/180 * aspect) + a7 * cos(3.14159/180 * aspect) + a8 * topographicShelterIndex)*topHeight^b1 * (1 - exp(b2*(tph/standBasalAreaPerHectare)^(b3 + b3p * isPlantation)*DBH))^(b4 + b4p * isPlantation), psme2016physio, start = list(a1 = 18, a4 = -0.002, a5 = -0.03, a6 = 0.16, a7 = 0.2, a8 = 0.067, b1 = 0.31, b2 = -0.026, b3 = 0.0, b3p = -0.14, b4 = 1.66, b4p = -0.75), control = nls.control(tol = 1E-3)) # a1p, b1p, b2p not significant, b3 debatable, nlrob() step factor with a1p
  ...
  psmeHeightFromDiameterNlrob$sharmaZhangBal = fit_nlrob("Sharma-Zhang BA+L", TotalHt ~ 1.37 + (a1 + (a2 + a2p * isPlantation) * basalAreaLarger)*standBasalAreaPerHectare^b1 * (1 - exp(b2*tph^(b3 + b3p * isPlantation)*DBH))^(b4 + b4p * isPlantation), psme2016, start = list(a1 = 54, a2 = 0.03, a2p = 0.7, b1 = 0.05, b2 = -0.03, b3 = -0.07, b3p = -0.09, b4 = 1.55, b4p = -0.5)) # a1p, a2, b1p, b2p, b3 not significant
  ...
  psmeHeightFromDiameterNlrob$weibullBal = fit_nlrob("Weibull BA+L", TotalHt ~ 1.37 + (a1 + (a2 + a2p * isPlantation) * basalAreaLarger + (a3 + a3p * isPlantation) * standBasalAreaPerHectare) * (1 - exp((b1 + b1p * isPlantation)*DBH^(b2 + b2p * isPlantation))), psme2016, start = list(a1 = 63, a2 = 0.04, a2p = 0.9, a3 = 0.02, a3p = -0.2, b1 = -0.005, b1p = -0.0026, b2 = 1.3, b2p = -0.15)) # a2, a3 debatably significant
  ...
  #psmeHeightFromDiameterNlrob$weibullBalRelHt = fit_nlrob("Weibull BA+L RelHt", TotalHt ~ 1.37 + (a1 + (a2 + a2p * isPlantation) * basalAreaLarger + a3 * standBasalAreaPerHectare + a4 * pmin(relativeHeight, 1.25)) * (1 - exp((b1 + b1p * isPlantation)*DBH^(b2 + b2p * isPlantation))), psme2016, start = list(a1 = 3.0, a2 = 0.22, a2p = 0.95, a3 = 0.12, a4 = 53, b1 = -0.06, b1p = 0.04, b2 = 0.75, b2p = 0.1)) # a3p, a4p not significant
  #lapply(psmeHeightFromDiameter$sharmaPartonBalPhysio$fit, confint_nlrob)
  
  # gsl_nls(weights = fixed)
  psmeHeightFromDiameterGslNlsFixed = list(chapmanRichards = fit_gsl_nls("Chapman-Richards", TotalHt ~ 1.37 + (a1 + a1p * isPlantation) *(1 - exp(b1*DBH))^(b2 + b2p * isPlantation), psme2016defaultWeight, start = list(a1 = 65.3, a1p = -13.1, b1 = -0.022, b2 = 1.51, b2p = -0.31)))
  psmeHeightFromDiameterGslNlsFixed$chapmanRichardsBal = fit_gsl_nls("Chapman-Richards BA+L", TotalHt ~ 1.37 + (a1 + a1p * isPlantation + (a2 + a2p * isPlantation) * basalAreaLarger + (a3 + a3p * isPlantation) * standBasalAreaPerHectare) * (1 - exp(b1*DBH))^(b2 + b2p * isPlantation), psme2016defaultWeight, start = list(a1 = 72.9, a1p = -11.8, a2 = 0.087, a2p = 0.84, a3 = -0.0021, a3p = -0.073, b1 = -0.016, b2 = 1.26, b2p = -0.054))
  psmeHeightFromDiameterGslNlsFixed$chapmanRichardsBalPhysio = fit_gsl_nls("Chapman-Richards BA+L physio", TotalHt ~ 1.37 + (a1 + (a2 + a2p * isPlantation) * basalAreaLarger + (a3 + a3p * isPlantation) * standBasalAreaPerHectare + a4 * elevation + a5 * slope + a6 * sin(3.14159/180 * aspect) + a7 * cos(3.14159/180 * aspect) + a8 * topographicShelterIndex) * (1 - exp((b1 + b1p * isPlantation)*DBH))^(b2 + b2p * isPlantation), psme2016defaultWeightPhysio, start = list(a1 = 74.3, a2 = 0.096, a2p = 0.92, a3 = 0, a3p = 0, a4 = -0.015, a5 = -0.101, a6 = 0.793, a7 = 1.695, a8 = 0.183, b1 = -0.018, b1p = 0.005, b2 = 1.30, b2p = -0.154))
  psmeHeightFromDiameterGslNlsFixed$chapmanRichardsPhysio = fit_gsl_nls("Chapman-Richards physio", TotalHt ~ 1.37 + (a1 + a1p * isPlantation + a4 * elevation + a5 * sin(3.14159/180 * slope) + a6 * cos(3.14159/180 * aspect) + a7 * sin(3.14159/180 * aspect) + a8 * topographicShelterIndex) * (1 - exp(b1*DBH))^(b2 + b2p * isPlantation), psme2016defaultWeightPhysio, start = list(a1 = 68.5, a1p = -13.4, a4 = -0.0045, a5 = -8.09, a6 = 0.783, a7 = 0.766, a8 = 0.213, b1 = -0.022, b2 = 1.50, b2p = -0.31))
  psmeHeightFromDiameterGslNlsFixed$curtis = fit_gsl_nls("Curtis", TotalHt ~ 1.37 + (a1 + a1p*isPlantation) * DBH / (1 + DBH)^(b1 + b1p*isPlantation), psme2016defaultWeight, start = list(a1 = 1.409, a1p = -0.685, b1 = 0.200, b1p = -0.156))
  psmeHeightFromDiameterGslNlsFixed$hossfeld = fit_gsl_nls("Hossfeld IV", TotalHt ~ 1.37 + (a1 + a1p * isPlantation) / (1 + (b1 + b1p * isPlantation) * DBH^(b2 + b2p * isPlantation)), psme2016defaultWeight, start = list(a1 = 75.4, a1p = -11.4, b1 = 462, b1p = -322, b2 = -1.54, b2p = 0.28))
  psmeHeightFromDiameterGslNlsFixed$korf = fit_gsl_nls("Korf", TotalHt ~ 1.37 + a1*exp(b1*DBH^(b2 + b2p * isPlantation)), psme2016defaultWeight, start = list(a1 = 320, b1 = -7.83, b2 = -0.323, b2p = 0.084), control = gsl_nls_control(maxiter = 500))
  psmeHeightFromDiameterGslNlsFixed$michaelisMenten = fit_gsl_nls("Michaelis-Menten", TotalHt ~ 1.37 + (a1 + a1p*isPlantation)*DBH^b1 / (a2 + a2p * isPlantation + DBH^b1), psme2016defaultWeight, start = list(a1 = 87.8, a1p = -26.9, a2 = 236, a2p = -92.0, b1 = 1.30))
  psmeHeightFromDiameterGslNlsFixed$prodan = fit_gsl_nls("Prodan", TotalHt ~ 1.37 + DBH^2 / (a1*DBH^2 + (a2 + a2p * isPlantation)*DBH + a3 + a3p* isPlantation), psme2016defaultWeight, start = list(a1 = 0.012, a2 = 0.41, a2p = 0.47, a3 = 17.9, a3p = -14.6))
  psmeHeightFromDiameterGslNlsFixed$power = fit_gsl_nls("power", TotalHt ~ 1.37 + (a1 + a1p * isPlantation)*DBH^(b1 + b1p * isPlantation), psme2016defaultWeight, start = list(a1 = 1.15, a1p = -0.422, b1 = 0.85, b1p = 0.14))
  psmeHeightFromDiameterGslNlsFixed$ratkowsky = fit_gsl_nls("Ratkowsky", TotalHt ~ 1.37 + (a1 + a1p * isPlantation)*exp((b1 + b1p * isPlantation)/(DBH + b2 + b2p * isPlantation)), psme2016defaultWeight, start = list(a1 = 90.0, a1p = -25.8, b1 = -55.2, b1p = 14.5, b2 = 10.0, b2p = -1.52))
  psmeHeightFromDiameterGslNlsFixed$richards = fit_gsl_nls("unified Richards", TotalHt ~ 1.37 + (Ha + Hap*isPlantation) * (1 + ((1.37/Ha)^(1 - (d + dp*isPlantation)) - 1) * exp((-(kU + kUp * isPlantation) * DBH)/(d + dp*isPlantation)^((d + dp*isPlantation)/(1 - (d + dp*isPlantation)))))^(1/(1 - (d + dp*isPlantation))), psme2016defaultWeight, start = list(Ha = 65.3, Hap = -29.3, d = 0.574, dp = 0.151, kU = 0.0118, kUp = 0.0126))
  psmeHeightFromDiameterGslNlsFixed$sharmaParton = fit_gsl_nls("Sharma-Parton", TotalHt ~ 1.37 + a1*topHeight^(b1 + b1p * isPlantation)*(1 - exp((b2 + b2p * isPlantation)*(tph/standBasalAreaPerHectare)^(b3 + b3p * isPlantation)*DBH))^(b4 + b4p * isPlantation), psme2016defaultWeight, start = list(a1 = 37.66, b1 = 0.19, b1p = -0.123, b2 = -0.017, b2p = -0.026, b3 = 0.061, b3p = -0.259, b4 = 1.33, b4p = -0.22))
  psmeHeightFromDiameterGslNlsFixed$sharmaPartonBal = fit_gsl_nls("Sharma-Parton BA+L", TotalHt ~ 1.37 + (a1 + a1p * isPlantation)*topHeight^(b1 + b1p * isPlantation) * (1 - exp((b2 + b2p * isPlantation)*(tph/(standBasalAreaPerHectare + basalAreaLarger))^b3*DBH))^(b4 + b4p * isPlantation), psme2016defaultWeight, start = list(a1 = 20, a1p = -9, b1 = 0.3, b1p = 0.07, b2 = -0.023, b2p = -0.02, b3 = 0.0, b4 = 1.53, b4p = -0.15))
  psmeHeightFromDiameterGslNlsFixed$sharmaPartonBalPhysio = fit_gsl_nls("Sharma-Parton BA+L physio", TotalHt ~ 1.37 + (a1 + a1p * isPlantation + a4 * elevation + a5 * slope + a6 * sin(3.14159/180 * aspect) + a7 * cos(3.14159/180 * aspect) + a8 * topographicShelterIndex)*topHeight^b1 * (1 - exp((b2 + b2p * isPlantation)*(tph/(standBasalAreaPerHectare + basalAreaLarger))^(b3 + b3p * isPlantation)*DBH))^(b4 + b4p * isPlantation), psme2016defaultWeightPhysio, start = list(a1 = 52.6, a1p = -0.10, a4 = 0.00004, a5 = 0, a6 = 0.0090, a7 = 0.0032, a8 = 0.0040, b1 = 0.53, b2 = -0.025, b2p = -0.0090, b3 = 0.036, b3p = -0.19, b4 = 1.57, b4p = -0.51))
  psmeHeightFromDiameterGslNlsFixed$sharmaPartonPhysio = fit_gsl_nls("Sharma-Parton physio", TotalHt ~ 1.37 + (a1 + a4 * elevation + a5 * slope + a6 * sin(3.14159/180 * aspect) + a7 * cos(3.14159/180 * aspect) + a8 * topographicShelterIndex)*topHeight^b1 * (1 - exp(b2*(tph/standBasalAreaPerHectare)^(b3 + b3p * isPlantation)*DBH))^(b4 + b4p * isPlantation), psme2016defaultWeightPhysio, start = list(a1 = 20, a4 = -0.0008, a5 = -0.04, a6 = 0.15, a7 = 0.23, a8 = 0.095, b1 = 0.30, b2 = -0.025, b3 = -0.012, b3p = -0.12, b4 = 1.6, b4p = -0.65))
  psmeHeightFromDiameterGslNlsFixed$sharmaZhang = fit_gsl_nls("Sharma-Zhang", TotalHt ~ 1.37 + (a1 + a1p * isPlantation)*standBasalAreaPerHectare^(b1 + b1p * isPlantation)*(1 - exp((b2 + b2p * isPlantation)*tph^(b3 + b3p * isPlantation)*DBH))^(b4 + b4p * isPlantation), psme2016defaultWeight, start = list(a1 = 54, a1p = -33, b1 = 0.05, b1p = 0.2, b2 = -0.03, b2p = -0.05, b3 = -0.04, b3p = -0.16, b4 = 1.56, b4p = -0.48))
  psmeHeightFromDiameterGslNlsFixed$sharmaZhangBal = fit_gsl_nls("Sharma-Zhang BA+L", TotalHt ~ 1.37 + (a1 + (a2 + a2p * isPlantation) * basalAreaLarger)*standBasalAreaPerHectare^b1 * (1 - exp(b2*tph^(b3 + b3p * isPlantation)*DBH))^(b4 + b4p * isPlantation), psme2016defaultWeight, start = list(a1 = 54, a2 = 0.03, a2p = 0.7, b1 = 0.05, b2 = -0.03, b3 = -0.07, b3p = -0.09, b4 = 1.55, b4p = -0.5))
  psmeHeightFromDiameterGslNlsFixed$sibbesen = fit_gsl_nls("Sibbesen", TotalHt ~ 1.37 + (a1 + a1p * isPlantation)*DBH^((b1 + b1p * isPlantation)*DBH^(b2 + b2p * isPlantation)), psme2016defaultWeight, start = list(a1 = 0.0006, a1p = 0.17, b1 = 5.8, b1p = -3.5, b2 = -0.182, b2p = 0.050))
  psmeHeightFromDiameterGslNlsFixed$weibull = fit_gsl_nls("Weibull", TotalHt ~ 1.37 + (a1 + a1p * isPlantation)*(1 - exp((b1 + b1p * isPlantation)*DBH^(b2 + b2p * isPlantation))), psme2016defaultWeight, start = list(a1 = 64, a1p = -20, b1 = -0.005, b1p = -0.006, b2 = 1.3, b2p = -0.1))
  psmeHeightFromDiameterGslNlsFixed$weibullBal = fit_gsl_nls("Weibull BA+L", TotalHt ~ 1.37 + (a1 + (a2 + a2p * isPlantation) * basalAreaLarger + (a3 + a3p * isPlantation) * standBasalAreaPerHectare) * (1 - exp((b1 + b1p * isPlantation)*DBH^(b2 + b2p * isPlantation))), psme2016defaultWeight, start = list(a1 = 62.5, a2 = 0.04, a2p = 0.85, a3 = 0.02, a3p = -0.2, b1 = -0.0045, b1p = -0.0026, b2 = 1.3, b2p = -0.15))
  save(file = "trees/height-diameter/data/PSME TotalHt nlrob gsl_nls default.rdata", psmeHeightFromDiameterNlrob, psmeHeightFromDiameterGslNlsFixed)
}
...
if (psmeOptions$fitDbhPrimary)
{
  ...
  # nonlinear regressions
  psmeDiameterFromHeight$chapmanReplace = fit_gsl_nls("Chapman-Richards replace", DBH ~ (a1 + a1p * isPlantation)*(exp((b1 + b1p * isPlantation)*(TotalHt - 1.37)) - 1)^(b2 + b2p * isPlantation), psme2016, start = list(a1 = 75.6, a1p = -47.4, b1 = 0.016, b1p = 0.020, b2 = 0.792, b2p = -0.0780))
  psmeDiameterFromHeight$chapmanReplaceAat = fit_gsl_nls("Chapman-Richards replace ABA+T", DBH ~ (a1 + a1p * isPlantation + a2 * tallerApproxBasalArea + a3 * standBasalAreaApprox)*(exp((b1 + b1p * isPlantation)*(TotalHt - 1.37)) - 1)^(b2 + b2p * isPlantation), psme2016, start = list(a1 = 75.4, a1p = -44.6, a2 = 0.0058, a3 = -0.0544, b1 = 0.00166, b1p = 0.017, b2 = 0.788, b2p = -0.056))
  psmeDiameterFromHeight$chapmanReplaceRelHt = fit_gsl_nls("Chapman-Richards replace RelHt", DBH ~ (a1 + a1p * isPlantation + a9 * relativeHeight)*(exp(b1*(TotalHt - 1.37)^(b2 + b2p * isPlantation)) - 1), psme2016, start = list(a1 = 21.1, a1p = -6.56, a9 = 0.278, b1 = 0.170, b2 = 0.580, b2p = 0.0395))
  psmeDiameterFromHeight$chapmanRichards = fit_gsl_nls("Chapman-Richards inverse", DBH ~ (a1 + a1p * isPlantation)*log(1 - pmin(((b1 + b1p * isPlantation)*(TotalHt - 1.37))^b2, 0.9999)), psme2016, start = list(a1 = -121, a1p = 48.7, b1 = 0.00866, b1p = 0.00364, b2 = 0.781))
  psmeDiameterFromHeight$chapmanRichardsAat = fit_gsl_nls("Chapman-Richards inverse ABA+T", DBH ~ (a1 + a1p * isPlantation + a2 * tallerApproxBasalArea + a3 * standBasalAreaApprox)*log(1 - pmin(((b1 + b1p * isPlantation)*(TotalHt - 1.37))^b2, 0.9999)), psme2016, start = list(a1 = -136, a1p = 59.2, a2 = 0.109, a3 = 0.0684, b1 = 0.00811, b1p = 0.00403, b2 = 0.786))
  psmeDiameterFromHeight$chapmanRichardsPhysio = fit_gsl_nls("Chapman-Richards inverse physio", DBH ~ (a1 + a1p * isPlantation + a5 * sin(3.14159/180 * slope) + a8 * topographicShelterIndex)*log(1 - pmin(((b1 + b1p * isPlantation)*(TotalHt - 1.37))^b2, 0.9999)), psme2016physio, start = list(a1 = -13.8, a1p = -0.65, a5 = -3.58, a8 = 0.091, b1 = 0.019, b1p = 0.0062, b2 = 0.30))
  psmeDiameterFromHeight$chapmanRichardsRelHt = fit_gsl_nls("Chapman-Richards inverse RelHt", DBH ~ (a1 + a9 * relativeHeight)*log(1 - pmin(((b1 + b1p * isPlantation)*(TotalHt - 1.37))^(b2 + b2p * isPlantation), 0.9999)), psme2016, start = list(a1 = -1.67, a9 = -9.26, b1 = 0.020, b1p = 0.004, b2 = 0.004, b2p = 0.089))
  psmeDiameterFromHeight$michaelisMentenReplace = fit_gsl_nls("Michaelis-Menten replace", DBH ~ (a1 + a1p * isPlantation) * (TotalHt - 1.37)^(b1 + b1p * isPlantation) / (a2 + a2p * isPlantation - (TotalHt - 1.37)^(b1 + b1p * isPlantation)), psme2016, start = list(a1 = 190, a1p = -118, a2 = 67.3, a2p = -38.3, b1 = 0.78, b1p = -0.08))
  ...
  psmeDiameterFromHeight$powerAat = fit_gsl_nls("power ABA+T", DBH ~ (a1 + a1p * isPlantation + (a2 + a2p * isPlantation) * tallerApproxBasalArea + a3 * standBasalAreaApprox)*(TotalHt - 1.37)^(b1 + b1p*isPlantation), psme2016, start = list(a1 = 2.14, a1p = -0.051, a2 = -0.0065, a2p = -0.0038, a3 = 0.00085, b1 = 0.963, b1p = -0.053))
  ...
  #psmeDiameterFromHeight$schnute = fit_gsl_nls("Schnute inverse", DBH ~ -1/a1 * log(1 - (1 - exp(-a2))*(TotalHt^b1 - 1.37^b1)/(Ha^b1 - 1.3^b1)), psme2016, start = list(a1 = 0.002, a2 = 0.055, b1 = 1.05, Ha = 18.6))
  psmeDiameterFromHeight$sharmaParton = fit_gsl_nls("modified Sharma-Parton", DBH ~ a1*(TotalHt - 1.37)^(b1 + b1p * isPlantation)*(exp(b2*(tph/topHeight)^(b3 + b3p * isPlantation)*(TotalHt - 1.37)) - 1)^(b4 + b4p * isPlantation), psme2016, start = list(a1 = 9, b1 = 0.4, b1p = -0.14, b2 = 0.04, b3 = -0.06, b3p = 0.11, b4 = 0.3, b4p = 0.13), control = gsl_nls_control(maxiter = 250))
  ...
  psmeDiameterFromHeight$sibbesenReplaceAat = fit_gsl_nls("Sibbesen replace ABA+T", DBH ~ (a1 + a1p * isPlantation + a2 * tallerApproxBasalArea + (a3 + a3p * isPlantation) * standBasalAreaApprox)*(TotalHt - 1.37)^(b1*(TotalHt - 1.37)^(b2 + b2p * isPlantation)), psme2016, start = list(a1 = 3.898, a1p = -0.879, a2 = 0.00198, a3 = -0.00386, a3p = -0.00386, b1 = 0.527, b2 = 0.111, b2p = 0.0190))
  psmeDiameterFromHeight$sibbesenReplacePhysio = fit_gsl_nls("Sibbesen replace physio", DBH ~ (a1 + a1p * isPlantation + a4 * elevation + a6 * cos(3.14159/180 * aspect) + a7 * sin(3.14159/180 * aspect) + a8 * topographicShelterIndex)*(TotalHt - 1.37)^(b1*(TotalHt - 1.37)^(b2 + b2p * isPlantation)), psme2016physio, start = list(a1 = 3.812, a1p = -0.925, a4 = 0.00022, a6 = -0.0495, a7 = -0.0151, a8 = -0.00630, b1 = 0.520, b2 = 0.111, b2p = 0.0173))
  psmeDiameterFromHeight$sibbesenReplaceRelHt = fit_gsl_nls("Sibbesen replace RelHt", DBH ~ (a1 + a1p * isPlantation + a9 * relativeHeight)*(TotalHt - 1.37)^(b1*(TotalHt - 1.37)^(b2 + b2p * isPlantation)), psme2016, start = list(a1 = 3.90, a1p = -0.95, a9 = 0.085, b1 = 0.520, b2 = 0.109, b2p = 0.016))
...
{
  psmeDiameterFromHeightNlrob = list(chapmanReplace = fit_nlrob("Chapman-Richards replace", DBH ~ (a1 + a1p * isPlantation)*(exp((b1 + b1p * isPlantation)*(TotalHt - 1.37)) - 1)^b2, psme2016, start = list(a1 = 61, a1p = -22, b1 = 0.019, b1p = 0.008, b2 = 0.76))) # b2p not significant
  psmeDiameterFromHeightNlrob$chapmanReplaceAat = fit_nlrob("Chapman-Richards replace ABA+T", DBH ~ (a1 + a1p * isPlantation + a2 * tallerApproxBasalArea + a3 * standBasalAreaApprox)*(exp((b1 + b1p * isPlantation)*(TotalHt - 1.37)) - 1)^(b2 + b2p * isPlantation), psme2016, start = list(a1 = 100, a1p = -50, a2 = -0.1, a3 = -0.03, b1 = 0.014, b1p = 0.01, b2 = 0.8, b2p = -0.04)) # a2, a2p, a3p not significant
  ...
  psmeDiameterFromHeightNlrob$chapmanReplaceRelHt = fit_nlrob("Chapman-Richards replace RelHt", DBH ~ (a1 + a1p * isPlantation + a9 * relativeHeight)*(exp(b1*(TotalHt - 1.37)^(b2 + b2p * isPlantation)) - 1), psme2016, start = list(a1 = 30, a1p = -9, a9 = 5.5, b1 = 0.1, b2 = 0.63, b2p = 0.02)) # a9p not significant
  psmeDiameterFromHeightNlrob$chapmanRichards = fit_nlrob("Chapman-Richards inverse", DBH ~ (a1 + a1p * isPlantation)*log(1 - pmin(((b1 + b1p * isPlantation)*(TotalHt - 1.37))^b2, 0.9999)), psme2016, start = list(a1 = -110, a1p = 40, b1 = 0.009, b1p = 0.0036, b2 = 0.77)) # b2p not significant
  psmeDiameterFromHeightNlrob$chapmanRichardsAat = fit_nlrob("Chapman-Richards inverse ABA+T", DBH ~ (a1 + a1p * isPlantation + a2 * tallerApproxBasalArea + a3 * standBasalAreaApprox)*log(1 - pmin(((b1 + b1p * isPlantation)*(TotalHt - 1.37))^b2, 0.9999)), psme2016, start = list(a1 = -136, a1p = 59, a2 = 0.24, a3 = 0.05, b1 = 0.008, b1p = 0.004, b2 = 0.79)) # a2p, a3p, b2p not significant
  psmeDiameterFromHeightNlrob$chapmanRichardsPhysio = fit_nlrob("Chapman-Richards inverse physio", DBH ~ (a1 + a1p * isPlantation + a8 * topographicShelterIndex)*log(1 - pmin(((b1 + b1p * isPlantation)*(TotalHt - 1.37))^b2, 0.9999)), psme2016physio, start = list(a1 = -13, a1p = -45, a8 = 0.05, b1 = 0.0198, b1p = -0.0057, b2 = 0.74), control = nls.control(maxiter = 250)) # a4, a5, a6, a7 not significant
  psmeDiameterFromHeightNlrob$chapmanRichardsRelHt = fit_nlrob("Chapman-Richards inverse RelHt", DBH ~ (a1 + a9 * relativeHeight)*log(1 - pmin((b1*(TotalHt - 1.37))^(b2 + b2p * isPlantation), 0.9999)), psme2016, start = list(a1 = -3.4, a9 = -8.0, b1 = 0.025, b2 = 0.02, b2p = 0.15)) # a1p, b1p not significant
  psmeDiameterFromHeightNlrob$michaelisMentenReplace = fit_nlrob("Michaelis-Menten replace", DBH ~ (a1 + a1p * isPlantation) * (TotalHt - 1.37)^(b1 + b1p * isPlantation) / (a2 + a2p * isPlantation - (TotalHt - 1.37)^(b1 + b1p * isPlantation)), psme2016, start = list(a1 = 170, a1p = -80, a2 = 50, a2p = -20, b1 = 0.77, b1p = -0.04))
  ...
  psmeDiameterFromHeightNlrob$powerAat = fit_nlrob("power ABA+T", DBH ~ (a1 + a1p * isPlantation + (a2 + a2p * isPlantation) * tallerApproxBasalArea)*(TotalHt - 1.37)^(b1 + b1p*isPlantation), psme2016, start = list(a1 = 1.5, a1p = 0.9, a2 = -0.003, a2p = -0.008, b1 = 1.05, b1p = -0.17)) # a3 not significant
  psmeDiameterFromHeightNlrob$powerPhysio = fit_nlrob("power physio", DBH ~ (a1 + a1p * isPlantation + a6 * cos(3.14159/180 * aspect))*(TotalHt - 1.37)^(b1 + b1p * isPlantation), psme2016physio, start = list(a1 = 1.2, a1p = 1.0, a6 = -0.03, b1 = 1.1, b1p = -0.22)) # a4, a5, a7, a8 not significant
  psmeDiameterFromHeightNlrob$powerRelHt = fit_nlrob("power RelHt", DBH ~ (a1 + a9 * relativeHeight)*(TotalHt - 1.37)^(b1 + b1p * isPlantation), psme2016, start = list(a1 = 2.2, a9 = 0.6, b1 = 0.89, b1p = -0.09)) # a1p, a4p not significant
  ...
  psmeDiameterFromHeightNlrob$sibbesenReplace = fit_nlrob("Sibbesen replace", DBH ~ (a1 + a1p * isPlantation)*(TotalHt - 1.37)^(b1*(TotalHt - 1.37)^(b2 + b2p * isPlantation)), psme2016, start = list(a1 = 2.9, a1p = -0.2, b1 = 0.64, b2 = 0.08, b2p = -0.005))
  psmeDiameterFromHeightNlrob$sibbesenReplaceAat = fit_nlrob("Sibbesen replace ABA+T", DBH ~ (a1 + a2 * tallerApproxBasalArea + (a3 + a3p * isPlantation) * standBasalAreaApprox)*(TotalHt - 1.37)^(b1*(TotalHt - 1.37)^(b2 + b2p * isPlantation)), psme2016, start = list(a1 = 2.65, a2 = -0.005, a3 = 0.002, a3p = -0.006, b1 = 0.68, b2 = 0.07, b2p = -0.005)) # a1p, a2, a2p not significant
  psmeDiameterFromHeightNlrob$sibbesenReplacePhysio = fit_nlrob("Sibbesen replace physio", DBH ~ (a1 + a1p * isPlantation + a6 * cos(3.14159/180 * aspect))*(TotalHt - 1.37)^(b1*(TotalHt - 1.37)^b2), psme2016physio, start = list(a1 = 2.95, a1p = -0.32, a6 = -0.06, b1 = 0.63, b2 = 0.08)) # a4, a5, a7, a8, b2p not significant
  psmeDiameterFromHeightNlrob$sibbesenReplaceRelHt = fit_nlrob("Sibbesen replace RelHt", DBH ~ (a1 + a1p * isPlantation + a9 * relativeHeight)*(TotalHt - 1.37)^(b1*(TotalHt - 1.37)^b2), psme2016, start = list(a1 = 3.3, a1p = -0.6, a9 = 0.55, b1 = 0.60, b2 = 0.075)) # a9p, b2p not significant
  psmeDiameterFromHeightNlrob$weibull = fit_nlrob("Weibull inverse", DBH ~ ((a1 + a1p * isPlantation)*log(1 - pmin((b1 + b1p * isPlantation)*(TotalHt - 1.37), 0.9999)))^b2, psme2016, start = list(a1 = -360, a1p = 140, b1 = 0.01, b1p = 0.003, b2 = 0.81)) # b2p not significant
  #confint_nlrob(psmeDiameterFromHeightNlrob$sibbesenReplaceRelHt, level = 0.99)
  
  save(file = "trees/height-diameter/data/PSME DBH nlrob gsl_nls default.rdata", psmeDiameterFromHeightNlrob)
}
...
if (psmeOptions$fitHeightPrimary & psmeOptions$fitHeightNlrobAndFixedWeight & psmeOptions$fitDbhPrimary & psmeOptions$fitDbhNlrobAndFixedWeight)
{
  ...
  if (exists("psmeHeightFromDiameterGslNlsFixed") == FALSE) { load("trees/height-diameter/data/PSME TotalHt nlrob gsl_nls default.rdata") }
  #if (exists("psmeHeightFromDiameterGnls") == FALSE) { load("trees/height-diameter/data/PSME TotalHt GNLS.rdata") }
  psmeCoefficients = bind_rows(bind_rows(lapply(psmeHeightFromDiameter, get_list_coefficients)),
                               bind_rows(lapply(psmeHeightFromDiameterGslNlsFixed, get_list_coefficients, fitSet = "gsl_nls", fixedWeight = -1)),
  ...
  psmeResults = bind_rows(bind_rows(lapply(psmeHeightFromDiameter, get_list_stats)),
                          bind_rows(lapply(psmeHeightFromDiameterGslNlsFixed, get_list_stats, fitSet = "gsl_nls", fixedWeight = -1)),
                  ...
                  mutate(responseVariable = "height")
  rm(psmeHeightFromDiameter, psmeHeightFromDiameterGslNlsFixed, psmeHeightFromDiameterNlrob)
  gc()
  
  if (exists("psmeDiameterFromHeight") == FALSE) { load("trees/height-diameter/data/PSME DBH primary.rdata") }
  if (exists("psmeDiameterFromHeightNls") == FALSE) { load("trees/height-diameter/data/PSME DBH nlrob gsl_nls default.rdata") }
  ...
  psmeResults = bind_rows(psmeResults,
                          bind_rows(bind_rows(lapply(psmeDiameterFromHeight, get_list_stats)),
                                    ...
                                    get_model_stats(name = "Chapman-Richards replace BA+L", fitSet = "nlrob", fitting = "nlrob"),
                                    get_model_stats(name = "Chapman-Richards replace BA+L RelHt", fitSet = "nlrob", fitting = "nlrob"),
                                    get_model_stats(name = "Schnute inverse", fitSet = "nlrob", fitting = "nlrob"),
                                    get_model_stats(name = "modified Sharma-Parton", fitSet = "nlrob", fitting = "nlrob")) %>%
                            mutate(responseVariable = "DBH")) %>%
    mutate(species = "PSME")
...
if (htDiaOptions$includeInvestigatory)
{
  ...
  ggplot(psme2016) +
    ...
    #geom_line(aes(x = predict(psmeDiameterFromHeight$chapmanReplaceAat), y = TotalHt, color = "Chapman replace ABA+T", group = isPlantation), alpha = 0.5) +
...
if (psmeOptions$fitHeightPrimary & psmeOptions$fitHeightNlrobAndFixedWeight & psmeOptions$fitDbhPrimary & psmeOptions$fitDbhNlrobAndFixedWeight)
{
  ...
  psmeDiameterFromHeightPreferred = list(chapmanReplace = fit_nlrob("Chapman-Richards replace", DBH ~ (a1 + a1p * isPlantation)*(exp((b1 + b1p * isPlantation)*(TotalHt - 1.37)) - 1)^(b2 + b2p * isPlantation), psme2016, start = list(a1 = 75.6, a1p = -47.4, b1 = 0.016, b1p = 0.020, b2 = 0.792, b2p = -0.0780), folds = 1, repetitions = 1))
  ...
  psmeDiameterFromHeightPreferred$michaelisMentenReplace = fit_nlrob("Michaelis-Menten replace", DBH ~ (a1 + a1p * isPlantation) * (TotalHt - 1.37)^(b1 + b1p * isPlantation) / (a2 + a2p * isPlantation - (TotalHt - 1.37)^(b1 + b1p * isPlantation)), psme2016, start = list(a1 = 190, a1p = -118, a2 = 67.3, a2p = -38.3, b1 = 0.78, b1p = -0.08), folds = 1, repetitions = 1)
