...
if (psmeOptions$fitDbhPrimary)
{
  ...
  if (psmeOptions$fitDbhGslNlsAndGams)
  {
    ...
    psmeDiameterFromHeight$chapmanReplaceRelHt = fit_gsl_nls("Chapman-Richards replace RelHt", DBH ~ (a1 + (a9 + a9p * isPlantation) * relativeHeight)*(exp((b1)*(TotalHt - 1.37)) - 1)^(b2 + b2p * isPlantation), psme2016, start = list(a1 = 43, a9 = 18, a9p = -11, b1 = 0.02, b2 = 0.61, b2p = 0.18))
    ...
    psmeDiameterFromHeight$ruarkAbatRelHtPhysio = fit_gsl_nls("Ruark ABA+T RelHt physio", DBH ~ (a1 + a2 * tallerApproxBasalArea + a3 * standBasalAreaApprox + a6 * cos(3.14159/180 * aspect) + a9*relativeHeight)*(TotalHt - 1.37)^(b1 + b1p * isPlantation) * exp((b2 + b2p * isPlantation) * (TotalHt - 1.37)), psme2016physio, start = list(a1 = 2.4, a2 = -0.013, a3 = -0.0037, a6 = -0.05, a9 = 0.1, b1 = 0.94, b1p = -0.16, b2 = 0.001, b2p = 0.01)) # a1p, a2p, a3p, a9 not significant
    ...
    #psmeDiameterFromHeight$schnute = fit_gsl_nls("Schnute inverse", DBH ~ -1/a1 * log(1 - pmin((1 - exp(-a2))*(TotalHt^b1 - 1.37^b1)/(Ha^b1 - 1.37^b1), 0.9999)), psme2016, start = list(a1 = 0.003, a2 = 0.55, b1 = 1.05, Ha = 90))
    ...
    psmeDiameterFromHeight$sibbesenReplaceAbatRelHtPhysio = fit_gsl_nls("Sibbesen replace ABA+T RelHt physio", DBH ~ (a1 + a1p * isPlantation + a2 * tallerApproxBasalArea + a3 * standBasalAreaApprox + a6 * cos(3.14159/180 * aspect) + a9 * relativeHeight)*(TotalHt - 1.37)^(b1*(TotalHt - 1.37)^b2), psme2016physio, start = list(a1 = 3.1, a1p = -0.5, a2 = -0.01, a3 = -0.005, a6 = -0.06, a9 = 0.1, b1 = 0.65, b2 = 0.07)) # a2p, a3p, a9 not significant
    ...
    #bind_rows(lapply(psmeDiameterFromHeight$sibbesenReplaceAbat$fit, get_model_coefficients))
  }
...
if (psmeOptions$fitDbhNlrobAndFixedWeight)
{
  ...
  psmeDiameterFromHeightNlrob$chapmanReplaceRelHt = fit_nlrob("Chapman-Richards replace RelHt", DBH ~ (a1 + (a9 + a9p * isPlantation) * relativeHeight)*(exp((b1)*(TotalHt - 1.37)) - 1)^(b2 + b2p * isPlantation), psme2016, start = list(a1 = 43, a9 = 18, a9p = -11, b1 = 0.02, b2 = 0.61, b2p = 0.18))
  ...
  #psmeDiameterFromHeightNlrob$schnute = fit_gsl_nls("Schnute inverse", DBH ~ -1/a1 * log(1 - (1 - exp(-a2))*(TotalHt^b1 - 1.37^b1)/(Ha^b1 - 1.37^b1)), psme2016, start = list(a1 = 0.002, a2 = 0.055, b1 = 1.05, Ha = 18.6)) # converges from red alder values but fails to reconverge (singular gradient or NaN-inf with nls())
  ...
  psmeDiameterFromHeightGslNlsDefault$chapmanReplaceRelHt = fit_gsl_nls("Chapman-Richards replace RelHt", DBH ~ (a1 + (a9 + a9p * isPlantation) * relativeHeight)*(exp((b1)*(TotalHt - 1.37)) - 1)^(b2 + b2p * isPlantation), psme2016defaultWeight, start = list(a1 = 43, a9 = 18, a9p = -11, b1 = 0.02, b2 = 0.61, b2p = 0.18))
  ...
  #psmeDiameterFromHeightGslNlsDefault$schnute = fit_gsl_nls("Schnute inverse", DBH ~ -1/a1 * log(1 - (1 - exp(-a2))*(TotalHt^b1 - 1.37^b1)/(Ha^b1 - 1.37^b1)), psme2016defaultWeight, start = list(a1 = 0.002, a2 = 0.055, b1 = 1.05, Ha = 86)) # NaN-inf
...
if (psmeOptions$fitDbhMixed)
{
  ...
  #psmeDiameterFromHeightMixed$chapmanReplaceRelHt = fit_nlme("Chapman-Richards replace RelHt", DBH ~ (a1 + (a9 + a9p * isPlantation) * relativeHeight + a1r)*(exp((b1)*(TotalHt - 1.37)) - 1)^(b2 + b2p * isPlantation), psme2016, 
  #                                                           fixedFormula = a1 + a9 + a9p + b1 + b2 + b2p ~ 1, randomFormula = a1r ~ 1,
  #                                                           start = list(fixed = c(a1 = 43, a9 = 18, a9p = -11, b1 = 0.02, b2 = 0.61, b2p = 0.18)), control = nlmeControl(tolerance = 0.1, pnlsTol = 1, msTol = 0.01)) # step halving
  ...
  #psmeDiameterFromHeightMixed$schnute = fit_nlme("Schnute inverse", DBH ~ -1/a1 * log(1 - pmin((1 - exp(-a2))*(TotalHt^b1 - 1.37^b1)/((Ha + Har)^b1 - 1.37^b1), 0.9999)), psme2016, 
...
if (psmeOptions$fitHeightPrimary & psmeOptions$fitHeightNlrobAndFixedWeight & psmeOptions$fitDbhPrimary & psmeOptions$fitDbhNlrobAndFixedWeight)
{
  ...
  psmeHeightFromDiameterPreferred$ratkowsky = fit_gsl_nls("Ratkowsky", TotalHt ~ 1.37 + (a1 + a1p * isPlantation)*exp((b1 + b1p * isPlantation)/(DBH + b2 + b2p * isPlantation)), psme2016, start = list(a1 = 90.0, a1p = -25.8, b1 = -55.2, b1p = 14.5, b2 = 10.0, b2p = -1.52), folds = 1, repetitions = 1)
