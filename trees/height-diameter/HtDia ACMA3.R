...
acmaHeightFromDiameterChapmanRichardsBalPhysio = nlrob(TotalHt ~ 1.37 + (a1 + a2 * basalAreaLarger + a3 * elevation + a4 * slope + a5 * sin(3.14159/180 * aspect) + a6 * cos(3.14159/180 * aspect) + a7 * topographicShelterIndex) * (1 - exp(b1*DBH))^(b2 + b2p * isPlantation), acma2016physio, start = list(a1 = 33.2, a2 = 0.156, a3 = -0.009, a4 = -0.130, a5 = 1.872, a6 = 0.276, a7 = 0, b1 = -0.024, b2 = 0.988, b2p = -0.129), weights = pmin(DBH^-0.8, 1)) # a1p, a2p, a3, a4, a6, b1p not significant
...
acmaHeightFromDiameterSharmaParton = nlrob(TotalHt ~ 1.37 + (a1 + a1p*isPlantation)*topHeight^(a2 + a2p * isPlantation)*(1 - exp(b1*(tph/standBasalAreaPerHectare)^b2*DBH))^(b3 + b3p * isPlantation), acma2016, start = list(a1 = 5.86, a1p = 39.2, a2 = 0.36, a2p = -0.502, b1 = -0.080, b2 = -0.398, b3 = 0.979, b3p = -0.116), weights = pmin(DBH^-0.8, 1)) # b1p, b2p not significant
acmaHeightFromDiameterSharmaPartonBal = nlrob(TotalHt ~ 1.37 + a1*topHeight^a2 * (1 - exp(b1*(tph/(standBasalAreaPerHectare + basalAreaLarger))^b2*DBH))^(b3 + b3p * isPlantation), acma2016, start = list(a1 = 46.0, a2 = -0.12, b1 = -0.054, b2 = -0.375, b3 = 0.960, b3p = -0.102), weights = pmin(DBH^-0.8, 1)) # a1p, a2p, b1p, b2p not significant
acmaHeightFromDiameterSharmaPartonBalPhysio = nlrob(TotalHt ~ 1.37 + (a1 + a1p * isPlantation)*topHeight^(a2 + a2p * isPlantation) * (1 + a3 * elevation + a4 * sin(3.14159/180 * aspect) + a5 * cos(3.14159/180 * aspect) + a6 * topographicShelterIndex) * (1 - exp(b1*(tph/(standBasalAreaPerHectare + basalAreaLarger))^b2*DBH))^(b3 + b3p * isPlantation), acma2016physio, start = list(a1 = 7.67, a1p = 48.0, a2 = 0.34, a2p = -0.48, a3 = -0.0003, a4 = 0.041, a5 = 0.0067, a6 = -0.0018, b1 = -0.056, b2 = -0.402, b3 = 0.948, b3p = -0.096), weights = pmin(DBH^-0.8, 1)) # a2, a5, a6, b1p, b2p not significant
acmaHeightFromDiameterSharmaPartonPhysio = nlrob(TotalHt ~ 1.37 + (a1 + a1p * isPlantation)*topHeight^(a2 + a2p * isPlantation) * (1 + a3 * elevation + a4 * sin(3.14159/180 * aspect) + a5 * cos(3.14159/180 * aspect) + a6 * topographicShelterIndex) * (1 - exp(b1*(tph/(standBasalAreaPerHectare))^b2*DBH))^(b3 + b3p * isPlantation), acma2016physio, start = list(a1 = 5.21, a1p = 41.5, a2 = 0.416, a2p = -0.540, a3 = -0.0003, a4 = 0.042, a5 = 0.0055, a6 = -0.0019, b1 = -0.083, b2 = -0.419, b3 = 0.968, b3p = -0.122), weights = pmin(DBH^-0.8, 1)) # a2, a5, a6, b1p, b2p not significant
acmaHeightFromDiameterSharmaZhang = nlrob(TotalHt ~ 1.37 + (a1 + a1p * isPlantation)*standBasalAreaPerHectare^a2*(1 - exp(b1*tph^b2*DBH))^(b3 + b3p * isPlantation), acma2016, start = list(a1 = 17.1, a1p = 1.36, a2 = 0.112, b1 = -0.044, b2 = -0.053, b3 = 1.038, b3p = -0.118), weights = pmin(DBH^-0.8, 1)) # a2p, b1p, b2p not significant
acmaHeightFromDiameterSharmaZhangBal = nlrob(TotalHt ~ 1.37 + a1*standBasalAreaPerHectare^a2 * (1 + a3 * basalAreaLarger) * (1 - exp(b1*tph^b2*DBH))^(b3 + b3p * isPlantation), acma2016, start = list(a1 = 22.0, a2 = 0.050, a3 = 0.002, b1 = -0.044, b2 = -0.064, b3 = 1.070, b3p = -0.197), weights = pmin(DBH^-0.8, 1)) # a1p, a2p, a3, a3p, b1p, b2p not significant
...
acmaHeightFromDiameterResults = bind_rows(as_row(acmaHeightFromDiameterChapmanRichards),
                                          ...
                                          as_row(acmaHeightFromDiameterSharmaZhangBal),
...
acmaDiameterFromHeightChapmanFormRelHt = gsl_nls(DBH ~ (a1 + a2 * relativeHeight)*(exp(b1*(TotalHt - 1.37)^b2) - 1), acma2016, start = list(a1 = 241000, a2 = -114000, b1 = 0.000007, b2 = 1.23), weights = pmin(TotalHt^if_else(isPlantation, -1.6, -0.9), 0.5), control = gsl_nls_control(maxiter = 50)) # step factor from nls() and nlrob()
...
acmaDiameterFromHeightSharmaParton = gsl_nls(DBH ~ a1*(TotalHt - 1.37)^a2*(exp(b1*(tph/topHeight)^b2*(TotalHt - 1.37)) - 1)^b3, acma2016, start = list(a1 = 0.0001, a2 = 3.8, b1 = 0.0174, b2 = 0.112, b3 = -2.4), weights = pmin(TotalHt^if_else(isPlantation, -1.6, -0.9), 0.5)) # NaN-inf from nls() at nls_multstart() positions, NaN-inf or code error with nlrob()
